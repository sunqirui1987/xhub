        ...
        <div key={label} className="flex flex-col min-w-0">
          ...
          {value ? (
            <div className="flex min-w-0 items-center gap-1">
              ...
              <CopyButton value={value} label={t("Copy {label}", { label })} iconClassName="size-3.5" />
            </div>
          ) : (
  ...
  return (
    <div>
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <h3 className="m-0 flex items-center gap-1 text-2xl font-semibold">
              {data.keyName}
              <CopyButton value={data.keyName} label={t("Copy Key Alias")} iconClassName="size-4" />
            </h3>
            {isBlocked && (
          ...
          <div className="flex min-w-0 items-center gap-1">
            <span className="min-w-0 break-words text-muted-foreground">{t("Key ID: {value0}", { value0: (data.keyId) })}</span>
            <CopyButton value={data.keyId} label={t("Copy Key ID")} iconClassName="size-3.5" />
          </div>
        </div>
        {canModifyKey && (
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger render={<Button variant="outline" size="icon" aria-label={t("More key actions")} />}>
      ...
      <div className="flex items-stretch gap-10" style={{ marginBottom: 40 }}>
        ...
        <div className="flex min-w-0 flex-col gap-4">
          <LabeledField label={t("Created At")} value={data.createdAt} icon={<Calendar className="size-3.5" />} />
          <LabeledField
            label={t("Created By")}
        ...
        <div className="flex min-w-0 flex-col gap-4">
          <LabeledField label={t("Last Updated")} value={data.lastUpdated} icon={<Clock className="size-3.5" />} />
          <LabeledField label={t("Last Active")} value={data.lastActive} icon={<Zap className="size-3.5" />} />
        </div>
