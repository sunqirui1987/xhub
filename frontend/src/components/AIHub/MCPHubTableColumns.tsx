...
export const getMCPHubTableColumns = ({ onServerClick }: MCPHubTableColumnsDeps): ColumnDef<MCPServerData>[] => [
  ...
  },
  {
    ...
    sortingFn: "alphanumeric",
    cell: ({ row }) => (
      <StatusBadge tone={STATUS_TONES[row.original.status] || "neutral"} label={row.original.status || t("unknown")} />
    ),
  },
  {
    id: "allowed_tools",
      ...
      return (
        <div className="flex flex-col gap-1">
          <span className="text-xs font-medium">
            {tools.length > 0 ? t("{value0} tool{value1}", { value0: (tools.length), value1: (tools.length !== 1 ? "s" : "") }) : t("All tools")}
          </span>
          {tools.length > 0 && (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <StatusBadge tone={isPublic ? "success" : "neutral"} label={isPublic ? t("Yes") : t("No")} />;
    },
  },
  {
    id: "actions",
