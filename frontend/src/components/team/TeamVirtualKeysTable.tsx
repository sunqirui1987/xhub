  ...
  const columns: ColumnDef<KeyResponse>[] = useMemo(
    () => [
      ...
      },
      {
        ...
        header: ({ column }) => <DataTableSortHeader column={column} title={t("Key Alias")} variant="header-cycle" />,
        size: 150,
        enableSorting: true,
        cell: (info) => {
      ...
      },
      {
        ...
        header: ({ column }) => <DataTableSortHeader column={column} title={t("Budget (USD)")} variant="header-cycle" />,
        size: 110,
        enableSorting: true,
        cell: (info) => (
          <MoneyCell value={info.getValue() as number | null} decimals={0} emptyText={t("Unlimited")} showZero />
        ),
      },
      {
        id: "budget_reset_at",
      ...
      },
      {
        ...
        cell: (info) => {
          ...
          const emptyModelsBadge = !scope.hasModelAccess ? (
            <SimpleTooltip content={t("Scoped to {value0} routes; this key cannot call any models", { value0: (scope.label) })}>
                    ...
                    <>
                      <div className="flex items-start">
                        {models.length > 3 && (
                          <button
                            type="button"
                            aria-label={expandedAccordions[info.row.id] ? t("Collapse models") : t("Expand models")}
            ...
            toolbar={(table) => (
              <>
                <DataTableToolbar
                  ...
                  searchPlaceholder={t("Search by key alias or ID…")}
                  onRefresh={() => refetch?.()}
                  isRefreshing={isFetching}
                  onOpenFilters={() => setFiltersOpen(true)}
                  filterLabels={{ user_id: t("User ID"), key_hash: t("Key ID") }}
                />
                <DataTableFilterDrawer
                  table={table}
                  open={filtersOpen}
                  ...
                  title={t("Filters")}
                  description={t("Narrow down keys for {value0}", { value0: (teamAlias ?? t("this team")) })}
                >
                  {({ get, set }) => (
                    <>
