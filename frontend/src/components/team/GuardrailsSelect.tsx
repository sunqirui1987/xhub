...
export const GuardrailsSelect: React.FC<GuardrailsSelectProps> = ({
  ...
  const groups: GuardrailGroup[] = grouped
    ? [
        { label: t("Global"), icon: true, items: [...globalGuardrails] },
        { label: t("Other"), icon: false, items: [...otherGuardrails] },
      ]
            ...
            <>
              {chips.map((option) => (
                <ComboboxChip key={option.name} aria-label={option.name}>
                  {globalGuardrailNames.has(option.name) && <Globe className="size-3" aria-label={t("Global guardrail")} />}
                  {option.name}
                </ComboboxChip>
              ))}
