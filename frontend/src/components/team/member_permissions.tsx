...
const MemberPermissions: React.FC<MemberPermissionsProps> = ({ teamId, accessToken, canEditTeam }) => {
  ...
  const fetchPermissions = async () => {
    try {
      ...
      toast.fromError(t("Failed to load permissions"));
  ...
  const handleSave = async () => {
    try {
      ...
      toast.fromError(t("Failed to update permissions"));
  ...
  if (loading) {
    return <div className="p-6 text-center">{t("Loading permissions...")}</div>;
  }
  ...
  return (
    <Card className="block bg-card shadow-md rounded-md p-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b pb-4 mb-6">
        <h3 className="text-lg font-medium text-foreground mb-2 sm:mb-0">{t("Member Permissions")}</h3>
      ...
      <p className="mb-6 text-sm text-muted-foreground">
        {t("Control what team members can do when they are not team admins.")}
      </p>
      ...
      {hasPermissions ? (
        ...
        <div className="py-12">
          <p className="text-center text-sm text-muted-foreground">{t("No permissions available")}</p>
        </div>
      )}
    </Card>
  );
