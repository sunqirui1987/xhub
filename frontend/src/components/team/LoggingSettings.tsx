...
const CALLBACK_TYPE_ITEMS = [
  ...
  { value: "failure", label: t("Failure Only") },
  { value: "success_and_failure", label: t("Success & Failure") },
];
    ...
    return (
      <div className="mt-6 pt-4 border-t border-border">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <span className="text-sm font-medium text-foreground">{t("Integration Parameters")}</span>
        </div>
          ...
          {Object.entries(dynamicParams).map(([paramName, paramType]) => (
            <div key={paramName} className="space-y-2">
              <label className="text-sm font-medium text-foreground capitalize flex items-center space-x-1">
                ...
                {paramType === "password" && <Badge variant="secondary">{t("Sensitive")}</Badge>}
                {paramType === "number" && <Badge variant="secondary">{t("Number")}</Badge>}
              </label>
              {paramType === "number" && (
                <span className="text-xs text-muted-foreground">{t("Value must be between 0 and 1")}</span>
              )}
              {paramType === "number" ? (
                <NumericalInput
                  step={0.01}
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-base font-semibold text-foreground">{t("Disabled Callbacks")}</span>
          <SimpleTooltip content={t("Select callbacks to disable for this key. Disabled callbacks will not receive any logging data.")}>
            <Info className="size-4 text-muted-foreground cursor-help" />
          </SimpleTooltip>
        </div>

        <div className="space-y-2">
          <label className="text-sm font-medium text-foreground">{t("Disabled Callbacks")}</label>
          <Select multiple value={disabledCallbacks} onValueChange={handleDisabledCallbacksChange}>
            <SelectTrigger className="w-full">
              <SelectValue placeholder={t("Select callbacks to disable")}>
                {(selected: string[]) => (selected.length === 0 ? t("Select callbacks to disable") : selected.join(", "))}
              </SelectValue>
            </SelectTrigger>
            <SelectContent>
      ...
      <div className="flex justify-between items-center">
        <div className="flex items-center space-x-2">
          ...
          <SimpleTooltip content={t("Configure callback logging integrations for this team.")}>
            <Info className="size-4 text-muted-foreground cursor-help" />
          </SimpleTooltip>
        </div>
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <div className="flex items-center space-x-2">
                  ...
                  <span className="text-sm font-medium">{callbackDisplayName || t("New Integration")} Configuration</span>
                </div>
                <Button
                  variant="ghost"
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground">{t("Integration Type")}</label>
                    <Select
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-foreground">{t("Event Type")}</label>
                    <Select
                    ...
                    >
                      <SelectTrigger aria-label={t("Event Type")} className="w-full">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
      ...
      {value.length === 0 && (
        <div className="text-center py-12 text-muted-foreground border-2 border-dashed border-border rounded-lg bg-muted/30">
          ...
          <div className="text-sm text-muted-foreground">
            {t("Click \"Add Integration\" to configure logging for this team")}
          </div>
        </div>
      )}
    </div>
