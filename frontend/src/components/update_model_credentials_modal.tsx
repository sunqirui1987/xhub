...
const updateCredentialsSchema = z.object({
  api_key: z.string().min(1, t("Enter a new API key")),
});
...
export default function UpdateModelCredentialsModal({
  ...
  const handleSubmit = async (values: UpdateCredentialsValues) => {
    ...
    if (!apiKey) {
      toast.fromError(t("Enter a new API key"));
      return;
    }
    setIsSaving(true);
    try {
      ...
      toast.fromError(t("Failed to update API key"));
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={(nextOpen) => !nextOpen && close()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[520px]">
        ...
        <span className="block mb-4 text-sm text-muted-foreground">
          {t("Update this model's API key. Only the new key is sent; the rest of the deployment configuration is left untouched.")}
        </span>
        <Alert variant="warning" className="mb-4">
          <TriangleAlert />
          <AlertTitle>
            {t("Only the API key is rotated here. Models that authenticate with an Azure AD token, AWS credentials, or a Vertex service-account JSON aren't supported yet; update those from the model's LiteLLM Params for now.")}
          </AlertTitle>
        </Alert>
        <form onSubmit={form.handleSubmit(handleSubmit)}>
          <FieldGroup>
            <FormField control={form.control} name="api_key" label={t("New API Key")}>
              {({ ref, ...field }) => (
                <PasswordInput {...field} ref={ref} placeholder={t("Enter the new API key")} autoComplete="new-password" />
              )}
            </FormField>
          </FieldGroup>
