...
const parseGitHubSource = (url: URL, subPath?: string): SkillSourcePreview | null => {
  ...
  const repoPreview: SkillSourcePreview = {
    ...
    label: t("GitHub repo — {repoFull}", { repoFull }),
  ...
  if (isTreeOrBlob) {
    ...
    return {
      ...
      label: t("GitHub subdir — {repoFull} @ {path}", { repoFull, path }),
  ...
  if (normalized !== "") {
    ...
    return {
      ...
      label: t("GitHub subdir — {repoFull} @ {normalized}", { repoFull, normalized }),
...
const parseRawGitSource = (url: URL, subPath?: string): SkillSourcePreview | null => {
  ...
  if (normalized !== "") {
    ...
    return {
      parsed: { source: "git-subdir", url: repoUrl, path: normalized },
      label: t("Git subdir — {repoUrl} @ {normalized}", { repoUrl, normalized }),
      suggestedName: toKebabCase(lastSegment(normalized)),
    };
  }
  ...
  return {
    ...
    label: t("Git repo — {repoUrl}", { repoUrl }),
...
const parseArchiveSource = (url: URL): SkillSourcePreview => ({
  ...
  label: t("Zip archive — {value0}{value1}", { value0: (url.host), value1: (url.pathname) }),
