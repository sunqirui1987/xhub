...
export const MODEL_BUDGET_PERIOD_OPTIONS = [
  ...
  { value: "1mo", label: t("Calendar month") },
];
...
export function ModelMaxBudgetEditor({
  ...
  const blurb = (
    <div className="text-xs text-muted-foreground">
      {premiumUser
        ? t("Cap spend per model over its own window. A budget set on the bare model name also covers the provider-prefixed spelling of that model.")
          ...
          <div key={entry.id} className="relative rounded-lg border border-border bg-muted p-4">
            ...
            {spent !== undefined && (
              <div className="text-[11px] text-muted-foreground mt-2 ml-1">
                {t("Current window spend: ${spent}{value1}", { spent, value1: (entry.budgetLimit !== null && t("of ${value0}", { value0: (entry.budgetLimit) })) })}
              </div>
            )}
          </div>
        );
...
export function ModelMaxBudgetField({ hint, ...editorProps }: ModelMaxBudgetFieldProps) {
  return (
    <Field>
      <FieldLabel>
        <span title={hint}>{t("Per-Model Budgets")}</span>
      </FieldLabel>
