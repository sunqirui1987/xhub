    ...
    return (
      <div>
        <div className="text-xs text-muted-foreground mb-2">
          {t("When a model exceeds its per-model budget, requests automatically reroute to fallback models")}
        </div>
        <Button variant="outline" size="sm" onClick={addEntry}>
          <Plus className="w-3 h-3" />
          {t("Add Budget Fallback")}
        </Button>
      </div>
    );
  }
      ...
      {entries.map((entry) => {
        ...
        return (
          <div key={entry.id} className="relative rounded-lg border border-border bg-muted p-4">
            ...
            <div className="mb-3">
              <label className="block text-xs font-medium text-muted-foreground mb-1">{t("Primary Model")}</label>
              <SearchSelect
            ...
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">{t("Fallback Models")}</label>
              <MultiSelect
                ...
                placeholder={entry.primaryModel ? t("Select fallback models") : t("Select a primary model first")}
                emptyText={t("No models found")}
              ...
              {entry.fallbackModels.length > 1 && (
                <div className="text-[10px] text-muted-foreground mt-1 ml-1">
                  {t("Tried in order; first model still within its own budget is used")}
                </div>
              )}
            </div>
          </div>
