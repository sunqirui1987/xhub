...
const reuseCredentialsSchema = z.object({
  credential_name: z.string().min(1, t("Credential name is required")),
});
  ...
  return (
    <Dialog open={isVisible} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>{t("Reuse Credentials")}</DialogTitle>
        </DialogHeader>
        <TooltipProvider>
          <form onSubmit={form.handleSubmit(handleSubmit)} noValidate>
            <FieldGroup>
              <FormField control={form.control} name="credential_name" label={t("Credential Name:")}>
                {({ ref, ...field }) => (
                  <Input {...field} ref={ref} placeholder={t("Enter a friendly name for these credentials")} />
                )}
              </FormField>
              ...
              <div className="flex items-center justify-end">
                <div className="flex gap-2.5">
                  ...
                  <Button type="submit">{t("Reuse Credentials")}</Button>
                </div>
              </div>
            </FieldGroup>
          </form>
