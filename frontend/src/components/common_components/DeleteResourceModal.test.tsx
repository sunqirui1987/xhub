...
describe("DeleteResourceModal", () => {
  ...
  it("renders typed-confirm copy from catalogs in both locales", () => {
    const textOf = (locale: "en" | "zh-CN") =>
      translate(locale, "common.typeToConfirm").replace("{value}", "DELETE");
    const match =
      (locale: "en" | "zh-CN") =>
      (_: string, node: Element | null) =>
        node?.tagName === "P" && node.textContent === textOf(locale);

    const first = renderWithProviders(<DeleteResourceModal {...defaultProps} requiredConfirmation="DELETE" />);
    expect(screen.getByText(match("en"))).toBeInTheDocument();
    first.unmount();
    ...
    expect(screen.getByText(match("zh-CN"))).toBeInTheDocument();
    expect(screen.queryByText(match("en"))).not.toBeInTheDocument();
    second.unmount();
    setActiveLocale("en");
  });
