  ...
  return [
    {
      ...
      label: t("Default"),
      description: t("Best effort throughput - no error if we're overallocating {lower} (Team/Key Limits checked at runtime).", { lower }),
    },
    {
      value: "guaranteed_throughput",
      label: t("Guaranteed throughput"),
      description: t("Guaranteed throughput - raise an error if we're overallocating {lower} (also checks model-specific limits)", { lower }),
    },
    {
      value: "dynamic",
      label: t("Dynamic"),
    ...
    <div className={className}>
      ...
      >
        <SelectTrigger id={controlId} className="w-full" aria-invalid={ariaInvalid} aria-describedby={ariaDescribedBy}>
          <SelectValue placeholder={t("Select rate limit type")}>
            {(selected: string | null) =>
              selected === null ? t("Select rate limit type") : plainLabels[selected] ?? selected
            }
          </SelectValue>
        </SelectTrigger>
        <SelectContent>
