...
export function LoggingSettingsView({
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-foreground">{t("Logging Integrations")}</span>
        ...
        {loggingConfigs.length > 0 ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="block text-xs text-info">
                        {t("{value0} parameters configured", { value0: (Object.keys(config.callback_vars).length) })}</span>
                    </div>
                  </div>
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted border border-border">
            ...
            <span className="text-muted-foreground text-sm">{t("No logging integrations configured")}</span>
          </div>
        )}
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-foreground">{t("Disabled Callbacks")}</span>
        ...
        {disabledCallbacks.length > 0 ? (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <span className="block text-xs text-destructive">{t("Disabled for this key")}</span>
                    </div>
                  </div>
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted border border-border">
            ...
            <span className="text-muted-foreground text-sm">{t("No callbacks disabled")}</span>
          </div>
        )}
      </div>
    </div>
    ...
    return (
      <div className={`bg-card border border-border rounded-lg p-6 ${className}`}>
        <div className="flex items-center gap-2 mb-6">
          <div>
            <span className="block font-semibold text-foreground">{t("Logging Settings")}</span>
  ...
  return (
    <div className={`${className}`}>
      <span className="block font-medium text-foreground mb-3">{t("Logging Settings")}</span>
      {content}
    </div>
  );
}
