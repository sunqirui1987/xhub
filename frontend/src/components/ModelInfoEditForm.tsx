...
const PTU_EDIT_FIELDS: PtuEditField[] = [
  { name: PTU_COUNT_FIELD, label: t("PTU Count"), input: "number", placeholder: "e.g. 15", isCount: true },
  { name: PTU_RATE_FIELD, label: t("Cost per PTU / Hour (USD)"), input: "number", placeholder: "e.g. 2.00" },
  { name: PTU_START_FIELD, label: t("PTU Effective From (UTC)"), input: "datetime" },
  { name: PTU_END_FIELD, label: t("PTU Effective To (UTC)"), input: "datetime" },
];
...
const DocsHint: React.FC<{ text: string; href: string }> = ({ text, href }) => {
  if (/litellm\.ai|berri\.ai|github\.com\/BerriAI/i.test(href)) {
  ...
  const textField = (name: ModelEditFieldName, label: string, placeholder: string, stored: unknown) => (
    <div>
      ...
      {isEditing ? (
        ...
        <Display>{(stored as string) || t("Not Set")}</Display>
      )}
    </div>
  );
    ...
    <TooltipProvider>
      <form onSubmit={submit}>
        <div className="space-y-4">
          <div className="space-y-4">
            {textField("model_name", t("Model Name"), t("Enter model name"), localModelData.model_name)}
            {textField(
              "litellm_model_name",
              t("LiteLLM Model Name"),
              t("Enter LiteLLM model name"),
              localModelData.litellm_model_name,
            )}

            {pricingField("input_cost", t("Input Cost (per 1M tokens)"), t("Enter input cost"))}
            {pricingField("output_cost", t("Output Cost (per 1M tokens)"), t("Enter output cost"))}
                  ...
                  {isEditing ? (
                    ...
                    <Display>
                      {(ptuField.input === "datetime"
                        ...
                        : localModelData?.model_info?.[ptuField.name]) ?? t("Not Set")}
                    </Display>
                  )}
                </div>
              ))}
            ...
            {pricingField(
              "cache_read_cost",
              t("Cache Read Cost (per 1M tokens)"),
              t("Defaults to Input Cost if blank"),
              t("If left blank on save, defaults to Input Cost."),
            )}
            {pricingField(
              "cache_write_cost",
              t("Cache Write Cost (per 1M tokens)"),
              t("Defaults to Input Cost if blank"),
              t("If left blank on save, defaults to Input Cost (backend falls back to input_cost_per_token)."),
            )}

            {textField("api_base", t("API Base"), t("Enter API base"), localModelData.litellm_params?.api_base)}
            {textField(
              "custom_llm_provider",
              t("Custom LLM Provider"),
              t("Enter custom LLM provider"),
            ...
            {numberField("tpm", t("TPM (Tokens per Minute)"), t("Enter TPM"), localModelData.litellm_params?.tpm)}
            {numberField("rpm", t("RPM (Requests per Minute)"), t("Enter RPM"), localModelData.litellm_params?.rpm)}
            {numberField("max_retries", t("Max Retries"), t("Enter max retries"), localModelData.litellm_params?.max_retries)}
            {numberField("timeout", t("Timeout (seconds)"), t("Enter timeout"), localModelData.litellm_params?.timeout)}
            {numberField(
              "stream_timeout",
              t("Stream Timeout (seconds)"),
              t("Enter stream timeout"),
              localModelData.litellm_params?.stream_timeout,
            )}

            <div>
              <FieldLabel>{t("Model Access Groups")}</FieldLabel>
              {isEditing ? (
                tagsField(
                  "model_access_group",
                  (modelAccessGroups ?? []).map((group) => ({ value: group, label: group })),
                  t("Select existing groups or type to create new ones"),
                )
              ) : (
                <Display>
            ...
            <div>
              <FieldLabel>
                {t("Guardrails")}
                <DocsHint
                  text={t("Apply safety guardrails to this model to filter content or enforce policies")}
              ...
              {isEditing ? (
                tagsField(
                  ...
                  t("Select existing guardrails or type to create new ones"),
                )
              ) : (
                <Display>
            ...
            <div>
              <FieldLabel>
                {t("Attached Knowledge Bases (RAG)")}
                <DocsHint
                  text={t("Vector stores used for RAG. Every request to this model will automatically retrieve context from these knowledge bases.")}
            ...
            <div>
              ...
              {isEditing ? (
                tagsField(
                  ...
                  t("Select existing tags or type to create new ones"),
                )
              ) : (
                <Display>
            ...
            <div>
              <FieldLabel>{t("Existing Credentials")}</FieldLabel>
              {isEditing ? (
                    ...
                    return (
                      ...
                      >
                        <SelectTrigger id={id} className="w-full" onBlur={onBlur}>
                          <SelectValue placeholder={t("Select or search for existing credentials")} />
                        </SelectTrigger>
                        <SelectContent>
                          {items.map((item) => (
                ...
                <Display>{localModelData.litellm_params?.litellm_credential_name || t("Manual")}</Display>
              )}
            </div>

            {isWildcardModel && (
              <div>
                <FieldLabel>{t("Health Check Model")}</FieldLabel>
                {isEditing ? (
                  <FormField control={form.control} name="health_check_model">
                    {({ id, value, onChange, onBlur }) => (
                      ...
                      >
                        <SelectTrigger id={id} className="w-full" onBlur={onBlur}>
                          <SelectValue placeholder={t("Select existing health check model")} />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  <Display>{localModelData.model_info?.health_check_model || t("Not Set")}</Display>
                )}
              </div>
            )}
            ...
            {isEditing ? (
              ...
              <div>
                <FieldLabel>{t("Cache Control")}</FieldLabel>
                <Display>
                      ...
                      <div className="mt-2">
                        {localModelData.litellm_params.cache_control_injection_points.map((point: any, i: number) => (
                          <div key={i} className="mb-1 text-sm text-muted-foreground">
                            {t("Location:")} {point.location},{point.role && <span> Role: {point.role}</span>}
            ...
            <div>
              <FieldLabel>{t("Model Info")}</FieldLabel>
              {isEditing ? (
            ...
            <div>
              <FieldLabel>
                {t("LiteLLM Params")}
                <DocsHint
                  text={t("Optional litellm params used for making a litellm.completion() call. Some params are automatically added by LiteLLM.")}
            ...
            <div>
              <FieldLabel>{t("Team")}</FieldLabel>
              <Display>
                {teamAlias
                  ? `${teamAlias} (${modelData.model_info.team_id})`
                  : modelData.model_info.team_id || t("Not Set")}
              </Display>
            </div>
          </div>
