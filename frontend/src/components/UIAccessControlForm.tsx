...
const uiAccessControlSchema = z
  ...
  .superRefine((values, ctx) => {
    ...
    ctx.addIssue({
      ...
      message: t("Please enter the restricted SSO group"),
    });
  });
...
const UI_ACCESS_MODE_OPTIONS = [
  { value: "all_authenticated_users", label: t("All Authenticated Users") },
  { value: "restricted_sso_group", label: t("Restricted SSO Group") },
] as const;
...
const UIAccessControlForm: React.FC<UIAccessControlFormProps> = ({ accessToken, onSuccess }) => {
  ...
  const handleUIAccessSubmit = async (formValues: UIAccessControlFormValues) => {
    ...
    try {
      ...
      toast.fromError(t("Failed to save UI access settings"));
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={form.handleSubmit(submitMountedValues)} noValidate>
          <FieldGroup>
            <FormField
              ...
              label={labelWithHint(t("UI Access Mode"), t("Controls who can access the UI interface"))}
            >
              {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
                ...
                >
                  ...
                  >
                    <SelectValue placeholder={t("Select access mode")} />
                  </SelectTrigger>
                  <SelectContent>
            ...
            {uiAccessModeType === "restricted_sso_group" && (
              <FormField control={form.control} name="restricted_sso_group" label={t("Restricted SSO Group")}>
                {({ ref, value, ...field }) => (
            ...
            <FormField
              ...
              label={labelWithHint(
                t("SSO Group JWT Field"),
                t("JWT field name that contains team/group information. Use dot notation to access nested fields."),
              )}
            >
              {({ ref, value, ...field }) => <Input {...field} ref={ref} value={value ?? ""} placeholder={t("groups")} />}
            </FormField>
          </FieldGroup>
