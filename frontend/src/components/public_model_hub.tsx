...
import { t } from "@/i18n";
import { isBrandOutbound, scrubBrand } from "@/i18n/brand";
...
const PublicModelHub: React.FC<PublicModelHubProps> = ({ accessToken, isEmbedded = false }) => {
  ...
  const [pageTitle, setPageTitle] = useState<string>("XHub");
  ...
  useEffect(() => {
    const initializeAndFetch = async () => {
      ...
      const fetchPublicModelHubInfo = async () => {
        ...
        setCustomDocsDescription(
          publicModelHubInfo.custom_docs_description ? scrubBrand(publicModelHubInfo.custom_docs_description) : null,
        );
          ...
          <div className={isEmbedded ? "w-full p-6" : "w-full px-8 py-12"}>
            ...
            {!isEmbedded && (
              <Card className="mb-10 p-8 bg-card border border-border rounded-lg shadow-xs">
                <h2 className="text-2xl font-semibold mb-6 text-foreground">{t("About")}</h2>
                ...
                {litellmVersion ? (
                  <p className="text-sm text-muted-foreground">v{litellmVersion}</p>
                ) : null}
              </Card>
            )}
            ...
            {usefulLinks && Object.keys(usefulLinks).length > 0 && (
              <Card className="mb-10 p-8 bg-card border border-border rounded-lg shadow-xs">
                <h2 className="text-2xl font-semibold mb-6 text-foreground">{t("Useful Links")}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {Object.entries(usefulLinks || {})
                    ...
                    .filter(({ url }) => !isBrandOutbound(url))
                    .map(({ title, url }) => (
                      <button
                        key={title}
                      ...
                      >
                        ...
                        <p className="text-sm font-medium break-words">{scrubBrand(title)}</p>
                      </button>
                    ))}
                </div>
              </Card>
            ...
            {!isEmbedded && (
              <Card className="mb-10 p-8 bg-card border border-border rounded-lg shadow-xs">
                <h2 className="text-2xl font-semibold mb-6 text-foreground">{t("Health and Endpoint Status")}</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <p className="text-success font-medium text-sm">{t("Service status: {serviceStatus}", { serviceStatus })}</p>
                </div>
              </Card>
            )}
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="public-hub-tabs">
                ...
                <TabsContent value="models">
                  <div className="flex justify-between items-center mb-8">
                    <h2 className="text-2xl font-semibold text-foreground">{t("Available Models")}</h2>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8 p-6 bg-muted rounded-lg border border-border">
                    <div>
                      <div className="flex items-center space-x-2 mb-3">
                        <p className="text-sm font-medium text-foreground">{t("Search Models:")}</p>
                        <Tooltip>
                          <TooltipTrigger render={<Info className="w-4 h-4 text-muted-foreground cursor-help" />} />
                          <TooltipContent side="top">
                            {t("Finds every published model whose name contains what you type, across all pages. Try 'grok', 'claude', 'gpt-4', or 'sonnet'")}
                          </TooltipContent>
                        </Tooltip>
                      </div>
                      <div className="relative">
                        ...
                        <input
                          ...
                          aria-label={t("Search model names")}
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium mb-3 text-foreground">{t("Provider:")}</p>
                      <Combobox
                        multiple
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium mb-3 text-foreground">{t("Mode:")}</p>
                      <MultiSelect
                        options={modeOptions}
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium mb-3 text-foreground">{t("Features:")}</p>
                      <MultiSelect
                  ...
                  <DataTable
                    ...
                    noDataMessage={
                      <PublicHubEmptyState
                        title={models.hasActiveQuery ? t("No matching models") : t("No models available")}
                        body={
                ...
                {hasAgents && (
                  <TabsContent value="agents">
                    <div className="flex justify-between items-center mb-8">
                      <h2 className="text-2xl font-semibold text-foreground">{t("Available Agents")}</h2>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 p-6 bg-muted rounded-lg border border-border">
                      <div>
                        <div className="flex items-center space-x-2 mb-3">
                          <p className="text-sm font-medium text-foreground">{t("Search Agents:")}</p>
                          <Tooltip>
                            <TooltipTrigger render={<Info className="w-4 h-4 text-muted-foreground cursor-help" />} />
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium mb-3 text-foreground">{t("Skills:")}</p>
                        <MultiSelect
                    ...
                    {selectedModel.model_group.includes("*") && (
                      <div className="bg-info/10 border border-info/20 rounded-lg p-4 mb-4">
                        <div className="flex items-start space-x-2">
                          ...
                          <div>
                            <p className="font-medium text-info mb-2">{t("Wildcard Routing")}</p>
                            <p className="text-sm text-info mb-2">
                              {t("This model uses wildcard routing. You can pass any value where you see the")}{" "}
                              <code className="bg-info/15 px-1 py-0.5 rounded-sm text-xs">*</code> {t("symbol.")}
                            </p>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(() => {
                        ...
                        if (capabilities.length === 0) {
                          return <p className="text-muted-foreground">{t("No special capabilities listed")}</p>;
                        }
              ...
              {selectedAgent && (
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="mb-4">
                      <p className="text-sm font-medium mb-2 text-foreground">{t("Step 1: Retrieve Agent Card")}</p>
                      <div className="bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto">
    ...
    try:
        ...
        logger.warning(
            ...
            exc_info=True,
                    ...
                    <div>
                      <p className="text-sm font-medium mb-2 text-foreground">{t("Step 2: Call the Agent")}</p>
                      <div className="bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto">
                        <pre className="text-xs">
                          {`client = A2AClient(
