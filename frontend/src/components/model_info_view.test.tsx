...
describe("ModelInfoView", () => {
  ...
  it("should display model information in overview tab", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText("XHub Model")).toBeInTheDocument();
  ...
  it("should display form fields in edit mode", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByPlaceholderText("Enter XHub model name")).toBeInTheDocument();
    });
  });
  ...
  it("should display XHub Params section", async () => {
    render(<ModelInfoView {...DEFAULT_ADMIN_PROPS} />, { wrapper });
    await waitFor(() => {
      expect(screen.getByText("XHub Params")).toBeInTheDocument();
    });
  });
  ...
  it("should keep selector credential and ignore litellm_credential_name from XHub Params json", async () => {
    const user = userEvent.setup();
    render(<ModelInfoView {...DEFAULT_ADMIN_PROPS} />, { wrapper });
  ...
  it("blocks the save when the XHub Params box does not hold valid JSON", async () => {
    const user = userEvent.setup();
    render(<ModelInfoView {...DEFAULT_ADMIN_PROPS} />, { wrapper });
  ...
  describe("payload parity pins", () => {
    ...
    it("routes each edited field into its own payload key", async () => {
      ...
      await user.clear(screen.getByPlaceholderText("Enter XHub model name"));
      await user.type(screen.getByPlaceholderText("Enter XHub model name"), "gpt-4o");
    ...
    it("sends the edited XHub extra params", async () => {
      const user = userEvent.setup();
      await enterEditMode(user);
