...
const semanticRouterShape = {
  ...
  auto_router_default_model: z
    .string()
    .nullable()
    .pipe(z.string({ error: "Default model is required" }).min(1, t("Default model is required"))),
  auto_router_embedding_model: z
    .string()
    .nullable()
    .pipe(z.string({ error: "Embedding model is required" }).min(1, t("Embedding model is required"))),
};
