...
export function MCPServerPermissions({
  ...
  const mergedItems = [
    ...directServerIds.map((server) => ({ type: "server", value: server, tooltip: t("Full ID: {server}", { server }) })),
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-foreground">{t("MCP Servers")}</p>
        <Badge variant={blocksAllMcpServers ? "destructive" : "secondary"}>
          {blocksAllMcpServers ? t("Blocked") : grantsAllProxyMcpServers ? t("All") : totalCount}
        </Badge>
      </div>
      ...
      {blocksAllMcpServers ? (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-destructive/10 border border-destructive/20">
          ...
          <p className="text-destructive text-sm">
            {t("No MCP servers — this key is blocked from all MCP servers, including its team's servers")}
          </p>
        </div>
      ) : grantsAllProxyMcpServers ? (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-info/10 border border-info/20">
          <ServerIcon className="h-4 w-4 text-info" />
          <p className="text-info text-sm">{t("All Proxy MCP Servers")}</p>
        </div>
      ) : totalCount > 0 ? (
                  ...
                  {hasToolRestrictions && (
                    <div className="flex items-center gap-1 shrink-0 whitespace-nowrap">
                      ...
                      <span className="text-xs text-muted-foreground">
                        {toolsForServer.length === 1 ? t("tool") : t("tools")}
                      </span>
                      {isExpanded ? (
                  ...
                  >
                    ...
                    {toolCount > 0 && (
                      <div className="flex items-center gap-1 shrink-0 whitespace-nowrap">
                        ...
                        <span className="text-xs text-muted-foreground">{toolCount === 1 ? t("tool") : t("tools")}</span>
                        {isExpanded ? (
                          <ChevronDownIcon className="h-3.5 w-3.5 text-muted-foreground ml-0.5" />
                        ) : (
                          <ChevronRightIcon className="h-3.5 w-3.5 text-muted-foreground ml-0.5" />
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted border border-border">
          ...
          <p className="text-muted-foreground text-sm">{t("No MCP servers, access groups, or toolsets configured")}</p>
        </div>
      )}
    </div>
  );
