...
export function AgentPermissions({
  ...
  const mergedItems = [
    ...agents.map((agent) => ({ type: "agent", value: agent, tooltip: t("Full ID: {agent}", { agent }) })),
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2">
        ...
        <p className="text-sm font-semibold text-foreground">{t("Agents")}</p>
      ...
      {totalCount > 0 ? (
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted border border-border">
          ...
          <p className="text-muted-foreground text-sm">{t("No agents or access groups configured")}</p>
        </div>
      )}
    </div>
  );
