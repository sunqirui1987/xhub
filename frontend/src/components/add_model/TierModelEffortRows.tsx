  ...
  return (
    <div className="mt-2 space-y-1">
      <div className="flex items-center gap-1">
        <span className="text-xs font-medium text-muted-foreground">{t("Reasoning effort")}</span>
        <SimpleTooltip
          content={t("Sent as reasoning_effort on requests this tier routes to the model, overriding the caller's value. Default leaves the request untouched.")}
        >
        ...
        <div key={model} className="flex items-center justify-between gap-2">
          ...
          >
            <SelectTrigger
              size="sm"
              className="w-36"
              aria-label={t("Reasoning effort for {model} in the {tierLabel} tier", { model, tierLabel })}
            >
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
