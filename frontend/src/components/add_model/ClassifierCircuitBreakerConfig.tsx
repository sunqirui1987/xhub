    ...
    <div className="space-y-2 rounded-md border border-border p-3">
      <div className="flex items-center gap-2">
        <Switch
          ...
          aria-label={t("Classifier circuit breaker")}
        />
        <strong className="font-semibold">{t("Classifier circuit breaker")}</strong>
      </div>
      <span className="block text-xs text-muted-foreground">
        {t("After one classifier timeout, use the fallback immediately for every session until a recovery probe succeeds. Enabled by default.")}
      </span>
      {enabled && (
        <div>
          <Label htmlFor={COOLDOWN_ID} className="block mb-1 font-semibold">
            {t("Circuit breaker cooldown (seconds)")}
          </Label>
          <Input
            id={COOLDOWN_ID}
            type="text"
