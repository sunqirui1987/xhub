    ...
    <>
      <div className="flex items-center gap-2 mb-2">
        <Switch
          ...
          aria-label={t("Add a non-reasoning tier")}
        />
        <strong className="font-semibold">{t("Add a non-reasoning tier")}</strong>
      </div>
      <span className="block text-xs text-muted-foreground">
        {t("Adds NON_REASONING below Simple, for operational agent traffic that relays or reformats information rather than reasoning about it. Escalation still moves up out of it when a request needs more. {value0}", { value0: (!available && t("Requires the LLM classification method.")) })}
      </span>
