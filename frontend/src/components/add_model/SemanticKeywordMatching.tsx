      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="font-medium">{t("Semantic keyword matching")}</span>
            <SimpleTooltip content={t("Recognize related phrasing beyond exact keyword matches by comparing embeddings instead of plain text. Overrides direct keyword matching")}>
        ...
        <Switch checked={enabled} onCheckedChange={onEnabledChange} aria-label={t("Semantic keyword matching")} />
      </div>

      {enabled && (
        <div className="grid gap-4 md:grid-cols-2 mt-4 pt-4 border-t border-border">
          <div>
            <span className="mb-1 block text-sm font-medium">{t("Embedding model")}</span>
            <SearchSelect
              ...
              emptyText={t("No embedding models found")}
              aria-label={t("Embedding model")}
              allowClear={false}
              className={embeddingModelMissing ? "border-destructive" : undefined}
            />
            {embeddingModelMissing && <span className="text-xs text-destructive">{t("An embedding model is required")}</span>}
          </div>
          <div>
            <span className="mb-1 block text-sm font-medium">{t("Minimum match score")}</span>
            <Input
              type="number"
            ...
            <span className="mt-1 block text-xs text-muted-foreground">
              {t("Match only at or above this similarity score.")}
            </span>
          </div>
        </div>
      )}
