...
export function serializeRouterConfig(config: RouterConfig | null): string {
  if (config?.routes?.some((route) => !(route.name ?? route.model))) {
    throw new Error(t("Please select a model for every route"));
  }
    ...
    <div className="flex min-h-9 w-full flex-wrap items-center gap-1.5 rounded-md border border-input bg-transparent px-2.5 py-1.5 shadow-xs transition-[color,box-shadow] focus-within:border-ring focus-within:ring-3 focus-within:ring-ring/50 dark:bg-input/30">
      {value.map((utterance) => (
        <Badge key={utterance} variant="secondary" className="max-w-full gap-1 pr-1">
          ...
          <button
            type="button"
            aria-label={t("Remove {utterance}", { utterance })}
      ...
      <input
        ...
        placeholder={value.length === 0 ? t("Type an utterance and press Enter...") : undefined}
      ...
      <div className="w-full space-y-6">
        <div className="flex w-full flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <HelpTooltip content={t("Configure routing logic to automatically select the best model based on user input patterns")} />
          </div>
        ...
        {routes.length === 0 ? (
          <Card>
            <CardContent className="py-8 text-center text-muted-foreground">
              {t("No routes configured. Click \"Add Route\" to get started.")}
            </CardContent>
          </Card>
        ) : (
                  ...
                  <div className="flex items-center gap-2 px-4 py-3">
                    ...
                    >
                      ...
                      <span className="truncate text-base font-medium">
                        {t("Route {value0}: {value1}", { value0: (index + 1), value1: (route.model || "Unnamed") })}
                      </span>
                    </CollapsibleTrigger>
                    <Button
                      type="button"
