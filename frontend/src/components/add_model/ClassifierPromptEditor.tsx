...
const ClassifierPromptEditor: React.FC<ClassifierPromptEditorProps> = ({
  ...
  const openEditor = useCallback(async () => {
    ...
    try {
      ...
      toast.fromError(t("Could not load the default classifier prompt"));
      setIsOpen(false);
    } finally {
      setIsLoading(false);
    }
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="rounded-md border border-warning/30 bg-warning/10 p-3 text-sm text-warning">
            ...
            <p className="mt-2">
              {t("This is the legacy whole-prompt mode: the tier definitions and labels are frozen into this text, so renaming a tier or changing the rubric will not update it. Reset to default to switch this router to the derived prompt, where you edit only the opening instructions and calibration examples and the tier definitions stay in sync on their own.")}
            </p>
          </div>
          ...
          <div className="mt-2 flex items-center justify-between">
            <p className="text-xs text-muted-foreground">
              {t("Prefilled from the {classificationRubric} rubric this router would send at a context window of {contextWindowSize}.", { classificationRubric, contextWindowSize })}</p>
            <Button
              type="button"
              size="sm"
              variant="link"
