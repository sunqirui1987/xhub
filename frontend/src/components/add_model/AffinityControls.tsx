  ...
  return (
    <>
      <div className="flex items-center gap-2 mb-2">
        ...
        />
        <strong className="font-semibold">{t("Pin a session to one deployment per model group")}</strong>
      </div>
      ...
      <div style={{ maxWidth: 320 }}>
        <label className="block text-sm font-medium mb-1" htmlFor="session-affinity-ttl">
          {t("How long a pin survives idle (seconds)")}
        </label>
        <Input
        ...
        <span className="block text-xs mt-1 text-muted-foreground">
          {t("Refreshes after every request that reuses a pin. Empty tracks the backend default of {DEFAULT_SESSION_AFFINITY_TTL_SECONDS} seconds.", { DEFAULT_SESSION_AFFINITY_TTL_SECONDS })}</span>
      </div>
    </>
  );
};
