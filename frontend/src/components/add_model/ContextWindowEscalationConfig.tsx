    ...
    <>
      <div className="flex items-center gap-2 mb-2">
        <Switch
          ...
          aria-label={t("Escalate oversized prompts to a tier that fits")}
        />
        <strong className="font-semibold">{t("Escalate oversized prompts to a tier that fits")}</strong>
      </div>
      <span className="block text-xs mb-3 text-muted-foreground">
        {t("When a prompt provably cannot fit the decided tier's context windows, route it to the lowest tier whose window holds it instead of letting the provider reject it. Off means requests dispatch on complexity alone.")}
      </span>
      {enabled && (
        <div style={{ maxWidth: 320 }}>
          ...
          <span className="block text-xs mt-1 text-muted-foreground">
            {t("Fraction of a model's window the counted prompt must fit within, above 0 up to 1. Empty tracks the backend default of 0.95.")}
          </span>
        </div>
      )}
    </>
