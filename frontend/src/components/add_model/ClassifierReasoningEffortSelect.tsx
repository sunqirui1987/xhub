  ...
  return (
    <div>
      <div className="flex items-center gap-2 mb-1">
        <strong className="font-semibold">{t("Reasoning Effort")}</strong>
        <SimpleTooltip content={t("Sent only to the classifier call. Default leaves the classifier deployment or provider setting unchanged.")}>
          <Info className="size-4 text-muted-foreground" />
        </SimpleTooltip>
      </div>
      <Select
        items={[
          { value: PROVIDER_DEFAULT, label: t("Default") },
      ...
      >
        <SelectTrigger aria-label={t("Reasoning effort for classifier model {model}", { model })} className="w-full">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
          <SelectItem value={PROVIDER_DEFAULT}>{t("Default")}</SelectItem>
          {options.map((effort) => (
      ...
      {status === "unverified" && (
        <p className="mt-1 text-xs text-amber-700 dark:text-amber-400">
          {t("This saved effort cannot be verified for the selected model. Choose Default unless you have confirmed provider support.")}
        </p>
      )}
      {status === "unsupported" && (
        <p className="mt-1 text-xs text-destructive">
          {t("This saved effort is not supported by every deployment in the selected model group. Choose Default or a supported value before saving.")}
        </p>
      )}
    </div>
  );
