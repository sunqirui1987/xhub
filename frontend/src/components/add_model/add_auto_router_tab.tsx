...
const autoRouterSchema = (requiresTeamScope: boolean) =>
  z.object({
    auto_router_name: z.string().min(1, t("Auto router name is required")),
    team_id: z
      .string()
      .nullable()
      .refine((teamId) => !requiresTeamScope || Boolean(teamId), t("Please select a team to continue")),
...
const AddAutoRouterTab: React.FC<AddAutoRouterTabProps> = ({
  ...
  const templateItems = React.useMemo(
    () => [
      ...
      { value: "custom", label: t("Custom Configuration") },
    ],
    [sortedPresetOptions],
  );
  ...
  const handleAutomaticSetup = () => {
    ...
    toast.success(t("Automatic setup created"), { description: tierConfigSummary(automaticRouterConfig) });
  };
  ...
  const submitRecommendedRouter = async (name: string) => {
    ...
    if (!(await form.trigger(validatedFields))) {
      toast.fromError(t("Please fill in all required fields"));
      return;
    }
  ...
  const handleAutoRouterSubmit = async () => {
    ...
    if (!name) {
      ...
      toast.fromError(t("Please enter an Auto Router Name"));
      return;
    }
  ...
  const handleTestConnection = () => {
    ...
    if (targets.length === 0) {
      toast.fromError(t("Please select at least one model for a complexity tier"));
      return;
    }
    ...
    <TooltipProvider>
            ...
            <FieldGroup>
              <div>
                <FormField
                  ...
                  label={labelWithHint(t("Auto Router Name"), t("Unique name for this auto router configuration"))}
                >
                  {({ ref, ...field }) => (
                    <Input {...field} ref={ref} placeholder={t("e.g., smart_router, auto_router_1")} />
                  )}
                </FormField>
                ...
                {!automaticSetupLoading && automaticRouterConfig && (
                  <div className="mt-5 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border bg-muted px-4 py-3">
                    <div>
                      <p className="text-sm font-medium text-foreground">{t("Not sure where to start?")}</p>
                      <p className="text-sm text-muted-foreground">{t("Let us pick models for each complexity tier.")}</p>
                    </div>
                ...
                <div className="mt-5">
                  <label className="block text-sm font-medium text-foreground mb-2">{t("Template")}</label>
                  <Select
                    items={templateItems}
                  ...
                  >
                    <SelectTrigger data-testid="template-selector" className="w-full">
                      <SelectValue placeholder={t("Choose a template or select Custom to define your own")} />
                    </SelectTrigger>
                    <SelectContent>
                      ...
                      <SelectItem value="custom" label={t("Custom Configuration")}>
                        <div>
                          <div className="font-medium">{t("Custom Configuration")}</div>
                          <div className="text-xs text-muted-foreground">{t("Define your auto router from scratch")}</div>
                        </div>
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  ...
                  {presetsPending && <div className="text-xs mt-1 text-muted-foreground">{t("Loading templates...")}</div>}
                  {presetsUnavailable && (
                    <div className="text-xs mt-1 text-destructive">
                      {t("Could not load templates, so only Custom Configuration is shown.")}{" "}
              ...
              {requiresTeamScope && (
                <FormField
                  ...
                  label={labelWithHint(
                    t("Select Team"),
                    t("Select the team this auto router belongs to. Only keys for this team will be able to call it."),
                  )}
                >
              ...
              {isAdmin && (
                <FormField
                  ...
                  label={labelWithHint(
                    ...
                    t("Use model access groups to control who can access this auto router"),
                  )}
                >
      ...
      <Dialog open={isRoutingTestVisible} onOpenChange={(open) => !open && setIsRoutingTestVisible(false)}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[760px]">
          <DialogHeader>
            <DialogTitle>{t("Test Routing")}</DialogTitle>
          </DialogHeader>
