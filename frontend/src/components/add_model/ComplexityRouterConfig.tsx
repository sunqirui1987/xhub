  ...
  <>
    ...
    {editing && (
      <span className="block mt-1 text-xs text-muted-foreground">
        {t("Add or remove tiers to define your own set. Every custom tier needs a definition the LLM classifier routes on, and an edited set requires the LLM classification method")}
      </span>
    )}
    {editing && keywordRulesError && (
      <span className="block mt-1 text-xs text-destructive">
        {t("{keywordRulesError}. Edit the rules under Advanced: Keyword/Semantic Matching, or bring the tier back", { keywordRulesError })}</span>
    )}
  </>
);
...
const FallbackTierField: React.FC<{
  ...
  <div className="mt-4">
    <div className="flex items-center gap-2 mb-2">
      <strong className="text-base font-semibold">{t("Fallback Tier")}</strong>
      <SimpleTooltip content={t("Where requests route when the LLM classifier errors, times out, or returns an unparseable reply. Required for an edited tier set: the heuristic scorer cannot produce your tiers.")}>
    ...
    <TierRowSelect
      ...
      placeholder={t("Pick the tier classifier failures route to")}
    />
  </div>
);
  ...
  <div className="flex items-center gap-2 mb-2">
    ...
    <SimpleTooltip
      content={
        row.definition.trim() ||
        description ||
        t("A tier you defined. The classifier routes requests matching its definition here.")
      }
    >
      <Info className="size-4 text-muted-foreground" />
    </SimpleTooltip>
    ...
    {editing && (
      <Button
        ...
        aria-label={t("Remove the {value0} tier", { value0: (activeTierName(row) || t("tier {value0}", { value0: (index + 1) })) })}
...
const TierRowEditFields: React.FC<{
  ...
  <>
    <Input
      ...
      placeholder={t("Tier name, e.g. SECURITY_REVIEW")}
      aria-label={t("Name for tier {value0}", { value0: (index + 1) })}
    ...
    <Textarea
      ...
      placeholder={
        isBuiltInTierName(row.name)
          ? t("Leave blank to keep the built-in definition")
          : t("What belongs in this tier, e.g. requests asking for a security audit")
      }
      aria-label={t("Definition for tier {value0}", { value0: (index + 1) })}
    ...
    {definitionMissing && (
      <span className="mb-2 block text-xs text-destructive">
        {t("A definition is required: it is the rubric the classifier routes on for this tier")}
      </span>
    )}
  </>
);
...
> = {
  NON_REASONING: {
    label: t("Non-reasoning"),
    description: t("Operational relay work: passing information along with no judgment about it"),
  ...
  <>
    <div className="flex items-center gap-2 mb-2">
      <Switch
        ...
        aria-label={t("Route plan-mode requests to a minimum tier")}
      />
      <strong className="font-semibold">{t("Route plan-mode requests to a minimum tier")}</strong>
    </div>
    <span className="block text-xs mb-3 text-muted-foreground">
      {t("Requests from coding agents in plan mode (Claude Code, GitHub Copilot) route to at least this tier. The classifier still wins when it picks higher, and the override only lasts while plan mode is active. {value0}", { value0: (planModeTierOptions.length === 0 && t("Add models to a tier to enable this.")) })}
    </span>
    {value.plan_mode_min_tier !== undefined && (
      <div style={{ maxWidth: 320 }}>
        <TierRowSelect
          label={t("Plan-mode minimum tier")}
  ...
  return (
    <div className="w-full max-w-none">
      <div className="inline-flex items-center gap-2 mb-4">
        <h4 className="m-0 text-xl font-semibold text-foreground">{t("Complexity Tier Configuration")}</h4>
        <SimpleTooltip content={t("Map each complexity tier to one or more models. Simple queries use cheaper/faster models, complex queries use more capable models.")}>
          <Info className="size-4 text-muted-foreground" />
        </SimpleTooltip>
      </div>
      ...
      <Card>
        <CardContent>
              ...
              <div key={row.id}>
                ...
                <div className="mb-4">
                  ...
                  {showsDisplayName && tierInfo && (
                    <InputGroup className="mb-2">
                      <InputGroupInput
                        ...
                        placeholder={t("Display name (default: {value0})", { value0: (tierInfo.label) })}
                        aria-label={t("Display name for the {value0} tier", { value0: (tierInfo.label) })}
                      />
                      {value.tier_labels?.[row.id as keyof ComplexityTiers] && (
                        <InputGroupAddon align="inline-end">
                          <InputGroupButton
                            size="icon-xs"
                            aria-label={t("Clear display name for the {value0} tier", { value0: (tierInfo.label) })}
                  ...
                  <MultiSelect
                    ...
                    placeholder={t("Select model(s) for {value0} queries", { value0: (label.toLowerCase()) })}
                  ...
                  {tierMissing && <span className="text-xs text-destructive">{t("The {label} tier is required", { label })}</span>}
                </div>
              </div>
            );
          })}
          ...
          <div className="mb-2">
            <div className="flex items-center gap-2 mb-2">
              <strong className="text-base font-semibold">{t("Default Model")}</strong>
              <SimpleTooltip content={t("Leave empty to follow the tiers. A model chosen here is pinned: it stays the default however the tiers change.")}>
                <Info className="size-4 text-muted-foreground" />
              </SimpleTooltip>
            </div>
            <SearchSelect
      ...
      <div className="rounded-lg border border-border bg-muted">
        {[
          {
            key: t("classifier"),
            label: <strong className="text-foreground font-semibold">{t("Advanced: Classification Method")}</strong>,
            children: (
          ...
          {
            key: t("adaptive"),
            label: <strong className="text-foreground font-semibold">{t("Advanced: Adaptive Routing")}</strong>,
            children: (
          ...
          {
            key: t("affinity"),
            label: <strong className="text-foreground font-semibold">{t("Advanced: Affinity")}</strong>,
            children: <AffinityControls value={value} onChange={onChange} />,
          },
          {
            key: t("modality"),
            label: <strong className="text-foreground font-semibold">{t("Advanced: Modality Routing")}</strong>,
            children: <ModalityRoutingControls value={value} onChange={onChange} />,
          },
          {
            key: "plan-mode",
            label: <strong className="text-foreground font-semibold">{t("Advanced: Plan-Mode Override")}</strong>,
            children: (
          ...
          {
            key: "context-window",
            label: <strong className="text-foreground font-semibold">{t("Advanced: Context Window Escalation")}</strong>,
            children: <ContextWindowEscalationConfig value={value} onChange={onChange} />,
          },
          {
            key: "stall-escalation",
            label: <strong className="text-foreground font-semibold">{t("Advanced: Stalled Task Escalation")}</strong>,
            children: (
          ...
          {
            key: t("response"),
            label: <strong className="text-foreground font-semibold">{t("Advanced: Response Format")}</strong>,
          ...
          ...(onEscalationKeywordsChange
            ? [
                {
                  key: t("escalation"),
                  label: <strong className="text-foreground font-semibold">{t("Advanced: Escalation Keywords")}</strong>,
                  children: (
          ...
          ...(onAutoRouterCompressionChange
            ? [
                {
                  key: t("compression"),
                  label: <strong className="text-foreground font-semibold">{t("Advanced: Compression")}</strong>,
                  children: (
            ...
            ? [
                {
                  ...
                  label: <strong className="text-foreground font-semibold">{t("Advanced: Keyword/Semantic Matching")}</strong>,
                  children: (
                    <>
                      {onKeywordTierRulesChange && (
                        <KeywordTierRules
