...
const HowClassificationWorks: React.FC<{ value: ComplexityRouterConfigValue }> = ({ value }) => {
  ...
  return (
    <Card className="bg-muted mt-4">
      <CardContent>
        <strong className="block mb-2 font-semibold">{t("How Classification Works")}</strong>
        ...
        {scorerRuns && ranges && (
          <ul className="mt-2 pl-5 text-[13px] text-muted-foreground">
            <li>
              <strong>{effectiveTierLabel("SIMPLE", value.tier_labels)}</strong>{t(": Score <")} {ranges.simpleMedium}
            </li>
            <li>
              <strong>{effectiveTierLabel("MEDIUM", value.tier_labels)}</strong>{t(": Score")} {ranges.simpleMedium} -{" "}
              {ranges.mediumComplex}
            </li>
            <li>
              <strong>{effectiveTierLabel("COMPLEX", value.tier_labels)}</strong>{t(": Score")} {ranges.mediumComplex} -{" "}
              {ranges.complexReasoning}
            </li>
            <li>
              <strong>{effectiveTierLabel("REASONING", value.tier_labels)}</strong>{t(": Score >")}{" "}
              {ranges.complexReasoning} {t("(or 2+ reasoning markers with a score of at least")}{" "}
      ...
      <div className="flex w-full flex-col items-start gap-2">
        <SimpleTooltip content={scorerLockedReason}>
          <Label className="items-start font-normal leading-normal has-data-disabled:cursor-not-allowed has-data-disabled:opacity-50">
            ...
            <span>
              <strong className="font-semibold">{t("Heuristic")}</strong>{" "}
              <span className="text-muted-foreground">
                {t("(default), rule-based scoring with no API calls and <1ms latency")}
              </span>
            </span>
          </Label>
        </SimpleTooltip>
        <SimpleTooltip content={scorerLockedReason}>
          <Label className="items-start font-normal leading-normal has-data-disabled:cursor-not-allowed has-data-disabled:opacity-50">
            ...
            <span>
              ...
              <span className="text-muted-foreground">
                {t("uses bundled calibrated four-tier probabilities with no API call")}
              </span>
            </span>
          </Label>
        </SimpleTooltip>
        <Label className="items-start font-normal leading-normal">
          ...
          <span>
            <strong className="font-semibold">{t("LLM Classifier")}</strong>{" "}
            <span className="text-muted-foreground">{t("calls a model to decide the tier (e.g. a small/fast model)")}</span>
          </span>
        </Label>
        <SimpleTooltip content={scorerLockedReason}>
          <Label className="items-start font-normal leading-normal has-data-disabled:cursor-not-allowed has-data-disabled:opacity-50">
            ...
            <span>
              <strong className="font-semibold">{t("Heuristic first")}</strong>{" "}
              <span className="text-muted-foreground">
                {t("scores locally, and only pays for the classifier when the score does not confidently land a cheap tier")}
              </span>
            </span>
          </Label>
        </SimpleTooltip>
        <SimpleTooltip content={scorerLockedReason}>
          <Label className="items-start font-normal leading-normal has-data-disabled:cursor-not-allowed has-data-disabled:opacity-50">
            ...
            <span>
              <strong className="font-semibold">{t("Hybrid")}</strong>{" "}
              <span className="text-muted-foreground">
                {t("keeps the local score at any tier, and only pays for the classifier when that score lands near a tier boundary")}
              </span>
            </span>
          </Label>
        </SimpleTooltip>
    ...
    <>
      ...
      {classifierType === "heuristic_first" && (
        <div className="mt-4 space-y-2">
          <strong className="block font-semibold">{t("Decide locally up to")}</strong>
          <Select
          ...
          <p className="text-sm text-muted-foreground">
            {t("A request the scorer places at or below this tier routes there without a classifier call. Anything the scorer places higher, and anything it found no signal for at all, goes to the classifier instead")}
          </p>
        </div>
      )}
      ...
      {classifierType === "hybrid" && (
        <div className="mt-4 space-y-2">
          <strong className="block font-semibold">{t("Boundary margin")}</strong>
          <Input
          ...
          />
          <p className="text-sm text-muted-foreground">
            {t("A score further than this from every tier boundary routes on the scorer's own tier, however expensive that tier is. A score closer than this, and anything the scorer found no signal for at all, goes to the classifier to break the tie")}
          </p>
        </div>
      )}

      <div className="mt-4 space-y-2">
        <strong className="block font-semibold">{t("How often to classify")}</strong>
        <RadioGroup
        ...
        >
          <div className="inline-flex flex-col gap-2">
            <Label className="items-start font-normal leading-normal">
              ...
              <span>
                <span>{t("Every request")}</span>{" "}
                <span className="text-muted-foreground">{t(": score every turn, tool-result continuations included")}</span>
              </span>
            </Label>
            <Label className="items-start font-normal leading-normal">
              <RadioGroupItem value="user_turn" className="mt-0.5" />
              <span>
                <span>{t("Every new user message")}</span>{" "}
                <span className="text-muted-foreground">
                  {t(": score each new human ask, then hold that tier for the tool calls that follow it")}
                </span>
              </span>
            </Label>
            <Label className="items-start font-normal leading-normal">
              ...
              <span>
                <span>{t("Once per session")}</span>{" "}
                <span className="text-muted-foreground">
                  {sessionFrequencyRestriction?.reason ??
                    t(": score the first turn only, then hold that tier and its deployment for the whole session")}
                </span>
              </span>
            </Label>
          </div>
        ...
        <p className="text-sm text-muted-foreground">
          {t("Holding the tier keeps an agent on one model for a whole tool loop and cuts scoring cost. A turn the router cannot match to a held decision, such as one with no session id or an expired one, is scored again")}
        </p>
      </div>
      ...
      {usesLlmClassifier(classifierType) && (
        <div className="mt-4 space-y-3">
          <div>
            <strong className="block mb-1 font-semibold">{t("Classifier Model")}</strong>
            <SearchSelect
              ...
              aria-label={t("Classifier Model")}
            />
            {classifierModelMissing && <span className="text-xs text-destructive">{t("A classifier model is required")}</span>}
          </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <strong className="font-semibold">{t("Classifier Prompt")}</strong>
              <SimpleTooltip content={t("Every rubric uses the same four tiers. They differ in the worked examples that show the classifier where the boundary between tiers sits, and the Business rubric also rewrites the tier definitions for business traffic. Pick the rubric, and write your own opening instructions and calibration examples, inside the prompt editor.")}>
          ...
          <RestrictedSection heading={t("If the classifier fails")} by={restrictedBy(value, "classifierFallback")}>
            <RadioGroup
            ...
            >
              <div className="inline-flex flex-col gap-2">
                <Label className="items-start font-normal leading-normal">
                  ...
                  <span>
                    <span>{t("Score with the heuristic")}</span>{" "}
                    <span className="text-muted-foreground">{t("— right when the classifier grades complexity too")}</span>
                  </span>
                </Label>
                <Label className="items-start font-normal leading-normal has-data-disabled:cursor-not-allowed has-data-disabled:opacity-50">
                  ...
                  >
                    <span>
                      <span>{t("Route to the default model{value0}", { value0: (defaultModel ? ` (${defaultModel})` : "") })}</span>{" "}
                      <span className="text-muted-foreground">
                        {t("— right when your prompt grades something other than complexity")}
                      </span>
                    </span>
                  </SimpleTooltip>
                </Label>
          ...
          <div>
            ...
            />
            <span className="text-xs text-muted-foreground">
              {t("Total characters of prior conversation sent to the classifier. Turns are taken newest first and quoted whole while they fit, so a short conversation is never cut.")}
            </span>
            {contextBudgetQuotesNothing && (
              <span className="block text-xs text-destructive">
                {t("Under {MIN_QUOTED_CONTEXT_TURN_CHARS} characters there is no room to quote a turn that does not already fit, so a long conversation reaches the classifier with no context at all. Set Context Window Size to 0 to turn context off deliberately.", { MIN_QUOTED_CONTEXT_TURN_CHARS })}</span>
            )}
          </div>
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              />
              <strong className="font-semibold">{t("Include Assistant Turns")}</strong>
              <SimpleTooltip content={t("Off by default. Enabling it changes tier decisions, and therefore spend, for an existing router, and sends assistant text to the classifier model, which may be a different provider than the routed model.")}>
                <Info className="size-4 text-muted-foreground" />
              </SimpleTooltip>
            </div>
            <span className="text-xs text-muted-foreground">
      ...
      {heuristicScoringRole(value) !== "never" && (
        <div className="mt-4">
          <div className="flex items-center gap-2 mb-1">
            <strong className="font-semibold">{t("Custom Technical Keywords")}</strong>
            <SimpleTooltip content={t("Domain-specific terms appended to the built-in technical keyword list. Prompts containing these terms score higher on the technical dimension and route to more capable models.")}>
              <Info className="size-4 text-muted-foreground" />
            </SimpleTooltip>
          </div>
