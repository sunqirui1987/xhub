...
const LiteLLMModelNameField: React.FC<LiteLLMModelNameFieldProps> = ({
  ...
  return (
    <>
      <MountedFormField
        name="model"
        label={labelWithHint(t("LiteLLM Model Name(s)"), t("The model name LiteLLM will send to the LLM API"))}
        required
        rules={{
          validate: {
            required: requiredRule(
              t("Please enter {value0}.", { value0: (selectedProvider === Providers.Azure ? t("a deployment name") : t("at least one model")) }),
            ),
          },
        }}
        className="mb-0"
        ...
        {(control) =>
          ...
          selectedProvider === Providers.Ollama ? (
            <Input
              ...
              placeholder={selectedProvider === null ? t("Select a provider first") : getPlaceholder(selectedProvider)}
              onChange={(event) => {
            ...
            <MultiSelect
              ...
              options={[
                ...
                {
                  label: t("All {value0} Models (Wildcard)", { value0: (selectedProvider ?? "provider") }),
                  value: "all-wildcard",
                },
      ...
      {selectedModels.includes("custom") && (
        <MountedFormField
          ...
          required
          rules={{ validate: { required: requiredRule(t("Please enter a custom model name.")) } }}
          className="mt-2"
        >
          {(control) => (
            <Input
