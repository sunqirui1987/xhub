      ...
      <Collapsible className="mt-2 mb-4 overflow-hidden rounded-lg border">
        ...
        <CollapsibleContent className="px-4 pb-3">
          <div className="rounded-lg">
            <MountedFormField name="custom_pricing" label={t("Custom Pricing")} className="mb-4">
              {(control) => (
                <Switch
                  id={control.id}
            ...
            <MountedFormField
              ...
              label={
                <span>
                  ...
                  <SimpleTooltip content={t("Vector stores to use for RAG. Every request to this model will automatically retrieve context from these knowledge bases.")}>
              ...
              className="mt-4"
              help={t("Select vector stores to attach. Requests to this model will automatically use these for RAG. Set up vector stores in Tools > Vector Stores.")}
            >
              {(control) => (
                <VectorStoreSelector
            ...
            >
              {(control) => (
                <MultiSelect
                  ...
                  emptyText={t("Type to add a guardrail")}
                  value={(control.value as string[] | undefined) ?? []}
            ...
            <MountedFormField name="tags" label={t("Tags")} className="mb-4">
              {(control) => (
                <MultiSelect
                  id={control.id}
                  placeholder={t("Select or enter tags")}
                  emptyText={t("Type to add a tag")}
                  value={(control.value as string[] | undefined) ?? []}
                  onValueChange={control.onChange}
            ...
            {ptuCostAttributionEnabled && (
              <>
                <MountedFormField
                  ...
                  label={labelWithHint(
                    t("PTU Count"),
                    t("Provisioned throughput units for this deployment. Set together with Cost per PTU / Hour and a Team to attribute a flat daily cost."),
                  )}
                  rules={{
                ...
                <MountedFormField
                  name={PTU_RATE_FIELD}
                  label={labelWithHint(
                    t("Calculated Cost per PTU / Hour (USD)"),
                    t("Flat cost = PTU count * this rate * active hours, attributed to the deployment's team."),
                  )}
                  rules={{
                    deps: [PTU_COUNT_FIELD],
                    validate: validatorRules(
                ...
                <MountedFormField
                  ...
                  label={labelWithHint(
                    t("PTU Effective From (UTC)"),
                    t("Start of the PTU window, required when PTU Count is set. Flat cost accrues by the hour within the window; a window opening at 23:00 charges one hour that day."),
                  )}
                  rules={{
                    deps: [PTU_END_FIELD],
                    validate: validatorRules(
                ...
                <MountedFormField
                  name={PTU_END_FIELD}
                  label={labelWithHint(
                    t("PTU Effective To (UTC)"),
                    t("Optional end of the PTU window (exclusive). Leave blank for open-ended."),
                  )}
                  rules={{
            ...
            {customPricing && (
              <div className="ml-6 pl-4 border-l-2 border-border">
                <MountedFormField name="pricing_model" label={t("Pricing Model")} className="mb-4">
                  {(control) => (
                    <Select
                ...
                {pricingModel === "per_token" ? (
                  <>
                    ...
                    <MountedFormField
                      ...
                      label={labelWithHint(t("Cache Read Cost (per 1M tokens)"), t("If left blank, defaults to Input Cost."))}
                      rules={usageCostRules}
                      className="mb-4"
                    >
                      {(control) => (
                        <Input
                          ...
                          onChange={control.onChange}
                          onBlur={control.onBlur}
                          placeholder={t("Defaults to Input Cost if blank")}
                        />
                      )}
                    </MountedFormField>
                    <MountedFormField
                      ...
                      label={labelWithHint(
                        ...
                        t("If left blank, defaults to Input Cost (the backend falls back to input_cost_per_token when no cache-write rate is set)."),
                      )}
                      rules={usageCostRules}
                      className="mb-4"
                    >
            ...
            <MountedFormField
              ...
              label={labelWithHint(
                t("LiteLLM Params"),
                t("Optional litellm params used for making a litellm.completion() call."),
              )}
              className="mb-4 mt-4"
            ...
            <MountedFormField
              ...
              label={labelWithHint(
                t("Model Info"),
                t("Optional model info params. Returned when calling `/model/info` endpoint."),
              )}
              className="mb-0"
              rules={{ validate: validatorRules({ validator: formItemValidateJSON }) }}
            >
