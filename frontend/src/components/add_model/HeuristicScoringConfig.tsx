          ...
          {isPending ? (
            ...
            <>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {spec.withSlider && defaults !== undefined && (
                          <span className="text-xs text-muted-foreground" data-testid="dimension-weight-total">
                            {t("total {value0}", { value0: (total.toFixed(2)) })}
                          </span>
                        )}
                      </div>
                      {resettable && (
                        ...
                        >
                          {spec.withSlider ? t("Restore default weights") : t("Reset to defaults")}
                        </Button>
                      )}
                    </div>
                        ...
                        <div key={key} className="flex items-center gap-3">
                          ...
                          {spec.withSlider && (
                            <Slider
                              ...
                              className="flex-1"
                              aria-label={t("{label} weight", { label })}
                            />
                          )}
                          <Input
                            id={id}
              ...
              <section className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">{t("Reasoning override floor")}</span>
                ...
                <p className="text-xs text-muted-foreground">
                  {t("Two or more reasoning markers promote a request to the reasoning tier, but only once its weighted score reaches this floor. {value0} Set it to 0 to promote on the markers alone.", { value0: (trackedFloor === undefined
                    ? t("Left untouched, it tracks the Simple to Medium boundary.")
                    : t("Left untouched, it tracks the Simple to Medium boundary, currently {value0}.", { value0: (trackedFloor.toFixed(2)) })) })}</p>
