...
const publicNameTooltipContent = (
  <div className="flex flex-col gap-2 text-left font-normal">
    <div>{t("The name you specify in your API calls to LiteLLM Proxy")}</div>
    <div>
      <strong>{t("Example:")}</strong> {t("If you name your public model")} <code className={tooltipCodeClassName}>example-name</code>
      {t(", and choose")} <code className={tooltipCodeClassName}>openai/qwen-plus-latest</code> {t("as the LiteLLM model")}
    </div>
    <div>
    ...
    <div>
      <strong>{t("Result:")}</strong> {t("LiteLLM sends")} <code className={tooltipCodeClassName}>qwen-plus-latest</code> {t("to the provider")}
    </div>
  </div>
);
  ...
  },
  {
    ...
    header: () => (
      <span className="flex items-center">
        ...
        <SimpleTooltip content={<div>{t("The model name LiteLLM will send to the LLM API")}</div>} width="360px" />
      </span>
    ),
  },
];
  ...
  return (
    <MountedFormField
      ...
      label={
        <span className="flex items-center">
          {t("Model Mappings")}
          <SimpleTooltip content={t("Map public model names to LiteLLM model names for load balancing")} />
        </span>
      }
      required
