...
export const prepareModelAddRequest = async (formValues: Record<string, any>, accessToken: string, form: any) => {
  try {
        ...
        // Check if key is "base_model"
          ...
          if (value && value != undefined) {
            try {
              ...
              toast.fromError(t("Failed to parse LiteLLM Extra Params: ") + error);
              throw new Error(t("Failed to parse litellm_extra_params: ") + error);
            }
    ...
    toast.fromError(t("Failed to create model: ") + error);
  }
};
...
export const handleAddModelSubmit = async (values: any, accessToken: string, form: any, callback?: () => void) => {
  try {
    ...
    toast.fromError(t("Failed to add model: ") + error);
  }
};
