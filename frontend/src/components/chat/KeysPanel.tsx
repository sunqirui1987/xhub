...
const KeysPanel: React.FC<Props> = ({ accessToken, userId, premiumUser }) => {
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      toast.error(t("Failed to rotate key"));
    } finally {
    ...
    <div className="w-full">
      <div className="mb-4">
        ...
        <p className="text-sm text-muted-foreground m-0">
          {t("View your virtual keys and spend {value0}", { value0: (premiumUser &&
            t(". Rotate keys to generate new credentials while optionally keeping the old key valid during a grace period")) })}
        </p>
      </div>
                    ...
                    <TableCell>
                      {!record.expires ? (
                        ...
                        <Badge variant={expired ? "destructive" : "outline"}>
                          {expired ? t("Expired") : formatExpiresUtc(record.expires)}
                        </Badge>
                      )}
                    </TableCell>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1.5">
                  ...
                  <p className={`text-xs ${keyIsExpired ? "text-destructive" : "text-muted-foreground"}`}>
                    Current: {rotateTarget?.expires ? formatExpiresUtc(rotateTarget.expires) : t("Never")}
