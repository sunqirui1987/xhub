...
const MCPAppsPanel: React.FC<Props> = ({ accessToken, selectedServers, onChange, connectMode }) => {
  ...
  const handleToggle = async (server: MCPServer, checked: boolean) => {
    ...
    try {
      ...
      if (result?.error) {
        toast.warning(t("Could not load tools for {serverName}", { serverName }));
        return;
      }
      if (connectableNow(server.server_id) === undefined) return;
  ...
  if (detailServer) {
    ...
    const renderDetailAction = () => {
      ...
      if (getMcpOAuthMode(detailServer) === "m2m") {
        return <span className="text-[13px] text-muted-foreground">{t("Authorized")}</span>;
      }
    ...
    return (
      <div className="w-full">
        ...
        <h3 className="m-0 mb-3 text-[15px] font-semibold text-foreground">{t("connect.information")}</h3>
        ...
        <div className="flex items-center gap-2 mb-3">
          <h3 className="m-0 text-[15px] font-semibold text-foreground">{t("connect.availableTools")}</h3>
          {!loadingTools && (
      ...
      <div className="flex items-center justify-between mb-5 gap-4 flex-wrap">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <h2 className="m-0 text-lg font-semibold text-foreground">{t("connect.mcpServers")}</h2>
            {!connectMode && (
            ...
            <div className="flex items-center gap-3">
              ...
              {loadingCounts ? (
                ...
                <span className="flex items-center gap-1 text-xs text-muted-foreground">
                  ...
                  {totalTools} {t("tool")}{totalTools !== 1 ? "s" : ""} {t("available")}
                </span>
              ) : null}
            </div>
          )}
