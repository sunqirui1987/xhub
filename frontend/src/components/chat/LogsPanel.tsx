          ...
          {rows.map((row) => (
            <TableRow key={row.request_id} className="cursor-pointer" onClick={() => onRowClick(row)}>
              <TableCell className="whitespace-nowrap text-xs text-muted-foreground">
                {moment(row.startTime).format(t("MMM D, HH:mm:ss"))}
              </TableCell>
      ...
      <>
        ...
        <div className="mt-3 flex items-center justify-between">
          <p className="m-0 text-xs text-muted-foreground">
            ...
            {totalPages > 1 ? t("· Page {page} of {totalPages}", { page, totalPages }) : ""}
          </p>
          {totalPages > 1 && (
  ...
  return (
    <div className="w-full">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h2 className="mb-0.5 text-base font-semibold tracking-tight text-foreground">{t("pages.chat.yourLogs")}</h2>
          <p className="m-0 text-sm text-muted-foreground">{t("pages.chat.logsSubtitle")}</p>
        </div>
        <div className="flex gap-1">
