...
const inheritedBadgeFor = (source: McpGrantSource): InheritedBadge | null => {
  switch (source.kind) {
    case "direct":
      return null;
    case "accessGroup":
      return { label: t("Via access group: {value0}", { value0: (source.name) }), className: "text-green-700 bg-green-50 border-green-200" };
    case "toolset":
      return { label: t("Via toolset: {value0}", { value0: (source.name) }), className: "text-purple-700 bg-purple-50 border-purple-200" };
    case "toolPermission":
      return { label: t("Via tool permissions"), className: "text-amber-700 bg-amber-50 border-amber-200" };
  }
};
    ...
    <div className="space-y-4">
      {serversFailed && (
        <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-sm text-yellow-800 font-medium">{t("Unable to load MCP servers")}</p>
          <p className="text-sm text-yellow-700 mt-1">
            {t("This list is incomplete; servers granted directly or through an access group may be missing. Reload before changing tool permissions")}
          </p>
        </div>
      )}
      ...
      {serversLoaded &&
        ...
        emptyMcpAccessGroups(allServers, populatedAccessGroups, selectedAccessGroups).map((group) => (
          <div key={group} className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-sm text-yellow-800 font-medium">{t("Access group \"{group}\" has 0 servers", { group })}</p>
            <p className="text-sm text-yellow-700 mt-1">
              {t("No MCP server lists this group, so it grants nothing. A server defined in config.yaml joins a group through its")} <code>access_groups</code> {t("key;")} <code>mcp_access_groups</code> {t("is ignored there")}
            </p>
          </div>
        ))}
      ...
      {toolsetsFailed && selectedToolsets.length > 0 && (
        <div className="p-4 bg-yellow-50 border border-yellow-200 rounded-lg">
          <p className="text-sm text-yellow-800 font-medium">{t("Unable to load toolsets")}</p>
          <p className="text-sm text-yellow-700 mt-1">
            {t("Servers reached through the selected toolsets are not listed below")}
          </p>
        </div>
      )}
      ...
      {serversLoading && (
        <div className="flex items-center justify-center py-6">
          <UiLoadingSpinner />
          <p className="ml-3 text-sm text-muted-foreground">{t("Loading MCP servers...")}</p>
        </div>
      )}
              ...
              <div>
                ...
                {entry.ambiguousKeys.length > 0 && (
                  <p className="text-sm text-amber-700 mt-1">
                    {t("Also granted by {value0}, which names another server too. Those tools stay allowed here until the servers no longer share that name", { value0: (entry.ambiguousKeys.map((key) => `"${key}"`).join(", ")) })}
                  </p>
                )}
                {toolsetTools.length > 0 && (
                  <p className="text-sm text-purple-700 mt-1">
                    {toolsetTools.length === 1
                      ? t("{value0} is granted by a selected toolset, so it stays allowed here; edit the toolset to revoke it", { value0: (toolsetTools[0]) })
                      : t("{value0} are granted by a selected toolset, so they stay allowed here; edit the toolset to revoke them", { value0: (toolsetTools.join(", ")) })}
                  </p>
                )}
              </div>
            ...
            <div className="p-4">
              ...
              {isLoading && (
                <div className="flex items-center justify-center py-8">
                  <UiLoadingSpinner />
                  <p className="ml-3 text-sm text-muted-foreground">{t("Loading tools...")}</p>
                </div>
              )}
              ...
              {error && !isLoading && (
                <div className="p-4 bg-destructive/10 border border-destructive/20 rounded-lg text-center">
                  <p className="text-sm text-destructive font-medium">{t("Unable to load tools")}</p>
                      ...
                      <div key={tool.name} className="flex items-start gap-2">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-sm text-muted-foreground">- {tool.description || t("No description")}</p>
                          </div>
                        </div>
                      </div>
                    );
              ...
              {!isLoading && !error && tools.length === 0 && (
                <div className="text-center py-6">
                  <p className="text-sm text-muted-foreground">{t("No tools available")}</p>
                </div>
              )}
            </div>
          </div>
