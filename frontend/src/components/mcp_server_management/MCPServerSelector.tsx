...
const MCPServerSelector: React.FC<MCPServerSelectorProps> = ({
  ...
  const selectOptions: MultiSelectOption[] = [
    ...(allowAllProxyMcpServers || hasAllProxyMcpServersSelected
      ? [{ label: t("All Proxy MCP Servers"), value: ALL_PROXY_MCP_SERVERS_SENTINEL }]
      : []),
    ...(allowNoMcpServers
      ? [{ label: t("No MCP Servers"), value: NO_MCP_SERVERS_SENTINEL, description: t("Block all") }]
      : []),
    ...options.map((opt) => ({
      ...opt,
  ...
  return (
    <div>
      <MultiSelect
        ...
        emptyText={t("No MCP servers found")}
        loading={loading}
        disabled={disabled}
