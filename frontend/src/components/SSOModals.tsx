...
const SSOModals: React.FC<SSOModalsProps> = ({
  ...
  const handleFormSubmit = async (formValues: SSOSettingsFormValues) => {
    ...
    try {
      ...
      toast.fromError(t("Failed to save SSO settings: ") + parseErrorMessage(error));
    }
  };
  ...
  const handleClearSSO = async () => {
    ...
    try {
      ...
      toast.fromError(t("Failed to clear SSO settings"));
    }
  };
    ...
    <>
      <Dialog open={isAddSSOModalVisible} onOpenChange={(open) => !open && handleAddSSOCancel()}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
          <DialogHeader>
            <DialogTitle>{ssoConfigured ? t("Edit SSO Settings") : t("Add SSO")}</DialogTitle>
          </DialogHeader>
          <FormProvider {...form}>
            <form
              onSubmit={(event) => {
            ...
            >
              <FieldGroup>
                ...
                <ProxyAdminEmailField />
                <ProxyBaseUrlField />
                {showRoleMappingToggle && <MappingToggleField name="use_role_mappings" label={t("Use Role Mappings")} />}
                {useRoleMappings && (
                  <>
                    <GroupClaimField />
                    <RoleMappingTeamFields />
      ...
      <Dialog open={isClearConfirmModalVisible} onOpenChange={(open) => !open && setIsClearConfirmModalVisible(false)}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t("Confirm Clear SSO Settings")}</DialogTitle>
          </DialogHeader>
      ...
      <Dialog open={isInstructionsModalVisible} onOpenChange={(open) => !open && handleInstructionsCancel()}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
          <DialogHeader>
            <DialogTitle>{t("SSO Setup Instructions")}</DialogTitle>
          </DialogHeader>
          <p>{t("Follow these steps to complete the SSO setup:")}</p>
          <p className="text-sm mt-2">{t("1. DO NOT Exit this TAB")}</p>
          <p className="text-sm mt-2">{t("2. Open a new tab, visit your proxy base url")}</p>
          <p className="text-sm mt-2">{t("3. Confirm your SSO is configured correctly and you can login on the new Tab")}</p>
          <p className="text-sm mt-2">{t("4. If Step 3 is successful, you can close this tab")}</p>
