        ...
        <Card>
          <CardContent>
            ...
            <p className="text-sm text-muted-foreground">
              {t("{value0} avg per successful request", { value0: (Math.round(metrics.total_tokens / metrics.total_successful_requests)) })}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent>
            ...
            <p className="text-sm text-muted-foreground">
              {t("${value0} per successful request", { value0: (formatNumberWithCommas(metrics.total_spend / metrics.total_successful_requests, 3)) })}</p>
          </CardContent>
        </Card>
      </div>
                  ...
                  <div key={keyData.api_key} className="flex justify-between items-center p-3 bg-muted rounded-lg">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-muted-foreground">
                        {t("{value0} requests | {value1} tokens", { value0: (keyData.requests.toLocaleString()), value1: (keyData.tokens.toLocaleString()) })}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {!hidePromptCachingMetrics && (
          <Card>
            <CardContent>
              ...
              <div className="mb-2">
                <p className="text-sm">
                  {t("Cache Read: {value0} tokens", { value0: (metrics.total_cache_read_input_tokens?.toLocaleString() || 0) })}</p>
                <p className="text-sm">
                  {t("Cache Creation: {value0} tokens", { value0: (metrics.total_cache_creation_input_tokens?.toLocaleString() || 0) })}</p>
              </div>
              <AreaChart
                className="mt-4"
                data={metrics.daily_data}
          ...
          <ModelCollapsible
            ...
            header={
              <div className="flex justify-between items-center w-full">
                <h3 className="text-lg font-medium text-foreground">
                  {modelMetrics[modelName].label || t("Unknown Item")}
                </h3>
