...
describe("LogDetailContent", () => {
  ...
  it("should link the Response Cache tooltip to the response caching docs", async () => {
    ...
    expect(await screen.findByText(/served from XHub's response cache/i)).toBeInTheDocument();
  ...
  it("should display XHub Overhead when litellm_overhead_time_ms is in metadata", () => {
    render(
      <LogDetailContent
        logEntry={createLogEntry({
          metadata: {
    ...
    expect(screen.getByText("XHub Overhead")).toBeInTheDocument();
    expect(screen.getByText("42.50 ms")).toBeInTheDocument();
  });

  it("should not display XHub Overhead when litellm_overhead_time_ms is absent from metadata", () => {
    render(<LogDetailContent logEntry={createLogEntry({ metadata: { status: "success" } })} />);

    expect(screen.queryByText("XHub Overhead")).not.toBeInTheDocument();
  });
