    ...
    cell: ({ row }) => {
      ...
      if (batchCounts && batchCounts.failed > 0) {
        ...
        return (
          <StatusBadge
            tone="warning"
            label={t("{value0}/{total} succeeded", { value0: (batchCounts.successful), total })}
            tooltip={t("{value0} of {total} batch requests failed", { value0: (batchCounts.failed), total })}
          />
        );
      }
      ...
      if (batchId) {
        return (
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-muted-foreground">{t("batch cost")}</span>
          </div>
        );
      }
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-col items-end">
          ...
          {isMultiCallSession && <span className="text-[10px] text-muted-foreground">{t("session total")}</span>}
          {mcpCount > 0 && mcpSpend > 0 && (
            <span className="text-[10px] text-warning">
              {t("incl. {value0} from {mcpCount} MCP", { value0: (getSpendString(mcpSpend)), mcpCount })}</span>
          )}
        </div>
      );
    },
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex flex-col items-end">
          ...
          {showSessionTotal && <span className="text-[10px] text-muted-foreground">{t("session total")}</span>}
        </div>
      );
    },
  },
