          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <TooltipProvider>
              <Tooltip>
                ...
                >
                  {t("{value0}/ 100 {value1}", { value0: (entry.overall_score?.toFixed(0)), value1: (entry.threshold != null && ` (threshold: ${entry.threshold})`) })}
                </TooltipTrigger>
                <TooltipContent>
                  {t("Weighted average of all criterion scores. Each criterion has a weight (%) set when the eval was created — higher-weight criteria count more toward the final score.")}
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
      ...
      <CardContent>
        {entry.eval_error && (
          <span className="text-warning" style={{ display: "block", marginBottom: 8, fontSize: 12 }}>
            {t("Judge error: {value0}", { value0: (entry.eval_error) })}
          </span>
        )}
        ...
        {verdicts.length > 0 ? (
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead style={{ width: 160 }}>{t("Criterion")}</TableHead>
                <TableHead style={{ width: 65 }}>{t("Weight")}</TableHead>
                <TableHead style={{ width: 65 }}>{t("Score")}</TableHead>
                <TableHead style={{ width: 75 }}>
                  <TooltipProvider>
                    <Tooltip>
                      ...
                      <TooltipContent>
                        {t("Score × Weight — how much each criterion contributes to the final score")}
                      </TooltipContent>
                    </Tooltip>
                  </TooltipProvider>
                </TableHead>
