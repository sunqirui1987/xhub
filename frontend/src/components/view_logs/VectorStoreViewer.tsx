    ...
    <div className="bg-card rounded-lg shadow-sm w-full max-w-full overflow-hidden mb-6">
      <Collapsible open={open} onOpenChange={setOpen}>
        <CollapsibleTrigger className="flex w-full items-center gap-3 px-4 py-3 text-left">
          ...
          <h3 className="text-lg font-medium text-foreground">{t("Vector Store Requests")}</h3>
        </CollapsibleTrigger>
        <CollapsibleContent>
          <div className="p-4">
                      ...
                      <div className="flex">
                        ...
                        <span className="flex items-center">
                          {(() => {
                            ...
                            return (
                              <>
                                {logo && <img src={logo} alt={t("{displayName} logo", { displayName })} className="h-5 w-5 mr-2" />}
                                {displayName}
                              </>
                            );
                          })()}
                      ...
                      <div key={resultIndex} className="border rounded-lg overflow-hidden">
                        ...
                        >
                          ...
                          <div className="flex items-center">
                            <span className="font-medium mr-2">{t("Result {value0}", { value0: (resultIndex + 1) })}</span>
                            <span className="text-muted-foreground text-sm">
                              {t("Score:")} <span className="font-mono">{result.score.toFixed(4)}</span>
                            </span>
                          </div>
                        </div>
