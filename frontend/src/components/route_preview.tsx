        ...
        {includeSubpath ? (
          <div>
            ...
            <p className="mt-3 text-sm text-muted-foreground">
              {t("Any path after {pathValue} will be appended to the target URL", { pathValue })}</p>
          </div>
        ) : (
          <div className="flex items-start gap-2 rounded-md border border-primary/20 bg-primary/5 p-3 text-sm">
            ...
            <p>
              ...
              <code className="rounded-sm bg-primary/10 px-1 py-0.5 font-mono text-xs">/api/v1/models</code> {t("to be forwarded automatically.")}
            </p>
          </div>
        )}
      </CardContent>
