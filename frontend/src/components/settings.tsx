  ...
  return (
    <div className="space-y-4 mt-6 p-4 bg-muted rounded-lg border">
      {params.map((param) => {
        ...
        const validationRules = isRequired ? { required: t("Please enter the {value0}", { value0: (fieldLabel.toLowerCase()) }) } : undefined;
                ...
                render={({ field }) => (
                  ...
                  >
                    <SelectTrigger id={fieldId} className="w-full" onBlur={field.onBlur}>
                      <SelectValue placeholder={t("Select {value0}", { value0: (fieldLabel.toLowerCase()) })} />
                    </SelectTrigger>
                    <SelectContent>
            ...
            {!isSelect &&
              (paramType === "password" ? (
                <Input
                  id={fieldId}
                  type="password"
                  placeholder={t("Enter your {value0}", { value0: (fieldLabel.toLowerCase()) })}
                  {...registration}
                />
              ) : paramType === "number" ? (
                <Input
                  id={fieldId}
                  type="number"
                  placeholder={t("Enter {value0}", { value0: (fieldLabel.toLowerCase()) })}
                  min={0}
                  max={1}
                  step={0.1}
                  {...registration}
                ...
                <Input id={fieldId} placeholder={t("Enter your {value0}", { value0: (fieldLabel.toLowerCase()) })} {...registration} />
              ))}
...
const Settings: React.FC<SettingsPageProps> = ({ accessToken, userRole, userID, premiumUser }) => {
  ...
  useEffect(() => {
    ...
    getCallbackConfigsCall(accessToken)
      ...
      .catch((error) => {
        toast.fromError(t("Failed to load callback configs: ") + parseErrorMessage(error));
      });
  }, [accessToken]);
  ...
  const handleCallbackSubmit = async (formValues: Record<string, any>, callbackName: string, isEdit: boolean) => {
    ...
    try {
      ...
      toast.success(isEdit ? t("Callback updated successfully") : t("Callback {callbackName} added successfully", { callbackName }));
  ...
  const handleSaveAlerts = async () => {
    ...
    toast.success(t("Alerts updated successfully"));
  };
  ...
  const confirmDeleteCallback = async () => {
    ...
    try {
      ...
      toast.success(t("Callback {value0} deleted successfully", { value0: (callbackToDelete.name) }));
      ...
      <div className="grid grid-cols-1 gap-2 p-8 w-full mt-2">
        <Tabs defaultValue="logging-callbacks">
          <TabsList variant="line">
            ...
            <TabsTrigger value="cloudzero-cost-tracking">{t("pages.loggingAndAlerts.cloudzero")}</TabsTrigger>
            <TabsTrigger value="alerting-types">{t("pages.loggingAndAlerts.alertingTypes")}</TabsTrigger>
            <TabsTrigger value="alerting-settings">{t("pages.loggingAndAlerts.alertingSettings")}</TabsTrigger>
            <TabsTrigger value="email-alerts">{t("pages.loggingAndAlerts.emailAlerts")}</TabsTrigger>
            <TabsTrigger value="ms-teams-alerts">{t("pages.loggingAndAlerts.msTeamsAlerts")}</TabsTrigger>
          </TabsList>
            ...
            <LoggingCallbacksTable
              ...
              onTest={async (cb) => {
                try {
                  ...
                  toast.success(t("Health check triggered"));
                } catch (error) {
          ...
          <TabsContent value="alerting-types" keepMounted>
            <Card className="p-6">
              <p className="my-2">
                {t("Alerts are sent to any Slack-compatible incoming webhook URL (Slack, Rocket.Chat, Mattermost, etc.). Get Slack webhook urls from")}{" "}
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead></TableHead>
                    <TableHead></TableHead>
                    <TableHead>{t("Webhook URL (Slack-compatible)")}</TableHead>
                  </TableRow>
                </TableHeader>
