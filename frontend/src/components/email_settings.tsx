...
const FIELD_HELP: Record<string, React.ReactNode> = {
  ...
  SMTP_USERNAME: <>{t("Enter the SMTP username")}{REQUIRED_MARKER}</>,
  SMTP_PASSWORD: REQUIRED_MARKER,
  SMTP_SENDER_EMAIL: <>{t("Enter the sender email address")}{REQUIRED_MARKER}</>,
  TEST_EMAIL_ADDRESS: <>{t("Email address to send the test email alert to")}{REQUIRED_MARKER}</>,
  EMAIL_LOGO_URL: <>{t("(Optional) Customize the Logo that appears in the email, pass a url to your logo")}</>,
  EMAIL_SUPPORT_CONTACT: <>{t("(Optional) Customize the support email address that appears in the email. Default is support@berri.ai")}</>,
};
...
const EmailSettings: React.FC<EmailSettingsProps> = ({ accessToken, premiumUser, alerts }) => {
  ...
  const handleSaveEmailSettings = async () => {
    ...
    try {
      ...
      toast.success(t("Email settings updated successfully"));
    } catch (error) {
                        ...
                        {isSensitive && (
                          <InputGroupAddon align="inline-end">
                            <InputGroupButton
                              ...
                              aria-label={isVisible ? t("Hide credential") : t("Show credential")}
                            >
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  toast.success(t("Email test triggered. Check your configured email inbox/logs."));
                } catch (error) {
                  toast.fromError(error);
                }
              }}
