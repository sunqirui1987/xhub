...
const RoutingGroups: React.FC = () => {
  ...
  const handleSubmit = async (incoming: RoutingGroup) => {
    ...
    try {
      ...
      toast.success(
        drawerMode === "create"
          ? t("Created routing group \"{value0}\"", { value0: (incoming.group_name) })
          : t("Updated routing group \"{value0}\"", { value0: (incoming.group_name) }),
      );
      setDrawerOpen(false);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : t("Failed to save routing group"));
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      await saveMutation.mutateAsync(next);
      toast.success(t("Deleted routing group \"{value0}\"", { value0: (deletingGroup.group_name) }));
      setDeletingGroup(null);
    } catch (err) {
      toast.error(err instanceof Error ? err.message : t("Failed to delete routing group"));
    }
  };
            ...
            <InputGroup className="max-w-sm">
              ...
              {searchQuery && (
                <InputGroupAddon align="inline-end">
                  <InputGroupButton size="icon-xs" aria-label={t("Clear search")} onClick={() => setSearchQuery("")}>
                    <X />
                  </InputGroupButton>
                </InputGroupAddon>
              )}
