...
const scimTokenSchema = z.object({
  key_alias: z.string().min(1, t("Please enter a name for your token")),
});
...
const SCIMConfig: React.FC<SCIMConfigProps> = ({ accessToken, userID, proxySettings }) => {
  ...
  const handleCreateSCIMToken = async (values: SCIMTokenFormValues) => {
    if (!accessToken || !userID) {
      toast.fromError(t("You need to be logged in to create a SCIM token"));
      return;
    }
    ...
    try {
      ...
      toast.success(t("SCIM token created successfully"));
    } catch (error: any) {
      console.error("Error creating SCIM token:", error);
      toast.fromError(t("Failed to create SCIM token: ") + parseErrorMessage(error));
    } finally {
      ...
      <Card>
        <CardContent>
          <div className="flex items-center mb-4">
            <CardTitle>{t("SCIM Configuration")}</CardTitle>
          </div>
          ...
          <div className="space-y-8">
            ...
            <div>
              ...
              <div className="flex items-center">
                ...
                <CopyToClipboard text={scimBaseUrl} onCopy={() => toast.success(t("URL copied to clipboard"))}>
            ...
            <div>
              ...
              <Alert variant="info" className="mb-4">
                <Info />
                <AlertTitle>{t("Using SCIM")}</AlertTitle>
                <AlertDescription>
              ...
              {!tokenData ? (
                <div className="bg-muted p-4 rounded-lg">
                  <form onSubmit={form.handleSubmit(handleCreateSCIMToken)}>
                    <FieldGroup>
                      <FormField control={form.control} name="key_alias" label={t("Token Name")}>
                        {({ ref, ...field }) => <Input {...field} ref={ref} placeholder={t("SCIM Access Token")} />}
                      </FormField>
                      <div>
                        <Button
                          type="submit"
                ...
                <Card className="block p-6 border border-warning/30 bg-warning/10">
                  <div className="flex items-center mb-2 text-warning">
                    ...
                    <h4 className="text-lg font-medium text-warning">{t("Your SCIM Token")}</h4>
                  </div>
                  ...
                  <div className="flex items-center">
                    ...
                    <CopyToClipboard text={tokenData.key} onCopy={() => toast.success(t("Token copied to clipboard"))}>
