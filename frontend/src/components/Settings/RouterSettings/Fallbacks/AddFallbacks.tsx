...
export default function AddFallbacks({ accessToken, value = [], onChange }: AddFallbacksProps) {
  ...
  const handleSaveAll = async () => {
    ...
    if (invalidGroups.length > 0) {
      toast.error(t("Please complete configuration for all groups. {value0} group(s) incomplete.", { value0: (invalidGroups.length) }));
      return;
    }
    ...
    if (onChange) {
      ...
      try {
        ...
        toast.success(t("{value0} fallback configuration(s) added successfully!", { value0: (groups.length) }));
        handleCancel();
      } catch (error) {
      ...
      toast.fromError(t("onChange callback not provided"));
    }
  };
        ...
        {groups.length > 0 && (
          <div className="flex items-center justify-end space-x-3 pt-6 mt-6 border-t border-border">
            ...
            <Button variant="outline" onClick={handleSaveAll} disabled={groups.length === 0 || isSaving}>
              ...
              {isSaving ? t("Saving Configuration...") : t("Save All Configurations")}
            </Button>
          </div>
        )}
      </AddFallbacksModal>
