      ...
      <div className="space-y-1">
        <div className="flex items-center gap-2">
          ...
          <Badge variant={isPageVisibilitySet ? "secondary" : "outline"}>
            {isPageVisibilitySet
              ? t("{value0} page{value1} selected", { value0: (selectedPages.length), value1: (selectedPages.length !== 1 ? "s" : "") })
              : t("Not set (all pages visible)")}
          </Badge>
        </div>
        {enabledPagesPropertyDescription && (
        ...
        <p className="text-xs italic text-muted-foreground">
          {t("By default, all pages are visible to internal users. Select specific pages to restrict visibility.")}
        </p>
        <p className="text-xs text-primary">
          {t("Note: Only pages accessible to internal user roles are shown here. Admin-only pages are excluded as they cannot be made visible to internal users regardless of this setting.")}
        </p>
      </div>
