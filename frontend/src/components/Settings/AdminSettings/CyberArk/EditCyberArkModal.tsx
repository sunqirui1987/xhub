...
const FIELD_GROUPS: CyberArkFieldGroup[] = [
  ...
  },
  {
    ...
    subtitle: t("Use a Conjur API key to authenticate. Only one auth method is required."),
    fields: ["cyberark_api_key"],
  },
  {
    title: t("Certificate Authentication"),
    subtitle: t("Use a client TLS certificate and key to authenticate. Only one auth method is required."),
    fields: ["client_cert", "client_key"],
  },
  {
    title: t("Advanced"),
    subtitle: t("Optional TLS and token caching settings."),
      ...
      fields.map((name) => [
        ...
        name === "cyberark_api_base"
          ? z.string().refine((value) => value.length === 0 || /^https?:\/\/.+/.test(value), {
              message: t("Must start with http:// or https://"),
            })
          : z.string(),
      ]),
    ),
...
const EditCyberArkModal: React.FC<EditCyberArkModalProps> = ({ isVisible, onCancel, onSuccess }) => {
  ...
  const handleSubmit = (formValues: CyberArkFormValues) => {
    ...
    mutate(config, {
      onSuccess: () => {
        toast.success(t("CyberArk configuration updated successfully"));
        onSuccess();
      },
      onError: (err) => {
        toast.fromError(err);
  ...
  return (
    <Dialog open={isVisible} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[700px]">
        <DialogHeader>
          <DialogTitle>{t("Edit CyberArk Configuration")}</DialogTitle>
        </DialogHeader>
