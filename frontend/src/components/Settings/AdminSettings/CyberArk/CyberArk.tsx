...
export default function CyberArk() {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      toast.success(result.message || t("Connection to CyberArk Conjur successful!"));
    } catch (err) {
      toast.fromError(err);
    } finally {
      setIsTesting(false);
  ...
  const handleDelete = () => {
    deleteConfig(undefined, {
      onSuccess: () => {
        toast.success(t("CyberArk configuration deleted"));
  ...
  const handleClearField = () => {
    ...
    updateConfig(
      ...
      {
        onSuccess: () => {
          toast.success(t("{value0} cleared", { value0: (FIELD_LABELS[clearingField] ?? clearingField) }));
    ...
    return (
      <div className="flex items-center justify-between gap-2">
        ...
        <Button
          ...
          variant="ghost"
          size="icon-sm"
          aria-label={t("Clear {value0}", { value0: (FIELD_LABELS[key] ?? key) })}
  ...
  const renderCard = () => {
    if (isLoading) {
      return (
        <Card role="status" aria-label={t("Loading CyberArk configuration")}>
    ...
    if (isError) {
      return (
        <Card>
          <CardContent>
            <Alert variant="error">
              <AlertTitle>{t("Could not load CyberArk configuration")}</AlertTitle>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          {isConfigured && (
            <Alert variant="info">
              <Info />
              <AlertTitle>{t("Configuration changes are hot-reloaded across all proxy instances")}</AlertTitle>
              <AlertDescription>
  ...
  return (
    <>
      ...
      <DeleteResourceModal
        ...
        title={t("Delete CyberArk Configuration?")}
        message={t("Models using CyberArk secrets will lose access to their API keys until a new configuration is saved.")}
        resourceInformationTitle="CyberArk Configuration"
        resourceInformation={[{ label: t("Conjur Server URL"), value: rawValues.cyberark_api_base }]}
      ...
      <DeleteResourceModal
        ...
        title={t("Clear {value0}?", { value0: (clearingField ? FIELD_LABELS[clearingField] ?? clearingField : "") })}
