...
function UserBannerSettingsForm({ persisted, isLoading, isPending, saveBanner }: UserBannerSettingsFormProps) {
  ...
  const handleSave = () => {
    saveBanner(draft, {
      onSuccess: () => {
        toast.success(t("User banner updated successfully"));
      },
      onError: (error) => {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {t("Publish an announcement to all dashboard users. Markdown is supported; the banner appears below the header on every page until you unpublish it. Users can dismiss it, and it reappears whenever the content changes.")}
        </CardDescription>
      </CardHeader>
      <CardContent>
        {isLoading ? (
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Textarea
                ...
                maxLength={4000}
                rows={3}
                placeholder={t("**Scheduled maintenance** tonight at 10 PM UTC. See [status page](https://example.com).")}
            ...
            <div>
              <Button onClick={handleSave} disabled={isPending || messageMissing}>
                {isPending ? t("Saving...") : t("Save banner")}
              </Button>
            </div>
          </div>
        )}
