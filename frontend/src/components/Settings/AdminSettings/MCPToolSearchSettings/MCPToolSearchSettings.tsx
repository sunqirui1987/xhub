...
export default function MCPToolSearchSettings({ accessToken }: MCPToolSearchSettingsProps) {
  ...
  const handleSave = (formValues: ToolSearchFormValues) => {
    updateSettings(formToPayload(formValues), {
      onSuccess: () => {
        ...
        toast.success(t("Settings updated successfully. Changes will be applied across all pods within 10 seconds."));
      },
  ...
  if (!accessToken) {
    return <div className="p-6 text-center text-muted-foreground">{t("Please log in to configure tool search.")}</div>;
  }
  ...
  if (isError) {
    return (
      <Alert variant="error" className="mb-6">
        <AlertTitle>{t("Could not load MCP tool search settings")}</AlertTitle>
  ...
  return (
    <div className="w-full">
      <Alert variant="info" className="mb-6">
        <Info />
        <AlertTitle>{t("Native MCP Tool Search")}</AlertTitle>
        <AlertDescription>
          {t("Controls the")} <code>mcp_tool_search</code> {t("virtual tool that native MCP clients call to discover tools. With an embedding model set, tools are ranked by the meaning of their name and description, so a query like \"FX\" finds a \"foreign exchange rates\" tool. Without one, keyword matching is used. Callers only ever see tools their key, team and server permissions already allow.")}
        </AlertDescription>
      </Alert>
      ...
      <TooltipProvider>
        <form onSubmit={(event) => event.preventDefault()} noValidate>
            ...
            <CardContent>
              <FieldGroup>
                <FormField
                  ...
                  label={labelWithHint(
                    t("Embedding Model"),
                    t("Embedding model from your model list used to rank tools by meaning. Clear it to fall back to keyword matching."),
                  )}
                >
                  {({ value, onChange, id }) => (
                    <SearchSelect
                      ...
                      allowClear
                      placeholder={loadingModels ? t("Loading models...") : t("Keyword matching (no embedding model)")}
                      emptyText={loadingModels ? t("Loading...") : t("No embedding models available")}
                ...
                <FormField
                  ...
                  label={labelWithHint(
                    t("Top K Results"),
                    t("Most ranked tools a search returns. A smaller top_k in the tool call wins. Core tools do not count."),
                  )}
                >
                ...
                <FormField
                  ...
                  label={labelWithHint(
                    ...
                    t("Lowest cosine similarity a tool needs to appear in semantic results. 0 means no cutoff."),
                  )}
                >
                  {({ value, onChange, id }) => (
                    <div className="w-full">
          ...
          <Card className="mb-4">
            <CardHeader className="border-b">
              <CardTitle>{t("Core Tools")}</CardTitle>
            </CardHeader>
            <CardContent>
              <FieldGroup>
                <FormField
                  ...
                  label={labelWithHint(
                    t("Always Returned First"),
                    t("One tool name per line, e.g. my_server-get_rates. Listed before ranked results whenever the caller is allowed to use them."),
                  )}
                >
                  {({ ref, value, onChange, onBlur, id }) => (
                    <Textarea
