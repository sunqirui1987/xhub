...
export const runSemanticFilterTest = async ({
  ...
  if (!testQuery || !testModel || !accessToken) {
    toast.error(t("Please enter a query and select a model"));
    return;
  }
  ...
  try {
    ...
    if (!parsedResult) {
      toast.warning(t("Semantic filter is not enabled or no tools were filtered"));
      return;
    }

    setTestResult(parsedResult);
    toast.success(t("Semantic filter test completed successfully"));
  } catch (error) {
    console.error("Test failed:", error);
    const message = error instanceof Error && error.message ? error.message : "Failed to test semantic filter";
    setTestError(message);
    toast.error(t("Failed to test semantic filter"));
  } finally {
    setIsTesting(false);
  }
};
