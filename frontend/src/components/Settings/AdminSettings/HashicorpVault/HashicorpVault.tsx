...
export default function HashicorpVault() {
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      toast.success(result.message || t("Connection to Vault successful!"));
    } catch (err) {
      toast.fromError(err);
    } finally {
      setIsTesting(false);
  ...
  const handleDelete = () => {
    deleteConfig(undefined, {
      onSuccess: () => {
        toast.success(t("Hashicorp Vault configuration deleted"));
  ...
  return (
    <>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            {isConfigured && (
              <Alert variant="info">
                <Info />
                <AlertTitle>{t("Secrets must be stored with the field name \"key\"")}</AlertTitle>
                <AlertDescription>
      ...
      <DeleteResourceModal
        ...
        message={t("Models using Vault secrets will lose access to their API keys until a new configuration is saved.")}
