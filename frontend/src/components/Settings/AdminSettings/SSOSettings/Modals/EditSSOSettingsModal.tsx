  ...
  const handleFormSubmit = async (formValues: SSOSettingsFormValues) => {
    try {
      ...
      await mutateAsync(payload, {
        onSuccess: () => {
          toast.success(t("SSO settings updated successfully"));
          onSuccess();
        },
        onError: (error) => {
      ...
      toast.fromError(t("Failed to process SSO settings: ") + parseErrorMessage(error));
    }
  };
  ...
  return (
    <Dialog open={isVisible} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
        <DialogHeader>
          <DialogTitle>{t("Edit SSO Settings")}</DialogTitle>
        </DialogHeader>
