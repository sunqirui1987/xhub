...
export const ssoProviderConfigs: Record<string, SSOProviderConfig> = {
  ...
  okta: {
    ...
    fields: [
      ...
      { label: t("Scopes"), name: "generic_scope", placeholder: t("openid email profile"), required: false },
    ],
  },
  generic: {
    envVarMap: {
  ...
  z.custom<SSOSettingsFormValues>().superRefine((values, ctx) => {
    ...
    providerConfig?.fields.forEach((field) => {
      ...
      ctx.addIssue({
        code: "custom",
        path: [field.name],
        message: t("Please enter the {value0}", { value0: (field.label.toLowerCase()) }),
      });
    });
    ...
    if ((proxyBaseUrl as string).endsWith("/")) {
      ctx.addIssue({
        code: "custom",
        path: ["proxy_base_url"],
        message: t("URL must not end with a trailing slash"),
      });
    }
  });
...
export const SSOProviderSelectField = () => {
  ...
  return (
    <FormField control={control} name="sso_provider" label={t("SSO Provider")}>
      {({ value, onChange, onBlur, id, ...rest }) => (
...
export const ProxyAdminEmailField = () => {
  ...
  return (
    <FormField control={control} name="user_email" label={t("Proxy Admin Email")}>
...
export const ProxyBaseUrlField = () => {
  ...
  return (
    <FormField control={control} name="proxy_base_url" label={t("Proxy Base URL")}>
...
export const GroupClaimField = () => {
  ...
  return (
    <FormField control={control} name="group_claim" label={t("Group Claim")}>
      {({ ref, value, ...rest }) => <Input ref={ref} value={(value as string) ?? ""} {...rest} />}
    </FormField>
  );
};
...
const DEFAULT_ROLE_OPTIONS: ReadonlyArray<{ value: string; label: string }> = [
  { value: "internal_user_viewer", label: t("Internal Viewer") },
  { value: "internal_user", label: t("Internal User") },
  { value: "proxy_admin_viewer", label: t("Admin Viewer") },
  { value: "proxy_admin", label: t("Proxy Admin") },
];
...
export const RoleMappingTeamFields = () => {
  ...
  return (
    <>
      <FormField control={control} name="default_role" label={t("Default Role")}>
        {({ value, onChange, onBlur, id, ...rest }) => (
          <Select value={(value as string) ?? ""} onValueChange={onChange}>
            <SelectTrigger
              id={id}
      ...
      <FormField control={control} name="proxy_admin_teams" label={t("Proxy Admin Teams")}>
        {({ ref, value, ...rest }) => <Input ref={ref} value={(value as string) ?? ""} {...rest} />}
      </FormField>

      <FormField control={control} name="admin_viewer_teams" label={t("Admin Viewer Teams")}>
        {({ ref, value, ...rest }) => <Input ref={ref} value={(value as string) ?? ""} {...rest} />}
      </FormField>

      <FormField control={control} name="internal_user_teams" label={t("Internal User Teams")}>
        {({ ref, value, ...rest }) => <Input ref={ref} value={(value as string) ?? ""} {...rest} />}
      </FormField>

      <FormField control={control} name="internal_viewer_teams" label={t("Internal Viewer Teams")}>
        {({ ref, value, ...rest }) => <Input ref={ref} value={(value as string) ?? ""} {...rest} />}
      </FormField>
    </>
  );
...
export const TeamIdsJwtFieldField = () => {
  ...
  return (
    <FormField control={control} name="team_ids_jwt_field" label={t("Team IDs JWT Field")}>
      {({ ref, value, ...rest }) => <Input ref={ref} value={(value as string) ?? ""} {...rest} />}
    </FormField>
  );
};
      ...
      <FormProvider {...form}>
        ...
        >
          <FieldGroup>
            ...
            <ProxyBaseUrlField />
            {showMappingToggles && <MappingToggleField name="use_role_mappings" label={t("Use Role Mappings")} />}
            ...
            {showMappingToggles && <MappingToggleField name="use_team_mappings" label={t("Use Team Mappings")} />}
