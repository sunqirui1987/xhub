    ...
    },
    {
      ...
      cell: ({ row }) =>
        row.original.groups.length > 0 ? (
          ...
          <span className="text-muted-foreground italic">{t("No groups mapped")}</span>
        ),
    },
  ];
  return (
    <Card>
      <CardContent>
        <div className="flex items-center gap-3">
          ...
          <h3 className="mb-2 text-2xl font-semibold text-foreground">{t("Role Mappings")}</h3>
        </div>
        <div className="space-y-8">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <h5 className="mb-2 text-base font-semibold text-foreground">{t("Group Claim")}</h5>
              <div>
                <code className={inlineCodeClass}>{roleMappings.group_claim}</code>
              </div>
            </div>
            <div>
              <h5 className="mb-2 text-base font-semibold text-foreground">{t("Default Role")}</h5>
              <div>
