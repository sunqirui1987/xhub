...
const cloudZeroSettingsSchema = z.object({
  ...
  connection_id: z.string().min(1, t("Please enter the CloudZero connection ID")),
});
...
const CloudZeroExportModal: React.FC<CloudZeroExportModalProps> = ({ isOpen, onClose, accessToken }) => {
  ...
  const loadExistingSettings = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.fromError(t("Failed to load existing settings: {value0}", { value0: (errorData.error || t("Unknown error")) }));
      }
    } catch (error) {
      console.error("Error loading CloudZero settings:", error);
      toast.fromError(t("Failed to load existing settings"));
    } finally {
  ...
  const handleSaveCloudZeroSettings = async (values: CloudZeroSettings) => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(data.message || t("CloudZero settings saved successfully"));
        setExistingSettings({
        ...
        toast.fromError(data.error || t("Failed to save CloudZero settings"));
        return false;
      }
    } catch (error) {
      console.error("Error saving CloudZero settings:", error);
      toast.fromError(t("Failed to save CloudZero settings"));
      return false;
    } finally {
      setLoading(false);
    }
  ...
  const handleExportCloudZero = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(data.message || t("Export to CloudZero completed successfully"));
        onClose();
      } else {
        toast.fromError(data.error || t("Failed to export to CloudZero"));
      }
    } catch (error) {
      console.error("Error exporting to CloudZero:", error);
      toast.fromError(t("Failed to export to CloudZero"));
    } finally {
  ...
  const handleExportCSV = async () => {
    ...
    try {
      ...
      toast.info(t("CSV export functionality coming soon!"));
      onClose();
    } catch (error) {
      console.error("Error exporting CSV:", error);
      toast.fromError(t("Failed to export CSV"));
    } finally {
      setExportLoading(false);
    }
  };
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleModalClose()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[600px]">
        ...
        <div className="space-y-4">
          ...
          <div>
            <p className="text-sm font-medium mb-2 block">{t("Export Destination")}</p>
            <Select items={exportOptions} value={exportType} onValueChange={(value) => value && setExportType(value)}>
              <SelectTrigger className="w-full" aria-label={t("Export Destination")}>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                ...
                <>
                  {existingSettings && (
                    <Alert className="mb-4">
                      <CircleCheck />
                      <AlertTitle>{t("Existing CloudZero Configuration")}</AlertTitle>
                      <AlertDescription>
                        {t("API Key:")} {existingSettings.api_key_masked}
                        <br />
                        {t("Connection ID:")} {existingSettings.connection_id}
                      </AlertDescription>
                    </Alert>
                  )}
                    ...
                    <form onSubmit={(event) => event.preventDefault()}>
                      <FieldGroup>
                        <FormField control={form.control} name="api_key" label={t("CloudZero API Key")}>
                          {({ ref, ...field }) => (
                            <PasswordInput {...field} ref={ref} placeholder={t("Enter your CloudZero API key")} />
                          )}
                        </FormField>

                        <FormField control={form.control} name="connection_id" label={t("Connection ID")}>
                          {({ ref, ...field }) => (
                            <Input {...field} ref={ref} placeholder={t("Enter CloudZero connection ID")} />
                          )}
                        </FormField>
                      </FieldGroup>
                    </form>
          ...
          {exportType === "csv" && (
            <Alert variant="info">
              <FileDown />
              <AlertTitle>{t("CSV Export")}</AlertTitle>
              <AlertDescription>
                {t("Export your usage data as a CSV file for analysis in spreadsheet applications.")}
              </AlertDescription>
            </Alert>
          )}
          ...
          <div className="flex justify-end space-x-2 pt-4">
            ...
            >
              ...
              {exportType === "cloudzero" ? t("Export to CloudZero") : t("Export CSV")}
            </Button>
          </div>
        </div>
      </DialogContent>
