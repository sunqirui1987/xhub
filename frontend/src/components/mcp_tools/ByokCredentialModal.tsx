...
export const ByokCredentialModal: React.FC<ByokCredentialModalProps> = ({ server, open, onClose, onSuccess }) => {
  ...
  const handleAuthorize = async () => {
    ...
    try {
      ...
      toast.success(t("Connected to {serverDisplayName}", { serverDisplayName }));
        ...
        <div className="relative p-2">
          ...
          {step === 1 ? (
            <div className="text-center">
              ...
              <h2 className="text-2xl font-bold text-foreground mb-2">{t("Connect {serverDisplayName}", { serverDisplayName })}</h2>
              <p className="text-muted-foreground mb-6">
                {t("LiteLLM needs access to {serverDisplayName} to complete your request.", { serverDisplayName })}</p>
              ...
              <div className="bg-muted rounded-xl p-4 text-left mb-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="font-semibold text-foreground mb-1">{t("How it works")}</p>
                    <p className="text-muted-foreground text-sm">
                      {t("LiteLLM acts as a secure bridge. Your requests are routed through our MCP client directly to {serverDisplayName}'s API.", { serverDisplayName })}</p>
                  </div>
                </div>
              </div>
              ...
              >
                {t("Continue to Authentication")} <ArrowRight className="size-4" />
              </button>
              <button
                onClick={handleClose}
            ...
            <div>
              ...
              <h2 className="text-2xl font-bold text-foreground mb-2">{t("Provide API Key")}</h2>
              <p className="text-muted-foreground mb-6">
                {t("Enter your {serverDisplayName} API key to authorize this connection.", { serverDisplayName })}</p>

              <div className="mb-4">
                <label htmlFor={apiKeyInputId} className="block text-sm font-semibold text-foreground mb-2">
                  {t("{serverDisplayName} API Key", { serverDisplayName })}</label>
                <PasswordInput
                  id={apiKeyInputId}
                ...
                {server.byok_api_key_help_url && (
                  ...
                  >
                    {t("Where do I find my API key?")} <Link2 className="size-3.5" />
                  </a>
                )}
              </div>
              ...
              <div className="bg-muted rounded-xl p-4 flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-foreground">{t("Save key for future use")}</span>
                </div>
                <Switch checked={saveKey} onCheckedChange={setSaveKey} aria-label={t("Save key for future use")} />
              </div>
              ...
              <div className="bg-info/10 rounded-xl p-4 flex items-start gap-3 mb-6">
                ...
                <p className="text-sm text-info">
                  {t("Your key is stored securely and transmitted over HTTPS. It is never shared with third parties.")}
                </p>
              </div>
              ...
              >
                <Lock className="size-4" /> {t("Connect & Authorize")}
              </button>
            </div>
          )}
        </div>
