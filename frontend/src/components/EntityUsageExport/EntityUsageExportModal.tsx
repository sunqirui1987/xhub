  ...
  const handleExport = async (format?: ExportFormat) => {
    ...
    try {
      if (formatToUse === "csv") {
        ...
        toast.success(t("{entityLabel} usage data exported successfully as CSV", { entityLabel }));
      } else {
        handleExportJSON(spendData, exportScope, entityLabel, entityType, dateRange, selectedFilters, teamAliasMap);
        toast.success(t("{entityLabel} usage data exported successfully as JSON", { entityLabel }));
      }
      onClose();
    } catch (error) {
      console.error("Error exporting data:", error);
      toast.fromError(t("Failed to export data"));
    } finally {
            ...
            {isLoadingTeams ? (
              ...
              <>
                ...
                <Button onClick={() => handleExport()} disabled={isExporting}>
                  ...
                  {isExporting ? t("Exporting...") : t("Export {value0}", { value0: (exportFormat.toUpperCase()) })}
                </Button>
              </>
            )}
          </div>
