...
const ExportTypeSelector: React.FC<ExportTypeSelectorProps> = ({ value, onChange, entityType }) => {
  const scopes: { value: ExportScope; title: string; description: string }[] = [
    {
      value: "daily",
      title: t("Day-by-day breakdown by {entityType}", { entityType }),
      description: t("Daily metrics for each {entityType}", { entityType }),
    },
    {
      value: "daily_with_keys",
      title: t("Day-by-day breakdown by {entityType} and key", { entityType }),
      description: t("Daily metrics for each {entityType}, split by API key", { entityType }),
    },
    {
      value: "daily_with_models",
      title: t("Day-by-day by {entityType} and model", { entityType }),
  ...
  return (
    <div>
      <label className="text-sm font-medium text-foreground block mb-2">{t("Export type")}</label>
