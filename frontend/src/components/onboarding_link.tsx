  ...
  return (
    <Dialog open={isInvitationLinkModalVisible} onOpenChange={(open) => !open && handleInvitationCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
        <DialogHeader>
          <DialogTitle>{modalType === "invitation" ? t("pages.users.invitationLink") : "重置密码链接"}</DialogTitle>
        </DialogHeader>
        <p className="text-sm text-foreground">
          {modalType === "invitation"
            ? t("Copy and send the generated link to onboard this user to the proxy.")
            : t("Copy and send the generated link to the user to reset their password.")}
        </p>
        <div className="flex justify-between pt-5 pb-2">
          <p className="text-base">{t("User ID")}</p>
          <p className="text-sm">{invitationLinkData?.user_id}</p>
        </div>
        <div className="flex justify-between pt-5 pb-2">
          <p className="text-sm">{modalType === "invitation" ? t("pages.users.invitationLink") : "重置密码链接"}</p>
          <p className="text-sm">{getInvitationUrl()}</p>
        </div>
        <div className="flex justify-end mt-5">
          <CopyToClipboard text={getInvitationUrl()} onCopy={() => toast.success(t("Copied!"))}>
            <Button>{modalType === "invitation" ? t("Copy invitation link") : t("Copy password reset link")}</Button>
          </CopyToClipboard>
        </div>
      </DialogContent>
    </Dialog>
