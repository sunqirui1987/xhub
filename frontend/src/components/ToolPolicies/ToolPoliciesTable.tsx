  ...
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">
        {filtered ? t("No matching tools") : t("No tools discovered")}
      </div>
      <div className="max-w-xs text-center text-sm text-muted-foreground">
        {filtered
          ? t("No tools match your search or filters.")
          : t("Make a chat completion that returns tool_calls to start auto-discovery.")}
      </div>
    </div>
  );
}
...
export function ToolPoliciesTable({
  ...
  const teamFilterItems = useMemo(
    () => [
      { value: ALL_VALUE, label: t("All Teams") },
  ...
  const keyAliasFilterItems = useMemo(
    () => [
      { value: ALL_VALUE, label: t("All Keys") },
