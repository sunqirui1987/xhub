...
export const ToolPoliciesPanel: React.FC<ToolPoliciesPanelProps> = ({ accessToken, onSelectTool }) => {
  ...
  const handleInputPolicyChange = useCallback(
    async (toolName: string, newPolicy: string) => {
      ...
      try {
        ...
        toast.fromError(t("Failed to update input policy: {value0}", { value0: (toMessage(e, t("unknown error"))) }));
      } finally {
  ...
  const handleOutputPolicyChange = useCallback(
    async (toolName: string, newPolicy: string) => {
      ...
      try {
        ...
        toast.fromError(t("Failed to update output policy: {value0}", { value0: (toMessage(e, t("unknown error"))) }));
      } finally {
  ...
  return (
    <div className="w-full">
      <h1 className="text-2xl font-semibold text-foreground mb-6">{t("pages.toolPolicies.title")}</h1>
      ...
      {needsReviewTools.length > 0 && (
        <div className="bg-warning/10 border border-warning/20 rounded-lg p-4 mb-6">
          <h2 className="text-sm font-semibold text-warning mb-1">{t("Needs Review")}</h2>
          <p className="text-sm text-warning mb-3">
            {t("{value0} new tool{value1} discovered that require policy decisions.", { value0: (needsReviewTools.length), value1: (needsReviewTools.length !== 1 ? "s" : "") })}</p>
