...
export const useMcpOAuthFlow = ({
  ...
  const startOAuthFlow = useCallback(async () => {
    ...
    if (!accessToken) {
      setError(t("Missing admin token"));
      toast.error(t("Access token missing. Please re-authenticate and try again."));
      return;
    }

    const temporaryPayload = getTemporaryPayload();
    if (!temporaryPayload || !temporaryPayload.url || !temporaryPayload.transport) {
      const message = t("Please complete server URL and transport before starting OAuth.");
      setError(message);
      toast.error(message);
      return;
    }
    try {
      ...
      if (!serverId) {
        throw new Error(t("Temporary MCP server identifier missing. Please retry."));
      }
      ...
      if (typeof window === "undefined") {
        throw new Error(t("OAuth redirect is only supported in the browser."));
      }
      ...
      try {
        ...
        throw new Error(t("Unable to access browser storage for OAuth. Please enable storage and retry."));
      }
  ...
  const resumeOAuthFlow = useCallback(async () => {
    ...
    try {
      ...
      setError(t("Failed to resume OAuth flow. Please retry."));
      setStatus("error");
      toast.error(t("Failed to resume OAuth flow. Please retry."));
      return;
    }
    ...
    try {
      if (!flowState || !flowState.state || !flowState.codeVerifier || !flowState.serverId) {
        throw new Error(
          t("OAuth session state was lost. This can happen if you have strict browser privacy settings.") + " " + t("Please try again and ensure cookies/storage is enabled."),
        );
      }
      if (!payload.state || payload.state !== flowState.state) {
        throw new Error(t("OAuth state mismatch. Please retry."));
      }
      if (payload.error) {
        throw new Error(payload.error_description || payload.error);
      }
      if (!payload.code) {
        throw new Error(t("Authorization code missing in callback."));
      }
      ...
      setStatus("success");
      setError(null);
      toast.success(t("OAuth token retrieved successfully"));
    } catch (err) {
      if (resetVersion !== resetVersionRef.current) {
        return;
      }
