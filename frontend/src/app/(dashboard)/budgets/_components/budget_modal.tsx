...
const budgetShape = {
  budget_id: z.string().min(1, t("Please input a human-friendly name for the budget")),
...
const BudgetModal: React.FC<BudgetModalProps> = ({ isModalVisible, setIsModalVisible }) => {
  ...
  const handleCreate = async (formValues: BudgetFormValues) => {
    try {
      ...
      toast.fromError(t("Error creating the budget: {error}", { error }));
    }
  };
  ...
  return (
    <Dialog open={isModalVisible} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
        <DialogHeader>
          <DialogTitle>{t("pages.budgets.create")}</DialogTitle>
        </DialogHeader>
        <form onSubmit={form.handleSubmit(handleCreate)} noValidate>
          <FieldGroup>
            <FormField
              ...
              name="budget_id"
              label={t("Budget ID")}
              description={t("A human-friendly name for the budget")}
            >
            ...
            <FormField
              control={form.control}
              name="tpm_limit"
              label={t("Max Tokens per minute")}
              description={t("Leave blank for no LiteLLM limit. Provider rate limits still apply.")}
            >
            ...
            <Collapsible open={optionalSettingsOpen} onOpenChange={setOptionalSettingsOpen} className="mt-20 mb-8">
              ...
              <CollapsibleContent>
                ...
                <FormField className="mt-8" control={form.control} name="budget_duration" label={t("Reset Budget")}>
          ...
          <div style={{ textAlign: "right", marginTop: "10px" }}>
            <Button type="submit">{t("pages.budgets.create")}</Button>
          </div>
        </form>
      </DialogContent>
    </Dialog>
