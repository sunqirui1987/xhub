...
describe("AccessGroupsPage", () => {
  ...
  it("renders the page title and subtitle", () => {
    ...
    expect(screen.getByRole("heading", { name: translate("en", "pages.accessGroups.title") })).toBeInTheDocument();
    expect(screen.getByText(translate("en", "pages.accessGroups.subtitle"))).toBeInTheDocument();
    expect(document.querySelector(".lucide-boxes")).not.toBeNull();
  });

  it("renders Chinese chrome when locale is zh-CN", async () => {
    ...
    expect(screen.getByRole("heading", { name: translate("zh-CN", "pages.accessGroups.title") })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: translate("zh-CN", "pages.accessGroups.create") })).toBeInTheDocument();
    expect(screen.queryByRole("heading", { name: translate("en", "pages.accessGroups.title") })).not.toBeInTheDocument();
    await user.click(await openRowMenu(user, "ag-1"));
    expect(screen.getByRole("dialog", { name: translate("zh-CN", "pages.accessGroups.deleteTitle") })).toBeInTheDocument();
    expect(screen.getByText(translate("zh-CN", "pages.accessGroups.deleteMessage"))).toBeInTheDocument();
    setActiveLocale("en");
  });
  ...
  it("opens the delete modal from the row actions menu", async () => {
    ...
    renderWithProviders(<AccessGroupsPage />);
    await user.click(await openRowMenu(user, "ag-1"));
    const dialog = screen.getByRole("dialog", { name: translate("en", "pages.accessGroups.deleteTitle") });
    expect(within(dialog).getByText(translate("en", "pages.accessGroups.deleteMessage"))).toBeInTheDocument();
    expect(within(dialog).getByText(translate("en", "pages.accessGroups.infoTitle"))).toBeInTheDocument();
  ...
  it("closes the delete modal on cancel without deleting", async () => {
    ...
    await user.click(await openRowMenu(user, "ag-1"));
    const dialog = screen.getByRole("dialog", { name: translate("en", "pages.accessGroups.deleteTitle") });
    await user.click(within(dialog).getByRole("button", { name: translate("en", "common.cancel") }));
    await waitFor(() => {
      expect(screen.queryByRole("dialog", { name: translate("en", "pages.accessGroups.deleteTitle") })).not.toBeInTheDocument();
    });
  ...
  it("calls the delete mutation with the group ID when confirmed", async () => {
    ...
    await user.click(await openRowMenu(user, "ag-1"));
    const dialog = screen.getByRole("dialog", { name: translate("en", "pages.accessGroups.deleteTitle") });
    await user.click(within(dialog).getByRole("button", { name: translate("en", "common.delete") }));
  ...
  it("still shows matches when searching from a later page", async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(translate("en", "pages.accessGroups.searchPlaceholder")), {
      target: { value: "ag-01" },
    });
    expect(await screen.findByText("ag-01")).toBeInTheDocument();
    expect(screen.queryByText(translate("en", "pages.accessGroups.emptyFiltered"))).not.toBeInTheDocument();
  });

  it("shows empty and loading chrome from catalogs in both locales", () => {
    mockUseAccessGroups.mockReturnValue({ data: [], isLoading: false });
    const first = renderWithProviders(<AccessGroupsPage />);
    expect(screen.getByText(translate("en", "pages.accessGroups.empty"))).toBeInTheDocument();
    expect(screen.getByText(translate("en", "pages.accessGroups.emptyHint"))).toBeInTheDocument();
    first.unmount();
    ...
    expect(screen.getByText(translate("zh-CN", "pages.accessGroups.empty"))).toBeInTheDocument();
    expect(screen.getByText(translate("zh-CN", "pages.accessGroups.emptyHint"))).toBeInTheDocument();
    expect(screen.queryByText(translate("en", "pages.accessGroups.empty"))).not.toBeInTheDocument();
    second.unmount();

    setActiveLocale("en");
    mockUseAccessGroups.mockReturnValue({ data: [], isLoading: true });
    renderWithProviders(<AccessGroupsPage />);
    expect(screen.getByText(translate("en", "pages.accessGroups.loading"))).toBeInTheDocument();
  });
