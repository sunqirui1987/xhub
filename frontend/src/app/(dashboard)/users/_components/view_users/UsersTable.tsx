...
function filterLabels(): Record<string, string> {
  return {
    user_id: t("pages.users.userId"),
    sso_user_id: t("pages.users.ssoId"),
    user_role: t("pages.users.filterRole"),
    team: t("pages.users.team"),
  };
}
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{t("pages.users.noUsersFound")}</div>
      <div className="text-sm text-muted-foreground">{t("pages.users.tryAdjustFilters")}</div>
    </div>
  );
}
      ...
      toolbar={(table) => (
        <>
          <DataTableToolbar
            ...
            searchPlaceholder={t("pages.users.searchByEmailOrId")}
          ...
          <DataTableFilterDrawer
            ...
            title={t("pages.users.filters")}
            description={t("pages.users.filtersBody")}
          >
            {({ get, set }) => (
              <>
                <DataTableFilterField label={t("pages.users.userId")}>
                  <Input
                    value={(get("user_id") as string) ?? ""}
                    onChange={(event) => set("user_id", event.target.value)}
                    placeholder={t("pages.users.enterUserId")}
                    data-testid="users-filter-user-id"
                  />
                </DataTableFilterField>
                <DataTableFilterField label={t("pages.users.ssoId")}>
                  <Input
                    value={(get("sso_user_id") as string) ?? ""}
                    onChange={(event) => set("sso_user_id", event.target.value)}
                    placeholder={t("pages.users.enterSsoId")}
                    data-testid="users-filter-sso-id"
                  />
                </DataTableFilterField>
                <DataTableFilterField label={t("pages.users.filterRole")}>
                  <SearchSelect
                    options={roleOptions}
                    ...
                    emptyText={t("pages.users.noRolesFound")}
                  />
                </DataTableFilterField>
                <DataTableFilterField label={t("pages.users.team")}>
                  <SearchSelect
                    options={teamOptions}
                    value={(get("team") as string) || undefined}
                    onValueChange={(value) => set("team", value ?? undefined)}
                    placeholder={t("pages.users.selectTeam")}
