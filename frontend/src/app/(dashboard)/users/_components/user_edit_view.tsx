...
export function UserEditView({
  ...
  const modelOptions = [
    { label: t("pages.users.allProxyModelsOption"), value: "all-proxy-models" },
    { label: t("pages.users.noDefaultModels"), value: "no-default-models" },
  ...
  return (
    <TooltipProvider>
      <form onSubmit={form.handleSubmit(handleSubmit)}>
        <FieldGroup>
          {!isBulkEdit && (
            <FormField control={form.control} name="user_id" label={t("pages.users.userId")}>
              {({ ref, value, ...control }) => <Input {...control} ref={ref} value={value ?? ""} disabled />}
            </FormField>
          )}

          {!isBulkEdit && (
            <FormField control={form.control} name="user_email" label={t("pages.users.email")}>
              {({ ref, value, ...control }) => <Input {...control} ref={ref} value={value ?? ""} />}
            </FormField>
          )}

          <FormField control={form.control} name="user_alias" label={t("pages.users.alias")}>
            {({ ref, value, ...control }) => <Input {...control} ref={ref} value={value ?? ""} />}
          </FormField>
          ...
          <FormField
            ...
            name="models"
            label={labelWithHint(
              t("pages.users.personalModels"),
              t("Select which models this user can access outside of team-scope. Choose 'All Proxy Models' to grant access to all models available on the proxy."),
            )}
          >
            {({ value, onChange }) => (
              <MultiSelect
                ...
                placeholder={t("pages.users.selectModels")}
          ...
          >
            {({ ref, value, onChange, ...control }) => (
              <Input
                ...
                placeholder={t("pages.users.enterNumber")}
                disabled={unlimitedBudget}
              />
            )}
          </FormField>

          <FormField control={form.control} name="budget_duration" label={t("pages.users.resetBudget")}>
          ...
          {!isBulkEdit && (
            <ModelMaxBudgetField
              ...
              hint={t("Cap this user's spend on individual models, each with its own reset window. Applies across every key the user holds.")}
            />
          )}

          <FormField control={form.control} name="metadata" label={t("pages.users.metadata")}>
            {({ ref, value, ...control }) => (
              <Textarea {...control} ref={ref} value={value ?? ""} rows={4} placeholder={t("pages.users.enterMetadataJson")} />
            )}
          </FormField>
        ...
        <div className="mt-6 flex justify-end gap-2">
          ...
          <Button type="submit">{t("pages.users.saveChanges")}</Button>
        </div>
      </form>
    </TooltipProvider>
  );
