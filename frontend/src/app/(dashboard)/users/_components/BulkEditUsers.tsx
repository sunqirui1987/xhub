  ...
  const handleSubmit = async (formValues: any) => {
    ...
    try {
      ...
      if (!hasUserUpdates && !hasTeamAdditions) {
        toast.fromError(t("Please modify at least one field or select teams to add users to"));
        return;
      }
      ...
      if (hasTeamAdditions) {
        ...
        if (failedTeams.length > 0) {
          toast.warning(t("Failed to add users to {value0} team(s)", { value0: (failedTeams.length) }));
        }
      }
      ...
      toast.fromError(t("Failed to perform bulk operations"));
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
        ...
        {!updateAllUsers && (
          <div className="mb-4">
            <h5 className="mb-2 text-sm font-semibold text-foreground">
              {t("pages.users.selectedUsers", { count: selectedUsers.length })}
            </h5>
            <div className="max-h-[200px] overflow-y-auto rounded-md border border-border">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="w-[30%]">{t("pages.users.userId")}</TableHead>
                    <TableHead className="w-[25%]">{t("pages.users.email")}</TableHead>
                    <TableHead className="w-[25%]">{t("pages.users.currentRole")}</TableHead>
                    <TableHead className="w-[20%]">{t("pages.users.budget")}</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {selectedUsers.map((user) => (
                    <TableRow key={user.user_id}>
                      ...
                      <TableCell className="text-xs text-muted-foreground">{user.user_email || t("pages.users.noEmail")}</TableCell>
                      <TableCell className="text-xs text-foreground">
                        {possibleUIRoles?.[user.user_role]?.ui_label || user.user_role}
                      </TableCell>
                      <TableCell>
                        <MoneyCell value={user.max_budget} decimals={2} emptyText={t("pages.users.unlimited")} showZero />
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
        ...
        <div className="mb-4">
          <p className="text-sm text-foreground">
            <strong>{t("Instructions:")}</strong> {t("Fill in the fields below with the values you want to apply to all selected users. You can bulk edit: role, budget, models, and metadata. You can also add users to teams.")}
          </p>
        </div>
        ...
        <Card size="sm" className="mb-4 bg-muted/50">
          <CardHeader>
            <CardTitle>{t("pages.users.teamManagement")}</CardTitle>
          </CardHeader>
          <CardContent>
                ...
                <>
                  <div>
                    <label htmlFor={selectedTeamsId} className="block text-sm font-medium text-foreground">
                      {t("pages.users.selectTeams")}
                    </label>
                    <MultiSelect
                      id={selectedTeamsId}
                      className="mt-2"
                      placeholder={t("pages.users.selectTeamsPlaceholder")}
                      value={selectedTeams}
                  ...
                  <div>
                    <label htmlFor={teamBudgetId} className="block text-sm font-medium text-foreground">
                      {t("pages.users.teamBudgetOptional")}
                    </label>
                    <NumericalInput
                      id={teamBudgetId}
                      className="mt-2"
                      placeholder={t("pages.users.maxBudgetPerUser")}
                      value={teamBudget ?? ""}
                  ...
                  <span className="text-xs text-muted-foreground">
                    {t("Users will be added with \"user\" role by default. All users will be added to each selected team.")}
                  </span>
                </>
              )}
            </div>
        ...
        {loading && (
          <div className="mt-2.5 text-center">
            <span className="text-sm text-foreground">
              {t("Updating {value0} user(s)...", { value0: (updateAllUsers ? t("all users") : selectedUsers.length) })}</span>
          </div>
        )}
      </DialogContent>
    </Dialog>
