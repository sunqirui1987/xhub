...
const PROMPT_INTEGRATION_OPTIONS = [{ label: t("dotprompt"), value: "dotprompt" }];

const addPromptSchema = z.object({
  prompt_id: z
    .string()
    .min(1, t("Please enter a prompt ID"))
    .regex(/^[a-zA-Z0-9_-]+$/, t("Prompt ID can only contain letters, numbers, underscores, and hyphens")),
...
const AddPromptForm: React.FC<AddPromptFormProps> = ({ visible, onClose, accessToken, onSuccess }) => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!picked.name.endsWith(".prompt")) {
      toast.fromError(t("Please upload a .prompt file"));
      clearSelectedFile();
      return;
    }
  ...
  const convertUploadedFile = async (
    ...
    try {
      ...
      toast.fromError(t("Failed to convert prompt file to JSON"));
      return null;
    }
  };
  ...
  const handleSubmit = async (values: AddPromptFormValues) => {
    if (!accessToken) {
      toast.fromError(t("Access token is required"));
      return;
    }
    ...
    try {
      ...
      toast.fromError(t("Failed to create prompt"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={visible} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>{t("Add New Prompt")}</DialogTitle>
        </DialogHeader>
        <form onSubmit={(event) => event.preventDefault()} noValidate>
          <FieldGroup>
            <FormField control={form.control} name="prompt_id" label={t("Prompt ID")}>
              {({ ref, ...field }) => (
                <Input {...field} ref={ref} placeholder={t("Enter unique prompt ID (e.g., my_prompt_id)")} />
              )}
            </FormField>

            <FormField control={form.control} name="prompt_integration" label={t("Prompt Integration")}>
            ...
            {promptIntegration === "dotprompt" && (
              <>
                <FieldSeparator />
                <Field>
                  <FieldTitle>{t("Prompt File")}</FieldTitle>
                  <input
                    ref={fileInputRef}
                    type="file"
                    accept=".prompt"
                    aria-label={t("Prompt file")}
                    className="sr-only"
                    onChange={handleFileChange}
                  />
                  <Button type="button" variant="outline" onClick={() => fileInputRef.current?.click()}>
                    <UploadIcon />
                    {t("Select .prompt File")}
                  </Button>
                  {selectedFile && (
                    <div className="mt-2 flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <button
                        type="button"
                        aria-label={t("Remove {value0}", { value0: (selectedFile.name) })}
                  ...
                  <FieldDescription>{t("Upload a .prompt file that follows the Dotprompt specification")}</FieldDescription>
                </Field>
              </>
            )}
          </FieldGroup>
