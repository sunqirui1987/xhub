...
const PromptInfoView: React.FC<PromptInfoProps> = ({
  ...
  const fetchPromptInfo = async (environment?: string) => {
    try {
      ...
      toast.fromError(t("Failed to load prompt information"));
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(t("Prompt \"{basePromptId}\" deleted successfully", { basePromptId }));
      onDelete?.();
      onClose();
    } catch (error) {
      console.error("Error deleting prompt:", error);
      toast.fromError(t("Failed to delete prompt"));
    } finally {
      setIsDeleting(false);
  ...
  const handleVersionClick = async (version: PromptSpec) => {
    ...
    try {
      ...
      toast.fromError(t("Failed to load version v{versionNum}", { versionNum }));
    }
  };
    ...
    <div className="p-4">
      <div>
        <Button variant="ghost" onClick={onClose} className="mb-4">
          ...
          {t("pages.prompts.back")}
        </Button>
        <div className="flex justify-between items-start mb-4">
          <div>
            <h1 className="text-2xl font-semibold">{t("pages.prompts.details")}</h1>
        ...
        <div>
          ...
          <TabsContent value="overview" keepMounted>
            ...
            <Card className="block mt-6 p-6">
              <h3 className="text-lg font-medium mb-3">{t("Version History — {selectedEnv}", { selectedEnv })}</h3>
              {loadingVersions ? (
                ...
                <p className="text-muted-foreground">{t("No versions found in {selectedEnv}", { selectedEnv })}</p>
              )}
            </Card>
          </TabsContent>
            ...
            <TabsContent value="prompt-template" keepMounted>
              <Card className="block p-6">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="text-lg font-medium">{t("Prompt Template")}</h3>
                  <Button
                    variant="ghost"
                    size="sm"
                ...
                <div className="space-y-4">
                  <div>
                    <p className="font-medium">{t("Template ID")}</p>
                  ...
                  {promptTemplate.metadata && Object.keys(promptTemplate.metadata).length > 0 && (
                    <div>
                      <p className="font-medium">{t("Template Metadata")}</p>
          ...
          <TabsContent value="raw-json" keepMounted>
            <Card className="block p-6">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium">{t("Raw API Response")}</h3>
                <Button
                  variant="ghost"
                  size="sm"
      ...
      <Dialog open={showDeleteConfirm} onOpenChange={(open) => !open && handleDeleteCancel()}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
          ...
          <p>
            {t("Are you sure you want to delete prompt:")} <strong>{basePromptId}</strong> {t("from every environment?")}
          </p>
