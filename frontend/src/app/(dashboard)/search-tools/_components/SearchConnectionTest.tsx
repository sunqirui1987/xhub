      ...
      <div className="rounded-lg bg-card p-6">
        <div className="flex flex-col items-center justify-center px-5 py-8">
          ...
          <p className="text-base text-foreground">
            {t("Testing connection to {value0}...", { value0: (litellmParams.search_provider || t("search provider")) })}</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center justify-center px-5 py-8">
          ...
          <div className="ml-3">
            <p className="text-lg font-medium text-success">
              {t("Connection to {value0} successful!", { value0: (litellmParams.search_provider) })}</p>
            {testResult.test_query && (
              <p className="mt-2 text-sm text-muted-foreground">
                {t("Test query:")} <code className="rounded bg-muted px-1.5 py-0.5">{testResult.test_query}</code>
              </p>
            )}
            {testResult.results_count !== undefined && (
              <p className="text-sm text-muted-foreground">{t("Results retrieved: {value0}", { value0: (testResult.results_count) })}</p>
            )}
          </div>
        </div>
      ) : (
        <div>
          <div className="mb-5 flex items-center">
            ...
            <p className="text-lg font-medium text-destructive">
              {t("Connection to {value0} failed", { value0: (litellmParams.search_provider || t("search provider")) })}</p>
          </div>
          ...
          <div className="rounded-lg border border-warning/20 border-l-4 border-l-amber-500 bg-warning/10 p-4">
            <p className="mb-2 font-semibold text-warning">{t("Troubleshooting tips:")}</p>
