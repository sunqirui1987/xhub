...
const createSearchToolShape = {
  search_tool_name: z
    .string()
    .min(1, t("Please enter a search tool name"))
    .regex(/^[a-zA-Z0-9_-]+$/, t("Name can only contain letters, numbers, hyphens, and underscores")),
  search_provider: z
    .string()
    .nullable()
    .pipe(z.string({ error: "Please select a search provider" }).min(1, t("Please select a search provider"))),
...
const CreateSearchTool: React.FC<CreateSearchToolProps> = ({
  ...
  const handleCreate = async (formValues: CreateSearchToolFormValues) => {
    ...
    try {
      ...
      if (accessToken != null) {
        ...
        toast.success(t("Search tool created successfully"));
        form.reset(EMPTY_VALUES);
        setModalVisible(false);
        onCreateSuccess(response);
      }
    } catch (error) {
      toast.error(t("Error creating search tool: ") + error);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleTestConnection = async () => {
    ...
    if (!isValid) {
      toast.error(t("Please fill in Search Provider and API Key before testing"));
      return;
    }
      ...
      <DialogContent className="top-8 max-h-[calc(100dvh-4rem)] translate-y-0 overflow-y-auto sm:max-w-[800px]">
        <DialogHeader>
          <div className="flex items-center space-x-3 pb-4 border-b border-border">
            ...
            <DialogTitle className="text-xl font-semibold text-foreground">{t("Add New Search Tool")}</DialogTitle>
          </div>
        </DialogHeader>
        <div className="mt-6">
          <TooltipProvider>
            <form onSubmit={form.handleSubmit(handleCreate)} className="space-y-6">
              <FieldGroup>
                <FormField
                  ...
                  label={labelWithHint(
                    t("Search Tool Name"),
                    t("A unique name to identify this search tool configuration (e.g., 'perplexity-search', 'tavily-news-search')."),
                  )}
                >
                  {({ ref, ...field }) => (
                    <Input
                ...
                <FormField
                  ...
                  label={labelWithHint(
                    t("Search Provider"),
                    t("Select the search provider you want to use. Each provider has different capabilities and pricing."),
                  )}
                >
                  {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
                    ...
                    >
                      ...
                      />
                      <ComboboxContent>
                        <ComboboxEmpty>{t("No matching search providers")}</ComboboxEmpty>
                        <ComboboxList>
                          {(providerName: string) => (
                ...
                <FormField
                  ...
                  label={labelWithHint(
                    t("API Key"),
                    t("The API key for authenticating with the search provider. This will be securely stored."),
                  )}
                >
                  {({ ref, value, ...field }) => (
                    <PasswordInput
                ...
                <FormField control={form.control} name="description" label={t("Description (Optional)")}>
                  {({ ref, value, ...field }) => (
                    <Textarea
                      {...field}
                      ref={ref}
