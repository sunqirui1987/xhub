...
export const SearchToolTester: React.FC<SearchToolTesterProps> = ({ searchToolName, accessToken, className = "" }) => {
  ...
  const handleSearch = async () => {
    if (!query.trim()) {
      toast.warning(t("Please enter a search query"));
      return;
    }
    ...
    try {
      ...
      toast.fromError(t("Failed to query search tool"));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const clearHistory = () => {
    setSearchHistory([]);
    setExpandedResults({});
    toast.success(t("Search history cleared"));
  };
                      ...
                      <div className="ml-4 text-right">
                        ...
                        <div className="mt-1 flex items-center gap-3">
                          <div className="text-sm font-semibold text-primary">
                            {t("{value0} {value1}", { value0: (latestResults.response?.results?.length || 0), value1: (latestResults.response?.results?.length === 1 ? "result" : "results") })}
                          </div>
                      ...
                      >
                        ...
                        <div className="mt-1.5 flex items-center gap-2 text-xs text-muted-foreground">
                          <span className="font-medium text-primary">
                            {t("{value0} {value1}", { value0: (entry.response?.results?.length || 0), value1: (entry.response?.results?.length === 1 ? "result" : "results") })}
                          </span>
