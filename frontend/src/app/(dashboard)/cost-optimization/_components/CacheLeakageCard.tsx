    ...
    <TableHead className="text-right">
      <span className="inline-flex items-center justify-end gap-1">
        <button
          ...
          aria-label={t("Sort by {label}", { label })}
        ...
        <CardHeader>
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div className="min-w-0">
              <CardTitle>{t("Cache leakage by {value0}", { value0: (dimension === "model" ? "model" : t("virtual key")) })}</CardTitle>
              <p className="mt-1 text-sm text-muted-foreground line-clamp-2">
                {t("{subject} sending large volumes of uncached input with a low cache hit rate are likely missing prompt caching. Potential savings is approximate: uncached input priced at what your cached traffic nets per cached token, after cache-write premiums.", { subject })}</p>
            </div>
        ...
        <CardContent>
          {rows.length > 0 && isFetchingMore && (
            <p className="mb-2 text-sm text-muted-foreground">
              {t("Data is still loading; rows and totals will update as the rest of the range arrives.")}
            </p>
          )}
          {rows.length === 0 ? (
            <p className="py-8 text-center text-sm text-muted-foreground">
              {loading || isFetchingMore ? t("Loading...") : t("No {emptyNoun} usage in this range.", { emptyNoun })}
            </p>
          ) : (
            <Table>
              <TableHeader>
