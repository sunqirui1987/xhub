...
const compressionSchema = z.object({
  ...
  apiBase: z.string().min(1, t("API base is required")),
  defaultOn: z.boolean(),
});
...
const PromptCompressionTab: React.FC<PromptCompressionTabProps> = ({ accessToken }) => {
  ...
  const loadGuardrails = useCallback(() => {
    ...
    getGuardrailsList(accessToken)
      ...
      .catch((error) => {
        ...
        toast.fromError(t("Failed to load compression guardrails"));
      })
  ...
  const handleAdd = async (values: CompressionFormValues) => {
    ...
    try {
      ...
      toast.success(t("Compression guardrail created"));
      form.reset(EMPTY_VALUES);
      await loadGuardrails();
    } catch (error) {
      console.error("Failed to create compression guardrail:", error);
      toast.fromError(t("Failed to create compression guardrail"));
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <Card>
        ...
        <CardContent>
          <p className="mb-4 text-sm text-muted-foreground">
            {t("Headroom is a native LiteLLM guardrail that compresses your prompts before they reach the model, so you pay for fewer input tokens. The tokens it removes are priced and shown on the Usage tab as compression savings.")}{" "}
            ...
            <form onSubmit={form.handleSubmit(handleAdd)} noValidate>
              <FieldGroup>
                ...
                <FormField
                  ...
                  label={labelWithHint(
                    t("Headroom API base"),
                    t("Base URL of your Headroom compression service (LiteLLM calls its /v1/compress endpoint)"),
                  )}
                  description={t("The URL where your Headroom compression service is hosted")}
                >
                  {({ ref, ...field }) => <Input {...field} ref={ref} placeholder="https://your-headroom-endpoint" />}
                </FormField>
                <FormField control={form.control} name="defaultOn" label={t("Apply to all requests")}>
              ...
              <div className="mt-6 mb-4 rounded-lg border border-warning/20 bg-warning/10 p-3">
                <p className="text-sm text-warning">
                  {t("Applying compression to all requests is available to all users. Enabling it selectively per key or team is a LiteLLM Enterprise feature. Get a trial key")}{" "}
