    ...
    <Card className="overflow-hidden py-0">
      <div className="grid md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        ...
        <div className="flex flex-col justify-center border-t p-6 md:border-t-0 md:border-l">
          <SpendRow label={t("Actual auto-router spend")} value={usd(stats.spend)} />
          ...
          {stats.classifier_cost == null && (
            <p className="mb-3 text-xs text-muted-foreground">
              {t("Breakdown unavailable because some usage predates classification-cost tracking.")}
            </p>
          )}
          <Separator />
          <SpendRow label={t("Estimated spend at highest-tier model")} value={usd(stats.baseline_spend)} />
        </div>
      </div>
    </Card>
  );
            ...
            <TooltipProvider delay={200}>
              <Tooltip>
                ...
                <TooltipContent className="max-w-64">
                  {t("share of all measured turns that missed cache because a return to an earlier tier came after its TTL lapsed")}
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )}
        ...
        <div className="flex flex-col gap-3 p-6">
          <div className="flex items-baseline justify-between">
            ...
            <p className="text-xs text-muted-foreground">
              <span className="text-lg font-semibold tabular-nums text-foreground">{total.toLocaleString()}</span> {t("turns measured")}
            </p>
          </div>
          ...
          {cache.unordered_turns > 0 && (
            <p className="text-xs text-muted-foreground">
              {t("{value0} turns arrived out of order across pods and are not bucketed", { value0: (cache.unordered_turns.toLocaleString()) })}</p>
          )}
        </div>
      </div>
    </Card>
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Metric
          label={t("Avg saved per session")}
          value={usd(stats.saved_per_session)}
          hint={t("· {value0} sessions", { value0: (stats.sessions.toLocaleString()) })}
        />
