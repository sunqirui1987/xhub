...
const jobModelScope = (job: ShadowEvalJob): React.ReactNode =>
  job.models && job.models.length > 0 ? (
    <>
      {" "}
      {t("on")} <span className="font-mono text-xs">{job.models.join(", ")}</span>
    </>
  ) : null;
...
const jobHeadline = (job: ShadowEvalJob): React.ReactNode =>
  job.direction === "reverse" ? (
    <>
      {t("Comparing")} <span className="font-mono text-xs">{jobRouters(job)}</span> {t("to")}{" "}
      <span className="font-mono text-xs">{job.baseline_model}</span> {t("on")} {job.shadow_percentage}{t("% of")}{" "}
      <span className="font-mono text-xs">{shadowedTargetsLabel(job)}</span> {t("traffic")}{jobModelScope(job)}
    </>
  ) : (
    <>
      {t("Shadowing")} {job.shadow_percentage}{t("% of")} <span className="font-mono text-xs">{shadowedTargetsLabel(job)}</span>{" "}
      {t("traffic")}{jobModelScope(job)} {t("via")} <span className="font-mono text-xs">{jobRouters(job)}</span>
    </>
  );
    ...
    <TableHeader>
      <TableRow>
        ...
        {[
          t("Judged turns"),
          t("Router wins"),
          t("{value0} wins", { value0: (otherArmLabel(direction)) }),
          t("Ties"),
          t("Judge confidence"),
          t("Router cost"),
          t("{value0} cost", { value0: (otherArmLabel(direction)) }),
        ].map((label) => (
        ...
        <TableRow key={slice.group}>
          <TableCell className="font-medium text-foreground">
            ...
            {slice.turn_count < MIN_TURNS_FOR_CONFIDENCE && (
              <span className="ml-2 text-xs font-normal text-muted-foreground">{t("(low sample)")}</span>
            )}
          </TableCell>
  ...
  return (
    <div className="flex min-w-[240px] flex-1 flex-col gap-1 border-t px-6 py-4 sm:border-l sm:border-t-0">
      <p className="flex items-center gap-1 text-[11px] uppercase tracking-wide text-muted-foreground">
        {t("Router cost vs")} {direction === "reverse" ? t("the baseline") : t("your current model")}
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger render={<CircleHelp className="size-3.5 shrink-0 cursor-help" />} />
            <TooltipContent>
              {t("Each arm is priced as its completion plus its own routing classifier call, measured on the same judged turns; the judge's cost is excluded from both arms")}
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      </p>
      ...
      <p className="text-xs text-muted-foreground">
        ...
        {cacheHits > 0 ? t("; {value0} cache-served turns excluded", { value0: (cacheHits.toLocaleString()) }) : ""}
      </p>
    </div>
  );
};
...
const VerdictBar: React.FC<{ direction: ShadowEvalDirection; results: NonNullable<ShadowEvalJob["results"]> }> = ({
  ...
  const segments = [
    { label: t("Router won"), value: routerWins, fill: "bg-success" },
    { label: t("Tie"), value: ties, fill: "bg-success/20" },
    {
      label: t("{value0} won", { value0: (otherArmLabel(direction)) }),
    ...
    <Table>
      <TableHeader>
        <TableRow>
          ...
          {[t("Budget used"), t("Router wins"), t("{value0} wins", { value0: (otherArmLabel(job.direction)) })].map((label) => (
            <TableHead key={label} className="text-right">
              {label}
            </TableHead>
          ))}
            ...
            <TableRow key={`${target.target_type}:${target.target_id}`}>
              ...
              <TableCell className="text-right tabular-nums">
                {target.max_budget != null
                  ...
                  : t("{value0} / {value1} turns", { value0: ((target.attempt_count ?? slice?.turn_count ?? 0).toLocaleString()), value1: (target.max_turns.toLocaleString()) })}
              </TableCell>
              {slice ? (
                <>
                  <TableCell className="text-right font-medium tabular-nums text-foreground">
          ...
          <div className="flex flex-wrap border-b">
            <div className="flex min-w-[240px] flex-1 flex-col gap-1 px-6 py-4">
              <p className="text-[11px] uppercase tracking-wide text-muted-foreground">
                {t("Router matched or beat {value0}", { value0: (job.direction === "reverse" ? t("the baseline") : t("your current model")) })}
              </p>
              <p className="text-3xl font-semibold text-foreground">
                {pct(routerMatchedOrBeatPct(job.direction, results))}
              </p>
              <p className="text-xs text-muted-foreground">
                {t("of {value0} judged responses", { value0: ((job.judged_count ?? 0).toLocaleString()) })}</p>
            </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              ...
              {totalBudget(job) !== null ? t("of {value0}", { value0: (usd(totalBudget(job) ?? 0)) }) : ""} eval spend
...
const PreviousJobs: React.FC<{ jobs: readonly ShadowEvalJob[] }> = ({ jobs }) => {
  ...
  return (
    <Card className="overflow-hidden py-0">
      ...
      >
        <span className="text-sm font-medium text-foreground">{t("Previous evaluations ({value0})", { value0: (jobs.length) })}</span>
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-baseline gap-2">
        ...
        <p className="text-sm text-muted-foreground">
          {t("Blind-judge the auto-router on the real traffic of a key, team, or user (teams and users cover JWT-authenticated traffic): against the models they use today before switching, or against a fixed baseline after they have switched.")}
        </p>
      </div>
