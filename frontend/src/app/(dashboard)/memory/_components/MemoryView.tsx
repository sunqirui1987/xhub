...
export const MemoryView: React.FC<MemoryViewProps> = ({ accessToken }) => {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (row) => {
      toast.success(t("Created {value0}", { value0: (row.key) }));
      invalidateList();
    },
    onError: (err: Error) => {
      toast.error(t("Save failed: {value0}", { value0: (err.message) }));
    },
  });
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: (row) => {
      toast.success(t("Updated {value0}", { value0: (row.key) }));
      invalidateList();
    },
    onError: (err: Error) => {
      toast.error(t("Save failed: {value0}", { value0: (err.message) }));
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (key) => {
      toast.success(t("Deleted {key}", { key }));
      invalidateList();
    },
    onError: (err: Error) => {
      toast.error(t("Delete failed: {value0}", { value0: (err.message) }));
    },
  });
  ...
  const handleSave = async (key: string, value: string, metadataText: string, isCreate: boolean): Promise<boolean> => {
    ...
    if (!metadataText.trim()) {
      ...
      try {
        ...
        toast.error(t("Metadata must be valid JSON (or leave empty)."));
        return false;
      }
    }
    ...
    <div className="w-full p-6">
      <div className="flex flex-col gap-6">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h1 className="text-2xl font-semibold text-foreground">{t("pages.memory.title")}</h1>
