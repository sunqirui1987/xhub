...
const INTENT_TYPES = [
  { value: "airline", label: t("Airline (auto-load competitors from IATA)") },
  { value: "generic", label: t("Generic (specify competitors manually)") },
] as const;

const COMPETITOR_COMPARISON_POLICIES = [
  { value: "refuse", label: t("Refuse (block request)") },
  { value: "reframe", label: t("Reframe (suggest alternative)") },
] as const;

const POSSIBLE_COMPETITOR_COMPARISON_POLICIES = [
  { value: "refuse", label: t("Refuse (block request)") },
  { value: "reframe", label: t("Reframe (suggest alternative to backend LLM)") },
] as const;

const THRESHOLDS = [
  { field: "threshold_high", label: t("High"), hint: "e.g. 0.7", fallback: 0.7 },
  { field: "threshold_medium", label: t("Medium"), hint: "e.g. 0.45", fallback: 0.45 },
  { field: "threshold_low", label: t("Low"), hint: "e.g. 0.3", fallback: 0.3 },
] as const;
...
const CompetitorIntentConfiguration: React.FC<CompetitorIntentConfigurationProps> = ({
  ...
  const header = (
    <CardHeader className="gap-0">
      <CardTitle className="text-base">{t("Competitor Intent Filter")}</CardTitle>
      <CardAction>
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            {t("Block or reframe competitor comparison questions. When enabled, airline type auto-loads competitors from IATA; generic type requires manual competitor list.")}
          </p>
        </CardContent>
      </Card>
    );
    ...
    <Card>
      ...
      <CardContent>
        <p className="mb-4 text-sm text-muted-foreground">
          {t("Block or reframe competitor comparison questions. Airline type uses major airlines (excluding your brand); generic requires manual competitor list.")}
        </p>
        <FieldGroup>
          <Field>
            <FieldLabel htmlFor={`${fieldId}-type`}>{t("Type")}</FieldLabel>
            <Select
              items={INTENT_TYPES}
          ...
          <Field>
            <FieldLabel htmlFor={`${fieldId}-brand-self`}>{t("Your Brand (brand_self)")}</FieldLabel>
            <TagsInput
          ...
          {effectiveConfig.competitor_intent_type === "airline" && (
            <Field>
              <FieldLabel htmlFor={`${fieldId}-locations`}>{t("Locations (optional)")}</FieldLabel>
              <TagsInput
              ...
              />
              <FieldDescription>{t("Countries, cities, airports for disambiguation (e.g. qatar, doha)")}</FieldDescription>
            </Field>
          )}

          {effectiveConfig.competitor_intent_type === "generic" && (
            <Field>
              <FieldLabel htmlFor={`${fieldId}-competitors`}>{t("Competitors")}</FieldLabel>
              <TagsInput
                ...
                placeholder={t("Type and press Enter to add")}
              />
              <FieldDescription>{t("Competitor names to detect (required for generic type)")}</FieldDescription>
            </Field>
          )}

          <Field>
            <FieldLabel htmlFor={`${fieldId}-competitor-comparison`}>{t("Policy: Competitor comparison")}</FieldLabel>
            <Select
          ...
          <Field>
            <FieldLabel htmlFor={`${fieldId}-possible-competitor-comparison`}>
              {t("Policy: Possible competitor comparison")}
            </FieldLabel>
            <Select
          ...
          <Field>
            <FieldLabel>{t("Confidence thresholds")}</FieldLabel>
            ...
            <FieldDescription>
              {t("Classify competitor intent by confidence (0–1). Higher confidence -> stronger intent.")}
              <ul className="mt-1 mb-0 list-disc pl-5">
                <li>
                  <strong>{t("High (≥)")}</strong>{t(": Treat as full competitor comparison -> uses \"Competitor comparison\" policy")}
                </li>
                <li>
                  <strong>{t("Medium (≥)")}</strong>{t(": Treat as possible comparison -> uses \"Possible competitor comparison\" policy")}
                </li>
                <li>
                  <strong>{t("Low (≥)")}</strong>{t(": Log only; allow request. Below Low -> allow with no action")}
                </li>
              </ul>
              {t("Raise thresholds to be more permissive; lower them to be stricter.")}
            </FieldDescription>
          </Field>
        </FieldGroup>
      </CardContent>
