...
const ContentFilterConfiguration: React.FC<ContentFilterConfigurationProps> = ({
  ...
  const handleAddPrebuiltPattern = () => {
    if (!selectedPatternName) {
      toast.error(t("Please select a pattern"));
      return;
    }
  ...
  const handleAddCustomPattern = () => {
    if (!customPatternName || !customPatternRegex) {
      toast.error(t("Please provide pattern name and regex"));
      return;
    }
  ...
  const handleAddKeyword = () => {
    if (!newKeyword) {
      toast.error(t("Please enter a keyword"));
      return;
    }
    ...
    try {
      ...
      if (accessToken) {
        ...
        if (result.valid) {
          ...
          toast.success(result.message || t("File uploaded successfully"));
        } else {
          const errorMessage = result.error || (result.errors && result.errors.join(", ")) || "Invalid file";
          toast.error(t("Validation failed: {errorMessage}", { errorMessage }));
        }
      }
    } catch (error) {
      toast.error(t("Failed to upload file: {error}", { error }));
    } finally {
