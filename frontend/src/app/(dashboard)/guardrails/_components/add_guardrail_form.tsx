  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        toast.fromError(t("Failed to load guardrail configuration"));
      }
    };
  ...
  const nextStep = async () => {
    ...
    if (currentStep === 1) {
      if (shouldRenderPIIConfigSettings(selectedProvider) && selectedEntities.length === 0) {
        toast.fromError(t("Please select at least one PII entity to continue"));
        return;
      }
    }
    ...
    try {
      ...
      // Add config values to the guardrail_info if provided
        ...
        try {
          ...
          toast.fromError(t("Invalid JSON in configuration"));
          setLoading(false);
          return;
        }
      }
      ...
      if (guardrailProvider === "llm_as_a_judge") {
        ...
        if (criteria.length === 0) {
          toast.fromError(t("Add at least one evaluation criterion"));
          setLoading(false);
          return;
        }
        const weightTotal = criteria.reduce((sum, c) => sum + (Number(c?.weight) || 0), 0);
        if (weightTotal !== 100) {
          toast.fromError(t("Criterion weights must sum to 100% (currently {weightTotal}%)", { weightTotal }));
          setLoading(false);
          return;
        }
      ...
      if (guardrailProvider === "tool_permission") {
        if (toolPermissionConfig.rules.length === 0) {
          toast.fromError(t("Add at least one tool permission rule"));
          setLoading(false);
          return;
        }
      ...
      toast.success(t("Guardrail created successfully"));
      ...
      toast.fromError(t("Failed to create guardrail: ") + (error instanceof Error ? error.message : String(error)));
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <FieldGroup>
        ...
        <GuardrailField
          ...
          rules={requiredRule(t("Please select a provider"))}
        >
          {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
            ...
            >
              ...
              />
              <ComboboxContent>
                <ComboboxEmpty>{t("No matching providers")}</ComboboxEmpty>
                <ComboboxList>
                  {(key: string) => (
        ...
        <GuardrailField
          control={form.control}
          name="mode"
          label={labelWithHint(t("Mode"), t("How the guardrail should be applied"))}
          rules={requiredRule(t("Please select a mode"))}
        >
          {({ id, value, onChange }) => (
            <MultiSelect
              id={id}
        ...
        <GuardrailField
          control={form.control}
          name="default_on"
          label={labelWithHint(t("Always On"), t("If enabled, this guardrail will be applied to all requests by default."))}
        >
          {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
            <Select
        ...
        <GuardrailField
          ...
          label={labelWithHint(
            ...
            t("Unified guardrails only: omit role: system from guardrail evaluation input (OpenAI chat + Anthropic messages). The model still receives full messages. Use global default follows litellm_settings.skip_system_message_in_guardrail."),
          )}
        >
        ...
        <GuardrailField
          ...
          label={labelWithHint(
            ...
            t("Unified guardrails only: omit role: tool from guardrail evaluation input (OpenAI chat + Anthropic messages). The model still receives full messages. Use global default follows litellm_settings.skip_tool_message_in_guardrail."),
          )}
        >
          {(fieldControl) => <SkipMessageSelect control={fieldControl} />}
        </GuardrailField>
      ...
      <div className="space-y-6">
        <div>
          <p className="text-sm text-muted-foreground">
            {t("Configure settings for a specific call type. Most guardrails don't need this — skip it unless you're using a specific endpoint like")} <code>/v1/realtime</code>.
          </p>
        </div>
        ...
        <div>
          ...
          >
            <SelectTrigger id="guardrail-call-type" className="w-65">
              <SelectValue placeholder={t("Select a call type")} />
            </SelectTrigger>
            <SelectContent>
          ...
          <p className="mt-1 text-xs text-muted-foreground">{t("More call types coming soon.")}</p>
        </div>
        ...
        <TooltipProvider>
          <div className="flex flex-col">
            ...
            <div className="flex items-center justify-between border-b border-border px-6 py-4">
              <DialogTitle className="m-0 text-base font-semibold text-foreground">{t("Create guardrail")}</DialogTitle>
              <button
                type="button"
                onClick={handleClose}
                    ...
                    <div key={index} className={`relative flex gap-4 ${isLast ? "" : "pb-2"}`}>
                      ...
                      <div className={`min-w-0 flex-1 ${isLast ? "" : "pb-4"}`}>
                        ...
                        >
                          ...
                          {isDone && <span className="text-[11px] text-info hover:underline">{t("Edit")}</span>}
                        </div>
