...
function onFailureItems() {
  return [
    { label: t("Block (return 422)"), value: "block" },
    { label: t("Log only"), value: "log" },
  ];
}
  ...
  return (
    <FieldGroup>
      <div className="rounded-md border border-success/20 bg-success/10 px-3.5 py-2.5 text-[13px] text-success">
        {t("After each LLM response, the")} <strong>{t("Judge Model")}</strong> {t("scores it 0–100 against your criteria. If the weighted average falls below the threshold, the response is blocked (or logged).")}
      </div>
      ...
      <GuardrailField
        ...
        label={labelWithHint(
          t("Judge Model"),
          t("The LLM that reads each response and grades it. Pick a capable model — it never sees end-user data beyond what the LLM returned."),
        )}
        rules={requiredRule(t("Select a judge model"))}
      >
      ...
      <GuardrailField
        ...
        label={labelWithHint(
          ...
          t("0–100. If the weighted average of criterion scores falls below this, the guardrail triggers. 80 is a good default."),
        )}
        defaultValue={80}
      >
        ...
        {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
          <Select items={onFailureItems()} value={asText(value) || null} onValueChange={onChange}>
            <SelectTrigger id={id} aria-invalid={ariaInvalid} aria-describedby={ariaDescribedBy} className="w-full">
              <SelectValue placeholder={t("Select an action")} />
            </SelectTrigger>
            <SelectContent>
      ...
      <Field>
        <FieldLabel>
          {labelWithHint(
            ...
            t("Each criterion is something the judge checks. Weights must add up to 100%."),
          )}
        </FieldLabel>
          ...
          <div key={index} className="mb-2 rounded-md border border-border p-3">
            <div className="flex items-end gap-2">
              <GuardrailField
                ...
                rules={requiredRule(t("Enter criterion name"))}
                className="flex-2"
              >
                {({ ref, value, ...field }) => (
                  <Input
                    ...
                    ref={ref}
                    value={asText(value)}
                    placeholder={t("Criterion name (e.g. Policy accuracy)")}
                  />
                )}
              </GuardrailField>
              <GuardrailField
                ...
                label={labelWithHint(
                  ...
                  t("How much this criterion counts toward the final score. All weights must add up to 100%."),
                )}
                rules={requiredRule(t("Enter weight"))}
                className="flex-1"
              >
                {(fieldControl) => (
            ...
            <GuardrailField
              ...
              rules={requiredRule(t("Describe what to check"))}
              className="mt-2"
            >
              {({ ref, value, ...field }) => (
                <Input
                  ...
                  ref={ref}
                  value={asText(value)}
                  placeholder={t("What should the judge check for this criterion?")}
                />
              )}
            </GuardrailField>
          </div>
        ...
        {criteria.length > 0 && (
          <div className={`mt-1.5 text-xs ${weightOk ? "text-success" : "text-warning"}`}>
            {t("Weights total: {weightTotal}%{value1}", { weightTotal, value1: (weightOk ? " ✓" : t("— must add up to 100%")) })}
          </div>
        )}
      </Field>
    </FieldGroup>
