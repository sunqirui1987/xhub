          ...
          >
            {(fieldControl) => {
              if (field.dict_value_type === "number") {
                return (
                  <NumericalInput
                    ...
                    step={1}
                    placeholder={t("Enter {value0} value", { value0: (entry.key) })}
              ...
              if (field.dict_value_type === "boolean") {
                return <BooleanSelect control={fieldControl} placeholder={t("Select {value0} value", { value0: (entry.key) })} />;
              }
        ...
        <div className="mt-2 flex items-center space-x-3">
          ...
          >
            <SelectTrigger className="w-50">
              <SelectValue placeholder={t("Select category to configure")} />
            </SelectTrigger>
            <SelectContent>
          ...
          <span className="text-sm text-muted-foreground">{t("Select a category to add threshold configuration")}</span>
        </div>
      )}
    </div>
  );
    ...
    return (
      <div key={fullFieldKey} className="mb-8 rounded-lg border border-border bg-card p-6 shadow-xs">
        <GuardrailField
          ...
          rules={field.required ? requiredRule(t("{fieldKey} is required", { fieldKey })) : undefined}
  ...
  return (
    <div className="guardrail-optional-params">
      <div className="mb-8 border-b border-border pb-4">
        <h3 className="mb-2 text-lg font-semibold text-foreground">{t("Optional Parameters")}</h3>
