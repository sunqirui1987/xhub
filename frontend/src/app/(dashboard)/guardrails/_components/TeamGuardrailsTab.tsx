...
const GUARDRAIL_MODES = [
  { value: "pre_call", label: t("Pre Call") },
  { value: "post_call", label: t("Post Call") },
  { value: "during_call", label: t("During Call") },
] as const;
...
const submitGuardrailSchema = z.object({
  team_id: z
    .string()
    .nullable()
    .pipe(z.string({ error: "Select a team" }).min(1, t("Select a team"))),
  guardrail_name: z.string().min(1, t("Enter a guardrail name")),
  mode: z.string().min(1, t("Select a mode")),
  api_base: z.string().min(1, t("Enter the API base URL")).refine(isValidUrl, t("Must be a valid URL")),
  extra_litellm_params: z.string().superRefine((value, ctx) => {
    if (!value) return;
    try {
      const parsed: unknown = JSON.parse(value);
      if (typeof parsed !== "object" || Array.isArray(parsed)) {
        ctx.addIssue({ code: "custom", message: t("Must be a JSON object") });
      }
    } catch {
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 text-xs text-muted-foreground">
            <span>
              {t("Model:")} <span className="font-medium text-foreground">{g.model}</span>
            </span>
            <span>
              {t("Submitted:")} <span className="font-medium text-foreground">{g.submittedAt}</span>
            </span>
          </div>
        </div>
        <div className="flex flex-col items-end gap-2 shrink-0">
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground whitespace-nowrap">{t("Forward API Key")}</span>
      ...
      <div className="mt-3 pt-3 border-t border-border">
        ...
        {isHeadersExpanded && (
          <div className="mt-2">
            {g.customHeaders.length === 0 ? (
              <p className="text-xs text-muted-foreground italic">{t("No static headers configured.")}</p>
            ) : (
    ...
    <div className="w-96 shrink-0 bg-card overflow-auto">
      <div className="p-5">
        ...
        <div className="space-y-4">
          ...
          <div className="border border-info/15 bg-info/10 rounded-lg p-3">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-semibold text-info">{t("Forward LiteLLM API Key")}</span>
              </div>
              <Toggle enabled={g.forwardKey} onToggle={onToggleForwardKey} disabled={!isAdmin} />
            </div>
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              <span className="text-xs font-semibold text-foreground">{t("Static headers")}</span>
              {g.customHeaders.length > 0 && (
            ...
            <p className="text-xs text-muted-foreground mb-2">{t("Sent with every request to the guardrail.")}</p>
            {g.customHeaders.length === 0 ? (
              <p className="text-xs text-muted-foreground italic mb-2">{t("No static headers configured.")}</p>
            ) : (
          ...
          <div>
            <div className="flex items-center gap-1.5 mb-2">
              <span className="text-xs font-semibold text-foreground">{t("Forward client headers")}</span>
            ...
            {g.extraHeaders.length === 0 ? (
              <p className="text-xs text-muted-foreground italic mb-2">{t("No forward client headers configured.")}</p>
            ) : (
              <ul className="list-none space-y-1 mb-2">
...
export function TeamGuardrailsTab({ accessToken }: TeamGuardrailsTabProps) {
  ...
  const handleSubmitGuardrail = submitForm.handleSubmit(async (values) => {
    ...
    try {
      ...
      toast.success(t("Guardrail submitted for review"));
  ...
  async function toggleForwardKey(id: string) {
    ...
    try {
      ...
      toast.success(newValue ? t("Forward API key enabled") : t("Forward API key disabled"));
    } catch {
      toast.fromError(t("Failed to update forward API key"));
    }
  }
  ...
  async function updateCustomHeaders(id: string, customHeaders: { key: string; value: string }[]) {
    ...
    try {
      ...
      toast.success(t("Static headers updated"));
    } catch {
      toast.fromError(t("Failed to update static headers"));
    }
  }
  ...
  async function updateExtraHeaders(id: string, extraHeaders: string[]) {
    ...
    try {
      ...
      toast.success(t("Forward client headers updated"));
    } catch {
      toast.fromError(t("Failed to update forward client headers"));
    }
  }
  ...
  async function handleApprove(id: string) {
    ...
    try {
      ...
      toast.success(t("Guardrail approved"));
    } catch {
      toast.fromError(t("Failed to approve guardrail"));
    }
  }
  ...
  async function handleReject(id: string) {
    ...
    try {
      ...
      if (selectedId === id) setSelectedId(null);
      await fetchSubmissions();
      toast.success(t("Guardrail rejected"));
    } catch {
      toast.fromError(t("Failed to reject guardrail"));
    }
  }
    ...
    <div className="flex h-full">
      <div className={`flex-1 min-w-0 p-6 overflow-auto ${selected ? "border-r border-border" : ""}`}>
        <div className="grid grid-cols-4 gap-4 mb-6">
          <StatCard label={t("Total Submitted")} value={totalCount} color="text-foreground" />
          <StatCard label={t("Pending Review")} value={pendingCount} color="text-warning" />
          <StatCard label={t("Active")} value={activeCount} color="text-success" />
          <StatCard label={t("Rejected")} value={rejectedCount} color="text-destructive" />
        </div>
      ...
      >
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t("Submit Guardrail for Review")}</DialogTitle>
          </DialogHeader>
          ...
          <TooltipProvider>
            <form onSubmit={handleSubmitGuardrail}>
              <FieldGroup>
                <FormField control={submitForm.control} name="team_id" label={t("Team")}>
                  {({ id, value, onChange }) => <TeamDropdown id={id} value={value} onChange={onChange} />}
                </FormField>
                <FormField control={submitForm.control} name="guardrail_name" label={t("Guardrail Name")}>
                  {({ ref, ...field }) => <Input {...field} ref={ref} placeholder={t("e.g. pii-detection")} />}
                </FormField>
                <FormField control={submitForm.control} name="mode" label={t("Mode")}>
                ...
                <FormField control={submitForm.control} name="api_base" label={t("API Base URL")}>
                  {({ ref, ...field }) => (
                    <Input
                      {...field}
                      ref={ref}
                ...
                <FormField control={submitForm.control} name="guardrail_info" label={t("Guardrail Info (optional)")}>
                  {({ ref, ...field }) => (
                    <Textarea
                      {...field}
                      ref={ref}
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSubmitGuardrail}>{t("Submit for Review")}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
