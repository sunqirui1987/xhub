...
const vectorStoreShape = {
  custom_llm_provider: z.string().min(1, t("Please select a provider")),
  vector_store_id: z.string().min(1, t("Please input the vector store ID from your api provider")),
  ...
  const handleCreate = async (formValues: VectorStoreFormValues) => {
    ...
    try {
      ...
      try {
        ...
        toast.fromError(t("Invalid JSON in metadata field"));
        return;
      }
      ...
      toast.success(t("Vector store created successfully"));
      form.reset(EMPTY_VALUES);
      setMetadataJson("{}");
      onSuccess();
    } catch (error) {
      console.error("Error creating vector store:", error);
      toast.fromError(t("Error creating vector store: ") + error);
    }
  };
  ...
  return (
    <Dialog open={isVisible} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[1000px]">
        <DialogHeader>
          <DialogTitle>{t("Add New Vector Store")}</DialogTitle>
        </DialogHeader>
        <TooltipProvider>
          <form onSubmit={form.handleSubmit(handleCreate)}>
            <FieldGroup>
              <FormField
                ...
                label={labelWithHint(t("Provider"), t("Select the provider for this vector store"))}
              >
              ...
              {selectedProvider === "pg_vector" && (
                <Alert variant="info">
                  <Info />
                  <AlertTitle>{t("PG Vector Setup Required")}</AlertTitle>
                  <AlertDescription>
              ...
              {selectedProvider === "valkey" && (
                <Alert variant="info">
                  <Info />
                  <AlertTitle>{t("Valkey Setup Required")}</AlertTitle>
                  <AlertDescription>
                    <p>
                      {t("LiteLLM searches documents you have already stored in Valkey. It does not create the index or upload documents for you. Before creating this vector store, make sure:")}
                    </p>
                    <ol style={{ marginLeft: "16px", marginTop: "8px", listStyleType: "decimal" }}>
                      <li>
                        {t("Your Valkey server has vector search enabled (the valkey-search module, included in the valkey-bundle image and in AWS ElastiCache / MemoryDB for Valkey)")}
                      </li>
                      <li>
                        {t("You have already created a search index and loaded your documents and their embeddings into it. Enter that index name as the Vector Store ID")}
                      </li>
                      <li>
                        {t("You know which embedding model created those stored embeddings. That model must be added to this proxy under Models so you can pick it below. Using a different model returns wrong results")}
                      </li>
                      <li>
                        {t("You know the field names your documents use for their text and their embedding. If they are not \"text\" and \"embedding\", set them below")}
                      </li>
                    </ol>
                    <p style={{ marginTop: "8px" }}>
                      {t("When a query comes in, LiteLLM converts it to an embedding with the model below and returns the closest matching documents from your index.")}
                    </p>
                  </AlertDescription>
                </Alert>
              )}
              ...
              {selectedProvider === "vertex_rag_engine" && (
                <Alert variant="info">
                  <Info />
                  <AlertTitle>{t("Vertex AI RAG Engine Setup")}</AlertTitle>
                  <AlertDescription>
                    ...
                    <ol style={{ marginLeft: "16px", marginTop: "8px", listStyleType: "decimal" }}>
                      ...
                      <li>
                        {t("Note the corpus ID from the Vertex AI console (now labeled \"RAG Engine\" in Google Cloud)")}
                      </li>
              ...
              {selectedProvider === "vertex_ai/search_api" && (
                <Alert variant="info">
                  <Info />
                  <AlertTitle>{t("Vertex AI Search Setup")}</AlertTitle>
                  <AlertDescription>
                    ...
                    <ol style={{ marginLeft: "16px", marginTop: "8px", listStyleType: "decimal" }}>
                      <li>
                        {t("Enable the Discovery Engine API on your Google Cloud project and create a data store following the guide:")}{" "}
                        <a
                        ...
                        >
                          {t("Create a Vertex AI Search data store")}
                        </a>
                      </li>
                      ...
                      <li>
                        {t("For website, healthcare, and connector-based sources (Drive, Gmail, Slack, Jira, etc.): create a search app on top of the data store, then copy the")} <strong>{t("Engine ID")}</strong> {t("and enter it in the Engine ID field. The Vector Store ID is still required as the LiteLLM-side name for this record, but it isn't used in the GCP URL when Engine ID is set.")}
                      </li>
                    </ol>
                  </AlertDescription>
                </Alert>
              ...
              <FormField
                ...
                label={labelWithHint(t("Vector Store ID"), t("Enter the vector store ID from your api provider"))}
              >
              ...
              <FormField
                ...
                label={labelWithHint(
                  t("Vector Store Name"),
                  t("Custom name you want to give to the vector store, this name will be rendered on the LiteLLM UI"),
                )}
              >
                {({ ref, value, ...field }) => <Input {...field} ref={ref} value={value ?? ""} />}
              </FormField>

              <FormField control={form.control} name="vector_store_description" label={t("Description")}>
              ...
              <FormField
                ...
                label={labelWithHint(
                  ...
                  t("Optionally select API provider credentials for this vector store eg. Bedrock API KEY"),
                )}
              >
                {({ id, value, onChange, "aria-invalid": ariaInvalid, "aria-describedby": ariaDescribedBy }) => (
                  <Combobox
                  ...
                  >
                    ...
                    />
                    <ComboboxContent>
                      <ComboboxEmpty>{t("No matching credentials")}</ComboboxEmpty>
                      <ComboboxList>
              ...
              <div role="group" className="flex w-full flex-col gap-3">
                <span className="flex w-fit gap-2 text-sm leading-snug font-medium">
                  {labelWithHint("Metadata", t("JSON metadata for the vector store (optional)"))}
                </span>
                <Textarea
                  rows={4}
                  value={metadataJson}
