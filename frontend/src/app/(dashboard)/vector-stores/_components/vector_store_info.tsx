...
const vectorStoreEditShape = {
  vector_store_id: z.string().min(1, t("Please input a vector store ID")),
...
const VectorStoreInfoView: React.FC<VectorStoreInfoViewProps> = ({
  ...
  const fetchVectorStoreDetails = async () => {
    ...
    try {
      ...
      toast.fromError(t("Error fetching vector store details: ") + error);
      setLoadFailed(true);
    }
  };
  ...
  const handleSave = async (values: VectorStoreEditValues) => {
    ...
    try {
      ...
      toast.success(t("Vector store updated successfully"));
      setIsEditing(false);
      fetchVectorStoreDetails();
    } catch (error) {
      console.error("Error updating vector store:", error);
      toast.fromError(t("Error updating vector store: ") + error);
    }
  };
  ...
  if (loadFailed) {
    return (
      <div className="p-4 max-w-full">
        ...
        <h1 className="text-xl font-semibold">{t("pages.vectorStores.notFound")}</h1>
        <p className="text-sm text-muted-foreground">
          {t("Vector store {vectorStoreId} could not be loaded. It may have been deleted.", { vectorStoreId })}</p>
      </div>
    );
  }
    ...
    <div className="p-4 max-w-full">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <h1 className="text-xl font-semibold">{t("Vector Store ID: {value0}", { value0: (vectorStoreDetails.vector_store_id) })}</h1>
          <p className="text-sm text-muted-foreground">
            {vectorStoreDetails.vector_store_description || t("No description")}
          </p>
        </div>
        {is_admin && !isEditing && <Button onClick={startEditing}>{t("Edit Vector Store")}</Button>}
      </div>
          ...
          {isEditing ? (
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium">{t("Edit Vector Store")}</h3>
              </div>
              <Card>
                <CardContent>
                  <TooltipProvider>
                    <form onSubmit={form.handleSubmit(handleSave)}>
                      <FieldGroup>
                        <FormField control={form.control} name="vector_store_id" label={t("Vector Store ID")}>
                          {({ ref, ...field }) => <Input {...field} ref={ref} disabled />}
                        </FormField>

                        <FormField control={form.control} name="vector_store_name" label={t("Vector Store Name")}>
                        ...
                        <FormField control={form.control} name="litellm_credential_name" label={t("Existing Credentials")}>
                          {({
                            id,
                            value,
                            onChange,
                        ...
                        <div role="group" className="flex w-full flex-col gap-3">
                          <span className="flex w-fit gap-2 text-sm leading-snug font-medium">
                            {labelWithHint("Metadata", t("JSON metadata for the vector store"))}
                          </span>
                          <Textarea
                            rows={4}
            ...
            <div>
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-medium">{t("Vector Store Details")}</h3>
                {is_admin && <Button onClick={startEditing}>{t("Edit Vector Store")}</Button>}
              </div>
              <Card>
                <CardContent>
                  <div className="space-y-4">
                    ...
                    <div>
                      <p className="font-medium">{t("Metadata")}</p>
                    ...
                    <div>
                      <p className="font-medium">{t("Last Updated")}</p>
                      <p>
