...
const VectorStoreManagement: React.FC<VectorStoreProps> = ({ accessToken, userID, userRole, isViewOnly }) => {
  ...
  const fetchVectorStores = async () => {
    ...
    try {
      ...
      toast.fromError(t("Error fetching vector stores: ") + error);
    } finally {
      setIsLoadingVectorStores(false);
    }
  };
  ...
  const fetchCredentials = async () => {
    ...
    try {
      ...
      toast.fromError(t("Error fetching credentials: ") + error);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.success(t("Vector store deleted successfully"));
      fetchVectorStores();
    } catch (error) {
      console.error("Error deleting vector store:", error);
      toast.fromError(t("Error deleting vector store: ") + error);
    } finally {
      setIsDeleting(false);
    ...
    <div className="mx-4">
      <div className="gap-2 p-8 w-full mt-2">
        <div className="flex justify-between mt-2 w-full items-center mb-4">
          <h1 className="text-xl font-semibold tracking-tight text-foreground">{t("pages.vectorStores.title")}</h1>
          <div className="flex items-center space-x-2">
            {lastRefreshed && <p className="text-sm text-muted-foreground">{t("Last Refreshed: {lastRefreshed}", { lastRefreshed })}</p>}
        ...
        <Tabs defaultValue={defaultTab} onValueChange={onTabChange}>
          ...
          {canCreateVectorStores && (
            <TabsContent keepMounted={hasVisited(t("create"))} value="create">
          ...
          <TabsContent keepMounted={hasVisited(t("test"))} value="test">
