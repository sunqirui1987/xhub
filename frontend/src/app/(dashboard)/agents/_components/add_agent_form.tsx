...
const stepTitles = () =>
  [t("Configure"), t("Entitlements"), t("Governance"), t("Agent Management"), t("Ready")] as const;
...
const StepProgress: React.FC<{ current: number }> = ({ current }) => (
  <ol aria-label={t("Agent creation steps")} className="mb-8 flex items-center">
...
const AddAgentForm: React.FC<AddAgentFormProps> = ({ visible, onClose, accessToken, onSuccess, teams }) => {
  ...
  const handleCreateAgent = async () => {
    ...
    if (keyAssignOption === "existing_key" && !selectedExistingKey) {
      toast.error(t("Please select an existing key to assign"));
      return;
    }
    ...
    try {
      ...
      if (!agentData) {
        toast.error(t("Failed to build agent data"));
        setIsSubmitting(false);
        return;
      }
      ...
      toast.error(errorMessage ? t("Failed to create agent: {errorMessage}", { errorMessage }) : t("Failed to create agent"));
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const renderEntitlementsStep = () => (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        {t("Configure which models, agents, and MCP tools this agent is allowed to use. Leave fields empty to allow all (subject to key/team permissions).")}
      </p>
      ...
      <FieldGroup>
        <AgentFormField
          ...
          label={labelWithHint(
            t("Allowed Models"),
            t("Restrict which models this agent can call. Leave empty to allow all."),
          )}
        >
          {({ id, value, onChange }) => (
            <AgentTagsInput
        ...
        <AgentFormField
          ...
          label={labelWithHint(
            t("Allowed Agents (Sub-Agents)"),
            t("Restrict which other agents this agent can invoke as sub-agents. Leave empty to allow all."),
          )}
        >
          {({ id, value, onChange }) => (
            <AgentMultiSelect
        ...
        <AgentFormField
          ...
          label={labelWithHint(
            ...
            t("Select which MCP servers or access groups this agent can access"),
          )}
        >
          {({ value, onChange }) => (
            <MCPServerSelector
  ...
  const renderObservabilityStep = () => (
    <div className="space-y-6">
      <div>
        <h4 className="mb-3 text-sm font-medium text-foreground">{t("Tracing")}</h4>
        <div className="space-y-4">
      ...
      <div>
        <h4 className="mb-3 text-sm font-medium text-foreground">{t("Budgets & Rate Limits")}</h4>
        <div className="space-y-4">
          {!requireTraceIdOutbound && (
            <div className="rounded-lg border border-warning/20 bg-warning/10 p-3 text-sm text-warning">
              {t("Enable \"Require x-litellm-trace-id on calls BY this agent\" in Tracing to configure budgets and rate limits.")}
            </div>
          )}

          <div className="text-sm font-medium text-foreground">{t("Session Budgets")}</div>
          <div className="grid grid-cols-2 gap-4">
            <Field className="gap-1">
              <FieldLabel htmlFor="agent-max-iterations">{t("Max Iterations")}</FieldLabel>
              <Input
              ...
              />
              <p className="mt-1 text-xs text-muted-foreground">{t("Hard cap on LLM calls per session")}</p>
            </Field>
            <Field className="gap-1">
              <FieldLabel htmlFor="agent-max-budget-per-session">{t("Max Budget Per Session ($)")}</FieldLabel>
              <Input
              ...
              />
              <p className="mt-1 text-xs text-muted-foreground">{t("Max spend per trace before returning 429")}</p>
            </Field>
          </div>

          <Separator className="my-2" />

          <div className="text-sm font-medium text-foreground">{t("Agent Rate Limits")}</div>
          <p className="text-xs text-muted-foreground">{t("Global rate limits applied across all callers of this agent.")}</p>
          <div className="grid grid-cols-2 gap-4">
            {rateLimitField("tpm_limit", t("TPM Limit"), "e.g. 100000")}
            {rateLimitField("rpm_limit", t("RPM Limit"), "e.g. 100")}
          </div>

          <div className="mt-4 text-sm font-medium text-foreground">{t("Per-Session Rate Limits")}</div>
          <p className="text-xs text-muted-foreground">
            {t("Rate limits per session (x-litellm-trace-id). Each session gets its own counters.")}
          </p>
          <div className="grid grid-cols-2 gap-4">
            {rateLimitField("session_tpm_limit", t("Session TPM Limit"), "e.g. 10000")}
            {rateLimitField("session_rpm_limit", t("Session RPM Limit"), "e.g. 20")}
          </div>
        </div>
      </div>
      ...
      <div>
        <h4 className="mb-3 text-sm font-medium text-foreground">{t("Guardrails")}</h4>
    ...
    <>
      <Field className="gap-1">
        <FieldLabel htmlFor="agent-type">
          {labelWithHint(t("Agent Type"), t("Select the type of agent you want to create"))}
        </FieldLabel>
          ...
          <SelectContent className="p-1">
            ...
            <SelectItem value={CUSTOM_AGENT_TYPE} className="focus:bg-warning/10">
              <span className="flex items-center gap-3">
                ...
                <span className="block">
                  <span className="flex items-center gap-2">
                    <span className="font-medium text-warning">{t("Custom / Other")}</span>
      ...
      <div className="mt-4">
        {agentType === CUSTOM_AGENT_TYPE ? (
          <FieldGroup>
            <AgentFormField name="agent_name" label={t("Agent Name")} rules={{ required: t("Please enter an agent name") }}>
              {({ value, onChange, ref, ...control }) => (
                <Input
                  {...control}
                  ref={ref}
                  placeholder={t("e.g. my-custom-agent")}
                  value={typeof value === "string" ? value : ""}
                  onChange={onChange}
                />
              )}
            </AgentFormField>
            <AgentFormField name="description" label={t("Description")}>
              {({ value, onChange, ref, ...control }) => (
                <Textarea
                  {...control}
                  ref={ref}
                ...
                <FieldGroup>
                  {selectedAgentTypeInfo.credential_fields.map((field) => (
                    <AgentFormField
                      ...
                      rules={field.required ? { required: t("Please enter {value0}", { value0: (field.label) }) } : undefined}
                    >
                      {({ value, onChange, ref, ...control }) =>
    ...
    return (
      <div>
        ...
        <AgentFormField
          ...
          label={labelWithHint(
            t("Assign to Team"),
            t("Optionally assign this agent to a team. The agent and its key will belong to the selected team."),
          )}
        >
          {({ value, onChange }) => (
        ...
        >
          ...
          >
            <div className="flex items-start justify-between">
              <div className="flex flex-1 items-start gap-3">
                <RadioGroupItem value="create_new" aria-label={t("Create a new key for this agent")} />
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <Key className="size-4 text-info" />
                    <span className="font-medium text-foreground">{t("Create a new key for this agent")}</span>
                  </div>
                  <p className="mt-1 text-sm text-muted-foreground">{t("A dedicated key scoped to this agent.")}</p>
                  {keyAssignOption === "create_new" && (
                    <div className="mt-3 space-y-3" onClick={(e) => e.stopPropagation()}>
                      <Field className="gap-1">
                        <FieldLabel htmlFor="agent-new-key-name">{t("Key Name")}</FieldLabel>
                        <Input
              ...
              <StatusBadge tone="success" label={t("Recommended")} />
            </div>
          </div>
          ...
          >
            <div className="flex items-start gap-3">
              <RadioGroupItem value="existing_key" aria-label={t("Assign an existing key")} />
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <Key className="size-4 text-muted-foreground" />
                  <span className="font-medium text-foreground">{t("Assign an existing key")}</span>
                </div>
                <p className="mt-1 text-sm text-muted-foreground">{t("Re-assign a key you already have to this agent.")}</p>
                {keyAssignOption === "existing_key" && (
                  <div className="mt-3" onClick={(e) => e.stopPropagation()}>
                    <SearchSelect
                      inputId="agent-existing-key"
                      placeholder={loadingKeys ? t("Loading keys…") : t("Search by key name…")}
  ...
  const renderReadyStep = () => (
    <div className="py-6 text-center">
      ...
      <h3 className="mb-2 text-xl font-semibold text-foreground">{t("Agent Created!")}</h3>
      ...
      {assignedKeyAlias && (
        <p className="mt-2 text-sm text-muted-foreground">
          {t("Key")} <span className="font-medium">{assignedKeyAlias}</span> {t("has been assigned to this agent.")}
        </p>
      )}
      {!createdKeyValue && !assignedKeyAlias && keyAssignOption === "skip" && (
        <p className="mt-2 text-sm text-muted-foreground">
          {t("No key assigned. You can create one from the Virtual Keys page.")}
        </p>
      )}
    </div>
  );
        ...
        <DialogHeader>
          <div className="flex items-center space-x-3 border-b border-border pb-4">
            {selectedLogo && currentStep < 1 && (
              <Logo src={selectedLogo} label={t("Agent")} className="h-6 w-6 object-contain" />
            )}
            <DialogTitle className="text-xl font-semibold text-foreground">{t("Add New Agent")}</DialogTitle>
          </div>
        </DialogHeader>
        <TooltipProvider>
          <div className="mt-4">
            ...
            <div className="mt-6 flex items-center justify-between border-t border-border pt-6">
              ...
              <div className="flex gap-3">
                ...
                {currentStep < 3 && <Button onClick={handleNext}>{t("Next →")}</Button>}
                {currentStep === 3 && (
                ...
                {currentStep === 4 && <Button onClick={handleClose}>{t("Done")}</Button>}
              </div>
            </div>
          </div>
        </TooltipProvider>
