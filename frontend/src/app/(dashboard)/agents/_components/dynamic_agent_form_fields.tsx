...
const CredentialField = ({ field }: { field: AgentCredentialFieldMetadata }) => {
  ...
  return (
    <AgentFormField
      ...
      rules={{
        ...(field.required ? { required: t("Please enter {value0}", { value0: (field.label) }) } : {}),
...
const DynamicAgentFormFields: React.FC<DynamicAgentFormFieldsProps> = ({ agentTypeInfo, panels }) => (
  <>
    <FieldGroup className="mb-4">
      ...
      >
        {({ value, onChange, ref, ...control }) => (
          <Input
            {...control}
            ref={ref}
            placeholder={t("e.g., my-langgraph-agent")}
            value={typeof value === "string" ? value : ""}
            onChange={onChange}
          />
        )}
      ...
      <AgentFormField
        name="description"
        label={labelWithHint(t("Description"), t("Brief description of what this agent does"))}
      >
        {({ value, onChange, ref, ...control }) => (
          <Textarea
            {...control}
