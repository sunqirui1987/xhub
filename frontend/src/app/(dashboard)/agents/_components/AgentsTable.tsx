...
function EmptyState({ isFiltered }: { isFiltered: boolean }) {
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{isFiltered ? t("No matching agents") : t("No agents yet")}</div>
      ...
      toolbar={() => (
        <div className="flex flex-wrap items-center justify-between gap-2">
          <InputGroup className="max-w-sm">
            ...
            <InputGroupInput
              placeholder={t("Search agents by name, ID, or description...")}
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            {searchTerm && (
              <InputGroupAddon align="inline-end">
                <InputGroupButton size="icon-xs" aria-label={t("Clear search")} onClick={() => setSearchTerm("")}>
                  <X />
                </InputGroupButton>
              </InputGroupAddon>
            )}
