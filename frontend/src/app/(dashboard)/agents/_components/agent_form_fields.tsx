    ...
    <>
      {fields.map((item, index) => (
        <div key={item.id} className="flex items-start gap-2">
          <AgentFormField name={`static_headers.${index}.header`} rules={{ required: t("Header name required") }}>
            {({ value, onChange, ref, ...control }) => (
              <Input
                {...control}
                ref={ref}
          ...
          <AgentFormField name={`static_headers.${index}.value`} rules={{ required: t("Value required") }}>
            {({ value, onChange, ref, ...control }) => (
              <Input
                {...control}
                ref={ref}
                className="w-65"
                placeholder={t("Value (e.g. Bearer token123)")}
                value={typeof value === "string" ? value : ""}
                onChange={onChange}
              />
            )}
          ...
          <Button
            ...
            variant="ghost"
            size="icon"
            aria-label={t("Remove static header")}
    ...
    <>
      {showAgentName && (
        <FieldGroup className="mb-4">
          <AgentFormField
            name="agent_name"
            label={labelWithHint(t("Agent Name"), t("Unique identifier for the agent"))}
            rules={{ required: t("Please enter a unique agent name") }}
          >
            {({ value, onChange, ref, ...control }) => (
              <Input
                {...control}
      ...
      <div className="mb-4 rounded-md border border-border px-4">
        {shouldShow(AGENT_FORM_CONFIG.basic.key) && (
          <AgentFormPanel
            ...
            title={t("{value0} (Required)", { value0: (AGENT_FORM_CONFIG.basic.title) })}
            panels={panels}
          >
            {AGENT_FORM_CONFIG.basic.fields.map((field) => (
              <AgentFormField
                ...
                rules={field.required ? { required: t("Please enter {value0}", { value0: (field.label.toLowerCase()) }) } : undefined}
              >
                {({ value, onChange, ref, ...control }) => {
        ...
        {shouldShow(AUTH_HEADERS_PANEL_KEY) && (
          <AgentFormPanel panelKey={AUTH_HEADERS_PANEL_KEY} title={t("Authentication Headers")} panels={panels}>
            <Field>
              <FieldTitle>
                {labelWithHint(
                  t("Static Headers"),
                  t("Headers always sent to the backend agent, regardless of the client request. Admin-configured, static wins on conflict."),
                )}
              </FieldTitle>
            ...
            <AgentFormField
              ...
              label={labelWithHint(
                ...
                t("Header names to extract from the client's request and forward to the agent. Type a name and press Enter."),
              )}
            >
              {({ id, value, onChange }) => (
                <AgentTagsInput
