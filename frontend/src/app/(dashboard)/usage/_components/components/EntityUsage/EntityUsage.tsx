...
const EntityUsage: React.FC<EntityUsageProps> = ({
  ...
  const costPanel = (
    <div className="grid grid-cols-2 gap-2 w-full">
      <div className="col-span-2">
        <ShadcnCard>
          <CardContent>
            <h3 className="text-lg font-medium text-foreground">{t("{capitalizedEntityLabel} Spend Overview", { capitalizedEntityLabel })}</h3>
          ...
          <CardContent>
            <BarChart
              data={[...spendData.results]
                ...
                .map((row) => ({
                  ...row,
                  [t("Request cost")]: row.metrics.spend ?? 0,
                  [t("Flat cost")]: row.metrics.flat_cost ?? 0,
                }))}
              index="date"
              categories={showFlatCost ? [t("Request cost"), t("Flat cost")] : ["metrics.spend"]}
              ...
              customTooltip={({ payload, active }) => {
                ...
                return (
                  <div className="bg-card p-4 shadow-lg rounded-lg border">
                    ...
                    {showFlatCost ? (
                      <>
                        <p className="text-info">{t("Request cost: ${value0}", { value0: (formatNumberWithCommas(requestSpend, 2)) })}</p>
                        <p className="text-violet-500">{t("Flat cost: ${value0}", { value0: (formatNumberWithCommas(flatCost, 2)) })}</p>
                        <p className="font-semibold">
                          {t("Total cost: ${value0}", { value0: (formatNumberWithCommas(requestSpend + flatCost, 2)) })}
                        </p>
                      </>
                    ) : (
                      <p className="text-info">{t("Total Spend: ${value0}", { value0: (formatNumberWithCommas(data.metrics.spend, 2)) })}</p>
                    )}
                    <p className="text-muted-foreground">{t("Total Requests: {value0}", { value0: (data.metrics.api_requests) })}</p>
                    ...
                    <p className="text-muted-foreground">{t("Total Tokens: {value0}", { value0: (data.metrics.total_tokens) })}</p>
                    <p className="text-muted-foreground">
                      {t("Total {capitalizedEntityLabel}s: {entityCount}", { capitalizedEntityLabel, entityCount })}
                    </p>
                    <div className="mt-2 border-t pt-2">
                      <p className="font-semibold">{t("Spend by {capitalizedEntityLabel}:", { capitalizedEntityLabel })}</p>
                      ...
                      {entityCount > 5 && (
                        <p className="text-sm text-muted-foreground italic">{t("...and {value0} more", { value0: (entityCount - 5) })}</p>
                      )}
                    </div>
                  </div>
                );
        ...
        <ShadcnCard>
          <CardContent className="flex flex-col space-y-4">
            <div className="flex flex-col space-y-2">
              <h3 className="text-lg font-medium text-foreground">{t("Spend Per {capitalizedEntityLabel}", { capitalizedEntityLabel })}</h3>
              <p className="text-xs text-muted-foreground">{t("Showing Top 5 by Spend")}</p>
              <div className="flex items-center text-sm text-muted-foreground">
                <span>{t("Get Started by Tracking cost per {capitalizedEntityLabel}", { capitalizedEntityLabel })} </span>
  ...
  const tabs: readonly { key: string; label: string; content: ReactNode }[] = [
    { key: "cost", label: t("pages.usage.cost"), content: costPanel },
    {
      key: "models",
      label: entityType === "agent" ? t("pages.usage.requestTokenConsumption") : t("pages.usage.modelActivity"),
      content: (
        <>
