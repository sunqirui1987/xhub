  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-xs text-muted-foreground">
        {isObject ? t("Provide a valid JSON object.") : t("Provide a valid JSON array.")}
      </p>
    </div>
  );
};
...
const ToolArgumentControl: React.FC<{
    ...
    return (
      ...
      >
        <option value={-1} disabled={field.required}>
          {t("Select {value0}", { value0: (field.key) })}
        </option>
        {prop.enum.map((option, index) => (
          <option key={option} value={index}>
            {option === "" ? t("Empty string") : option}
          </option>
        ))}
      </select>
    );
  ...
  if (prop.type === "number" || prop.type === "integer") {
    return (
      <Input
        ...
        placeholder={prop.description || t("Enter {value0}", { value0: (field.key) })}
  ...
  if (prop.type === "boolean") {
    return (
      <Select
        items={field.required ? BOOLEAN_ITEMS : [{ value: null, label: t("Select {value0}", { value0: (field.key) }) }, ...BOOLEAN_ITEMS]}
      ...
      >
        <SelectTrigger
          ...
          className="w-full"
        >
          <SelectValue placeholder={t("Select {value0}", { value0: (field.key) })} />
        </SelectTrigger>
        <SelectContent>
          {!field.required && <SelectItem value={null}>{t("Select {value0}", { value0: (field.key) })}</SelectItem>}
    ...
    return (
      <FieldGroup>
        <FormField
          ...
          label={
            <span>
              {t("Input")} <span className="text-destructive">*</span>
            </span>
          }
        >
          {(field) => (
    ...
    return (
      <div className="rounded-lg border border-border bg-muted py-6 text-center">
        <div className="mx-auto max-w-sm">
          <h4 className="mb-1 text-sm font-medium text-foreground">{t("No Parameters Required")}</h4>
          <p className="text-xs text-muted-foreground">{t("This tool can be called without any input parameters.")}</p>
        </div>
      </div>
    );
  }
