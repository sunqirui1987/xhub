...
const toolsetSchema = z.object({
  toolset_name: z.string().min(1, t("Please enter a toolset name")),
      ...
      {expanded && (
        <div className="p-2">
          {loading ? (
            ...
            <p className="text-xs text-muted-foreground px-2 py-2">{t("No tools found for this server.")}</p>
          ) : (
  ...
  return (
    <Dialog open={open} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[960px]">
        <DialogHeader>
          <DialogTitle>{initialToolset ? t("Edit Toolset") : t("New Toolset")}</DialogTitle>
        </DialogHeader>
        <form onSubmit={(event) => event.preventDefault()} className="mt-2">
          <FieldGroup className="mb-4 flex-row gap-4">
            <FormField control={form.control} name="toolset_name" label={t("Toolset Name")} className="flex-1">
              {(field) => <Input {...field} placeholder={t("e.g. github-linear-tools")} />}
            </FormField>
            <FormField control={form.control} name="description" label={t("Description")} className="flex-1">
              {(field) => <Input {...field} placeholder={t("Optional description")} />}
            </FormField>
          </FieldGroup>
        </form>
        ...
        <div className="flex gap-4 mt-2" style={{ minHeight: 360 }}>
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm font-semibold text-foreground">{t("Available Tools")}</p>
            </div>
            <InputGroup className="mb-2">
              ...
              {serverSearch && (
                <InputGroupAddon align="inline-end">
                  <InputGroupButton size="icon-xs" aria-label={t("Clear search")} onClick={() => setServerSearch("")}>
                    <X />
                  </InputGroupButton>
                </InputGroupAddon>
              )}
          ...
          <div className="w-72 shrink-0">
            ...
            <div className="space-y-1 overflow-y-auto" style={{ maxHeight: 340 }}>
              {selectedTools.length === 0 ? (
                <p className="text-muted-foreground text-sm">{t("No tools added yet")}</p>
              ) : (
...
function ToolsetUsageGuide() {
  ...
  return (
    <div className="mb-6 rounded-lg border border-border bg-muted px-5 py-4">
      <p className="text-sm font-medium text-foreground mb-1">{t("How toolsets work")}</p>
      <p className="text-sm text-muted-foreground mb-3">
        {t("Create a toolset, assign it to a key via")}{" "}
        <span className="font-medium text-foreground">{t("API Keys → Edit Key → MCP Servers")}</span>{t(", then point your MCP client at the toolset URL. The client only sees the tools you picked.")}
      </p>
      <div className="text-xs text-muted-foreground mb-1">{t("Claude Code / Cursor config")}</div>
...
export function MCPToolsetsTab({ accessToken, userRole }: MCPToolsetsTabProps) {
  ...
  const handleCreate = async (name: string, description: string | undefined, tools: MCPToolsetTool[]) => {
    ...
    toast.success(t("Toolset created"));
  ...
  const handleUpdate = async (name: string, description: string | undefined, tools: MCPToolsetTool[]) => {
    ...
    toast.success(t("Toolset updated"));
    queryClient.invalidateQueries({ queryKey: ["mcpToolsets"] });
    setEditToolset(null);
  };
  ...
  return (
    <div className="mt-4">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-medium text-foreground">{t("MCP Toolsets")}</h3>
          <p className="text-muted-foreground text-sm">
      ...
      <Dialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t("Delete Toolset")}</DialogTitle>
          </DialogHeader>
          <p>
