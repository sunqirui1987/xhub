    ...
    <div className="mb-5 border border-border rounded-lg bg-card overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-foreground">{t("Submission Rules")}</span>
          {activeLabels.length > 0 ? (
            <span className="text-xs text-muted-foreground">
              ({activeLabels.length} required field{activeLabels.length !== 1 ? "s" : ""})
            </span>
          ) : (
            <span className="text-xs text-muted-foreground italic">{t("no rules set")}</span>
          )}
        </div>
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 mt-1.5 text-xs text-muted-foreground">
              <span>
                {t("Transport:")} <span className="text-muted-foreground">{server.transport ?? t("sse")}</span>
              </span>
              <span>·</span>
              <span>
                {t("Submitted by:")} <span className="text-muted-foreground">{server.submitted_by ?? "—"}</span>
              </span>
              <span>·</span>
              <span>{formatDate(server.submitted_at)}</span>
            </div>
            {approvalStatus === "rejected" && server.review_notes && (
              <p className="text-xs text-destructive mt-1.5">{t("Rejection reason: {value0}", { value0: (server.review_notes) })}</p>
            )}
          </div>
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className={`text-sm font-semibold leading-tight ${allPassed ? "text-success" : "text-destructive"}`}>
                {allPassed ? t("All checks passed") : t("{failCount} check{value1} failed", { failCount, value1: (failCount !== 1 ? "s" : "") })}
              </div>
              <div className="text-xs text-muted-foreground mt-0.5">
                {t("{passCount} passing, {failCount} failing", { passCount, failCount })}</div>
            </div>
...
export function MCPSubmissionsTab({ accessToken }: MCPSubmissionsTabProps) {
  ...
  const handleSaveRules = async () => {
    ...
    try {
      ...
      toast.success(t("Submission rules saved"));
    } catch {
      toast.fromError(t("Failed to save submission rules"));
    } finally {
  ...
  async function handleApprove(serverId: string, serverName: string) {
    ...
    try {
      ...
      await fetchData();
      toast.success(t("MCP server \"{serverName}\" approved", { serverName }));
    } catch {
      toast.fromError(t("Failed to approve MCP server"));
    } finally {
  ...
  async function handleReject(serverId: string, serverName: string, reviewNotes?: string) {
    ...
    try {
      ...
      await fetchData();
      toast.success(t("MCP server \"{serverName}\" rejected", { serverName }));
    } catch {
      toast.fromError(t("Failed to reject MCP server"));
    } finally {
      setConfirmAction(null);
    }
  }
  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <StatCard label={t("Total Submitted")} value={summary.total} color="text-foreground" />
        <StatCard label={t("Pending Review")} value={summary.pending_review} color="text-warning" />
        <StatCard label={t("Active")} value={summary.active} color="text-success" />
        <StatCard label={t("Rejected")} value={summary.rejected} color="text-destructive" />
      </div>
      ...
      <div className="space-y-3">
        ...
        {!isLoading && !error && filtered.length === 0 && (
          <div className="text-center py-12 text-muted-foreground text-sm">
            {t("No MCP server submissions match your filters.")}
          </div>
        )}
        {!isLoading &&
          !error &&
