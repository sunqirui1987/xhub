  ...
  const {
    ...
    onTokenReceived: (token) => {
      ...
      if (isClientForwardedTokenMode(getEffectiveAuthType())) {
        ...
        toast.success(
          t("Token held for this browser session. Tools can now be loaded and configured; the token is not saved to LiteLLM."),
        );
        return;
      }
      ...
      toast.success(t("OAuth authorization successful! Please click 'Update MCP Server' to save the credentials."));
    },
    ...
    try {
      ...
      if (oauthTokenResponse?.access_token) {
        ...
        try {
          ...
          toast.fromError(t("MCP Server updated, but failed to persist OAuth token") + (message ? `: ${message}` : ""));
          return;
        }
      }
      ...
      toast.fromError(t("Failed to update MCP Server") + (error?.message ? `: ${error.message}` : ""));
    }
  };
          ...
          <FormProvider {...form}>
            <MountedFormProvider value={{ control: form.control, registry }}>
              ...
              >
                ...
                <MountedFormField label={t("Description")} name="description">
                  {(control) => (
                    <Input
                      {...textControl(control)}
                      className="rounded-lg border-border focus:border-info focus:ring-ring"
                ...
                <MountedFormField
                  ...
                  name="transport"
                  required
                  rules={{ validate: { required: requiredRule(t("Transport Type is required")) } }}
                >
                  {(control) => (
                    <Select
                ...
                {isMCPTransport && (
                  <MountedFormField
                    ...
                    rules={{
                      validate: {
                        required: requiredRule(t("Please enter a server URL")),
                  ...
                  <MountedFormField
                    label={
                      <span className="text-sm font-medium text-foreground flex items-center">
                        {t("OpenAPI Spec URL")}
                        <SimpleTooltip content={t("URL to an OpenAPI specification (JSON or YAML). MCP tools will be automatically generated from the API endpoints defined in the spec.")}>
                    ...
                    name="spec_path"
                    required
                    rules={{ validate: { required: requiredRule(t("Please enter an OpenAPI spec URL")) } }}
                  >
                    {(control) => (
                      <Input
                        {...textControl(control)}
                ...
                <MountedFormField
                  label={
                    <span className="text-sm font-medium text-foreground flex items-center">
                      ...
                      <SimpleTooltip content={t("Maximum number of tool calls LiteLLM will run against this server at the same time. Additional calls wait for a free slot. Leave blank for no limit.")}>
                        <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                      </SimpleTooltip>
                    </span>
                  }
                ...
                {!isStdioTransport && (
                  <>
                    <MountedFormField
                      ...
                      name="auth_type"
                      required
                      rules={{ validate: { required: requiredRule(t("Authentication is required")) } }}
                    >
                      {(control) => (
                ...
                {isStdioTransport && (
                  <div className="rounded-lg border border-border p-4 space-y-4">
                    <p className="text-sm text-muted-foreground">
                      {t("Configure the stdio transport used to launch the MCP server process. You can either fill in the fields below or paste a JSON configuration.")}
                    </p>
                    ...
                    <MountedFormField
                      ...
                      name="command"
                      required
                      rules={{ validate: { required: requiredRule(t("Please enter a command for stdio transport")) } }}
                    >
                      {(control) => (
                        <Input
                          {...textControl(control)}
                    ...
                    <MountedFormField label={t("Args")} name="args">
                      {(control) => (
                        <MultiSelect
                          {...tagsControl(control)}
                    ...
                    <MountedFormField
                      ...
                      rules={{
                        validate: {
                          jsonObject: parsesAsJsonObject(t("Please enter valid JSON"), t("Env must be a JSON object")),
                        },
                      }}
                    >
                      {(control) => (
                  ...
                  <MountedFormField
                    label={
                      <span className="text-sm font-medium text-foreground flex items-center">
                        ...
                        <SimpleTooltip content={t("Token, password, or header value to send with each request for the selected auth type.")}>
                          <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                        </SimpleTooltip>
                      </span>
                    }
                    name={["credentials", "auth_value"]}
                    rules={{ validate: { notWhitespace: notOnlyWhitespace(t("Authentication value cannot be empty")) } }}
                  >
                    {(control) => (
                      <PasswordInput
                        {...textControl(control)}
                  ...
                  <>
                    {!oauthFlowTypeValue && !isDelegateAuth && (
                      <Alert variant="warning" className="mb-4 rounded-lg">
                        <TriangleAlert />
                        <AlertTitle>{t("This server has no OAuth flow set")}</AlertTitle>
                        <AlertDescription>
                          {t("Choose Machine-to-Machine (M2M) or Interactive (PKCE) so LiteLLM authenticates it the way you intend, then save. Until it is set, LiteLLM falls back to interactive per-user auth and treats a machine-to-machine credential shape conservatively.")}
                        </AlertDescription>
                      </Alert>
                    )}
                    <OAuthFormFields
                  ...
                  <>
                    ...
                    <MountedFormField
                      label={
                        <span className="text-sm font-medium text-foreground flex items-center">
                          {t("AWS Region")}
                          <SimpleTooltip content={t("AWS region for SigV4 signing (e.g., us-east-1)")}>
                            <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                          </SimpleTooltip>
                        </span>
                      }
                    ...
                    <MountedFormField
                      label={
                        <span className="text-sm font-medium text-foreground flex items-center">
                          {t("AWS Service Name")}
                          <SimpleTooltip content={t("AWS service name for SigV4 signing. Defaults to 'bedrock-agentcore'.")}>
                            <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                          </SimpleTooltip>
                        </span>
                      }
                    ...
                    <MountedFormField
                      label={
                        <span className="text-sm font-medium text-foreground flex items-center">
                          {t("AWS Access Key ID")}
                          <SimpleTooltip content={t("Optional. If not provided, falls back to the boto3 credential chain (IAM role, env vars, etc.).")}>
                            <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                          </SimpleTooltip>
                        </span>
                      }
                    ...
                    <MountedFormField
                      label={
                        <span className="text-sm font-medium text-foreground flex items-center">
                          ...
                          <SimpleTooltip content={t("Optional. Required if AWS Access Key ID is provided.")}>
                            <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                          </SimpleTooltip>
                        </span>
                      }
                    ...
                    <MountedFormField
                      label={
                        <span className="text-sm font-medium text-foreground flex items-center">
                          {t("AWS Session Token")}
                          <SimpleTooltip content={t("Optional. Only needed for temporary STS credentials.")}>
                            <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                          </SimpleTooltip>
                        </span>
                      }
                    ...
                    <MountedFormField
                      label={
                        <span className="text-sm font-medium text-foreground flex items-center">
                          {t("AWS Role ARN")}
                          <SimpleTooltip content={t("Optional. IAM role ARN to assume via STS before signing. If set, LiteLLM calls sts:AssumeRole to get temporary credentials.")}>
                            <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                          </SimpleTooltip>
                        </span>
                      }
                    ...
                    <MountedFormField
                      label={
                        <span className="text-sm font-medium text-foreground flex items-center">
                          {t("AWS Session Name")}
                          <SimpleTooltip content={t("Optional. Session name for the AssumeRole call — appears in CloudTrail logs. Auto-generated if omitted.")}>
                            <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                          </SimpleTooltip>
                        </span>
                      }
        ...
        <TabsContent value="cost" keepMounted>
          <div className="space-y-6">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button onClick={() => void submitForm()}>{t("Save Changes")}</Button>
            </div>
          </div>
        </TabsContent>
      </div>
