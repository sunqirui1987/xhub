  ...
  return (
    <div className="rounded-lg border border-border bg-muted p-4">
      <div className="flex items-center gap-2 mb-1">
        <strong className="text-sm font-semibold">{t("Variables")}</strong>
        <SimpleTooltip
          content={
            <>
              {t("Define variables you can interpolate in Static Headers or Authentication using")}{" "}
              <code>{"${VAR_NAME}"}</code>. <br />
              <b>{t("Instance")}</b>{t(": admin-defined value used for every user.")}
              <br />
              <b>{t("Per-user")}</b>{t(": each user supplies their own value (e.g. personal credentials) via the MCP Gateway dashboard.")}
            </>
          }
        >
      ...
      <span className="mb-3 block text-xs text-muted-foreground">
        {t("Reference these in Static Headers or Authentication as")} <code>{"${VAR_NAME}"}</code>{t(". For example:")}{" "}
        ...
        {fields.map((item, index) => (
          <div key={item.id} className="flex gap-3 items-start">
            <MountedFormField
              ...
              rules={{
                validate: {
                  required: requiredRule(t("Variable name is required")),
                  pattern: matchesPattern(
                    VARIABLE_NAME_PATTERN,
                    t("Use letters, digits, underscores; cannot start with a digit."),
                  ),
                },
              }}
            >
    ...
    return (
      <MountedFormField name={["env_vars", String(index), "description"]} className="mb-0">
        {(control) => (
          <InputGroup>
            <InputGroupAddon>
              <SimpleTooltip content={t("Per-user variables have no shared value. This text is only a hint shown to each user when they fill in their own value.")}>
