        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2">
              ...
              <Badge variant={isEnabled ? "secondary" : "outline"}>{isEnabled ? t("Enabled") : t("Disabled")}</Badge>
            ...
            <p className="mt-1 block text-xs text-muted-foreground">
              {isEnabled ? t("✓ Users can call this tool") : t("✗ Users cannot call this tool")}
            </p>
          </div>
          <Button
            variant="ghost"
    ...
    <Card className="p-6">
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-border bg-muted p-3">
          <p className="text-sm">
            <strong>{t("Select which tools users can call:")}</strong> {t("Only checked tools will be available for users to invoke. Unchecked tools will be blocked from execution.")}
          </p>
        </div>
        ...
        {isLoadingTools && (
          <div className="flex items-center justify-center gap-3 py-6">
            ...
            <p className="text-sm">{t("Loading tools...")}</p>
          </div>
        )}
        ...
        {!isLoadingTools &&
          ...
          (keyTools && keyTools.length > 0 ? (
            <div className="rounded-lg border border-dashed py-4 text-center text-muted-foreground">
              ...
              <p className="mt-1 block text-sm">{t("Expected tools: {value0}", { value0: (keyTools.map((t) => t.name).join(", ")) })}</p>
            </div>
          ) : (
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2 rounded-lg border border-border bg-muted p-3">
              ...
              <p className="text-sm font-medium">
                {effectiveAllowedTools.length} of {tools.length} {tools.length === 1 ? t("tool") : t("tools")} enabled for
                user access
              </p>
            </div>
              ...
              <>
                {filteredTools.length === 0 ? (
                  <div className="rounded-lg border border-dashed py-6 text-center text-muted-foreground">
                    ...
                    <p className="text-sm">{t("No tools found matching \"{toolSearchTerm}\"", { toolSearchTerm })}</p>
                  </div>
                ) : (
                    ...
                    {restFiltered.length > 0 && (
                      <>
                        <div className="flex items-center justify-between px-1 pt-2">
                          <p className="text-xs font-semibold tracking-wide text-muted-foreground uppercase">
                            {pinnedFiltered.length > 0 ? t("All tools") : t("Tools")}
                          </p>
                          <div className="flex gap-2">
