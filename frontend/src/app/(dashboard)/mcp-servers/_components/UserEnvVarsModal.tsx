...
const buildSchema = (required: readonly MCPUserEnvVarSpec[]) =>
  z.object(
    Object.fromEntries(
      required.map((spec) => [spec.name, spec.is_set ? z.string() : z.string().min(1, t("{value0} is required", { value0: (spec.name) }))]),
    ),
  );
          ...
          <FormField
            ...
            label={
              <span className="flex items-center gap-2">
                ...
                {spec.is_set && <Badge variant="secondary">{t("Set")}</Badge>}
              </span>
            }
          >
            {(field) => (
              <PasswordInput
                ...
                disabled={isSaving}
                placeholder={
                  spec.is_set ? t("Enter a new value to overwrite") : spec.description || t("Enter your {value0}", { value0: (spec.name) })
                }
              />
            )}
          </FormField>
...
const UserEnvVarsModal: React.FC<UserEnvVarsModalProps> = ({ server, open, accessToken, onClose, onSaved }) => {
  ...
  const saveMutation = useMutation({
    ...
    onError: (err) => {
      toast.fromError(t("Failed to save env vars: {value0}", { value0: (err instanceof Error ? err.message : String(err)) }));
    },
  });
      ...
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[520px]">
        <DialogHeader>
          <div className="flex items-center gap-2">
            <DialogTitle className="text-base font-semibold">{t("Set your credentials")}</DialogTitle>
            <StatusBadge tone="info" label={t("Per-user")} />
          </div>
        ...
        <div className="mt-2 space-y-4">
          {isLoading ? (
            ...
            <Alert variant="error">
              <CircleAlert />
              <AlertTitle>{t("Failed to load env vars")}</AlertTitle>
            </Alert>
          ) : required.length === 0 ? (
            <Alert variant="info">
              <Info />
              <AlertTitle>{t("No per-user fields configured for this server.")}</AlertTitle>
            </Alert>
          ) : (
            <>
              <span className="block text-sm text-muted-foreground">
                {t("These values are private to you. Your admin configured this MCP server to require these per-user credentials. Saved values are never shown back; leave an already-set field blank to keep it, or enter a value to set or change it.")}
              </span>
