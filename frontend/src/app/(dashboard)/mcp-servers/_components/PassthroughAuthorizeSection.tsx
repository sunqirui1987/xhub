  ...
  return (
    <div className="rounded-lg border border-dashed border-border p-4 space-y-2 mb-4">
      <p className="text-sm text-muted-foreground">
        {t("Callers bring their own upstream token for this auth type, so LiteLLM never stores tokens. To preview tools and configure the tool allowlist, authorize against the upstream here: the token stays in this browser session only and is never saved to LiteLLM. An OAuth app configured below IS saved with the server, so internal users who authorize from the Tools page go through it.")}
      </p>
      {appMayNotMatchUpstream && (
        <p className="text-sm text-warning">
          {t("You changed the upstream URL or endpoints; the OAuth app entered here was registered for the previous upstream and may not be valid. Update the client ID, or clear it to use dynamic client registration.")}
        </p>
      )}
      <MountedFormField
        label={<span className="text-sm font-medium text-foreground">{t("OAuth Client ID (optional)")}</span>}
      ...
      <MountedFormField
        label={<span className="text-sm font-medium text-foreground">{t("OAuth Client Secret (optional)")}</span>}
      ...
      {oauthFlow.status === "success" && oauthFlow.tokenResponse?.access_token && (
        <p className="text-sm text-success">
          {t("Token held for this browser session. Tools can now be previewed and configured; the token was not saved to LiteLLM.")}
        </p>
      )}
    </div>
  );
