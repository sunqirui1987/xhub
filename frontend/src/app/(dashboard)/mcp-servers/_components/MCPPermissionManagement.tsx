      ...
      {fields.map((item, index) => (
        <div key={item.id} className="flex w-full items-baseline gap-4">
          <MountedFormField
            ...
            className="flex-1"
            rules={{ validate: { required: requiredRule(t("Header name is required")) } }}
          >
            {(headerControl) => (
              <ClearableInput
          ...
          <MountedFormField
            ...
            className="flex-1"
            rules={{ validate: { required: requiredRule(t("Header value is required")) } }}
          >
            {(valueControl) => (
              <ClearableInput control={valueControl} placeholder={t("Header value")} clearLabel="Clear header value" />
            )}
          </MountedFormField>
          <CircleMinus
      ...
      <CollapsibleTrigger className="group flex w-full items-center justify-between gap-4 p-4 text-left">
        <span className="flex items-center">
          <span className="flex items-center space-x-2">
            ...
            <span className="text-lg font-semibold text-foreground">{t("Permission Management / Access Control")}</span>
          </span>
          <span className="text-sm text-muted-foreground ml-4">
            {t("Configure access permissions and security settings (Optional)")}
          </span>
        </span>
        ...
        <div className="space-y-6 pt-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <span className="text-sm font-medium text-foreground flex items-center">
                ...
                <SimpleTooltip content={t("When enabled, every API key can access this MCP server.")}>
            ...
            <MountedFormField name="allow_all_keys" defaultValue={mcpServer?.allow_all_keys ?? false} className="mb-0">
              {(control) => <Switch aria-label={t("Allow All LiteLLM Keys")} {...switchControl(control)} />}
            </MountedFormField>
          </div>
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              <span className="text-sm font-medium text-foreground flex items-center">
                ...
                <SimpleTooltip content={t("When on, only requests from within your internal network are accepted. Turn off to allow external clients (other clusters, ChatGPT, etc). API key authentication is always required regardless of this setting.")}>
                  <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                </SimpleTooltip>
              </span>
              <p className="text-sm text-muted-foreground mt-1">
            ...
            <MountedFormField name="available_on_public_internet" defaultValue={true} className="mb-0">
              {(control) => <Switch aria-label={t("Internal network only")} {...invertedSwitchControl(control)} />}
            </MountedFormField>
          </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <span className="text-sm font-medium text-foreground flex items-center">
                  ...
                  <SimpleTooltip content={t("When on, LiteLLM skips its own API key/SSO check for this server and lets the client complete PKCE directly with the upstream MCP server. Only honored when Auth Type is oauth2. No spend tracking or per-key rate limiting will run on this route.")}>
                    <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                  </SimpleTooltip>
                </span>
                <p className="text-sm text-muted-foreground mt-1">
              ...
              >
                {(control) => (
                  <Switch aria-label={t("Delegate auth to upstream (PKCE passthrough)")} {...switchControl(control)} />
                )}
              </MountedFormField>
            </div>
          )}
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <span className="text-sm font-medium text-foreground flex items-center">
                  ...
                  <SimpleTooltip content={t("When on, this server is treated as an OAuth pass-through: the gateway proxies the upstream /.well-known/oauth-protected-resource metadata, emits spec-compliant 401 challenges when no bearer is supplied, and propagates upstream 401/403 responses. Only honored when Auth Type is None and 'Authorization' is in Extra Headers.")}>
                    <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                  </SimpleTooltip>
                </span>
                <p className="text-sm text-muted-foreground mt-1">
              ...
              <MountedFormField
                ...
                className="mb-0"
              >
                {(control) => <Switch aria-label={t("OAuth pass-through")} {...switchControl(control)} />}
              </MountedFormField>
            </div>
          )}
          ...
          {showInternalDelegatePkceWarning && (
            <Alert variant="warning" className="mb-2">
              <TriangleAlert />
              <AlertTitle>{t("Internal server with upstream OAuth delegation")}</AlertTitle>
              <AlertDescription>
                {t("This MCP server is configured as internal-only but delegates auth to upstream. Anonymous users will be able to reach the upstream OAuth2 /authorize flow without a LiteLLM session. Ensure your upstream provider and network enforce access controls.")}
              </AlertDescription>
            </Alert>
          )}
          ...
          <MountedFormField
            label={
              <span className="text-sm font-medium text-foreground flex items-center">
                {t("MCP Access Groups")}
                <SimpleTooltip content={t("Specify access groups for this MCP server. Users must be in at least one of these groups to access the server.")}>
                  <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                </SimpleTooltip>
              </span>
            }
          ...
          <MountedFormField
            label={
              <span className="text-sm font-medium text-foreground flex items-center">
                {t("Extra Headers")}
                <SimpleTooltip content={t("Forward custom headers from incoming requests to this MCP server (e.g., Authorization, X-Custom-Header, User-Agent)")}>
                  <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                </SimpleTooltip>
          ...
          <Field>
            <FieldLabel>
              <span className="text-sm font-medium text-foreground flex items-center">
                {t("Static Headers")}
                <SimpleTooltip content={t("Send these key-value headers with every request to this MCP server.")}>
                  <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                </SimpleTooltip>
              </span>
            </FieldLabel>
