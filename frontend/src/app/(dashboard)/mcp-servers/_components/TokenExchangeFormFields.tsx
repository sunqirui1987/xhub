...
const TOKEN_EXCHANGE_PROFILE_ITEMS = [
  { value: "rfc8693", label: t("RFC 8693 (standard)") },
  { value: "entra_obo", label: t("Microsoft Entra OBO") },
];
  ...
  return (
    <>
      <MountedFormField
        ...
        {...(isEditing ? {} : { defaultValue: t("rfc8693") })}
      >
        {(control) => (
      ...
      <MountedFormField
        label={
          <FieldLabel
            ...
            tooltip={t("RFC 8693 token endpoint. The proxy exchanges the user's incoming token here for a scoped token used to call the upstream MCP server. Leave blank to auto-discover it from the upstream's protected-resource metadata (RFC 9728 then RFC 8414).")}
          />
        }
      ...
      <MountedFormField
        ...
        required={!isEditing}
        rules={requiredWhenCreating(t("Client ID is required for token exchange"))}
      >
        {(control) => (
          <PasswordInput
            {...textControl(control)}
      ...
      <MountedFormField
        ...
        required={!isEditing}
        rules={requiredWhenCreating(t("Client Secret is required for token exchange"))}
      >
        {(control) => (
          <PasswordInput
            {...textControl(control)}
          ...
          <MountedFormField
            label={
              <FieldLabel
                ...
                tooltip={t("Target audience for the exchanged token (RFC 8693 audience). Identifies the upstream MCP server the token is for.")}
              />
            }
            name="audience"
          >
          ...
          <MountedFormField
            label={
              <FieldLabel
                ...
                tooltip={t("Type of the user's incoming token (RFC 8693 subject_token_type). Defaults to urn:ietf:params:oauth:token-type:access_token.")}
              />
            }
