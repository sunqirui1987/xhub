  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (response.imported.length > 0) {
        toast.success(t("Imported {value0} MCP server{value1}", { value0: (response.imported.length), value1: (response.imported.length === 1 ? "" : "s") }));
        onImported();
      }
    } catch (error) {
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && handleClose()}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>{t("Import MCP Connectors")}</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <p className="text-sm text-muted-foreground">
            {t("Paste an Anthropic connector configuration: the")} <code>mcpServers</code> {t("mapping from a Claude Desktop / Claude Code config file, or the")} <code>mcp_servers</code> {t("array from the Anthropic Messages API.")}
          </p>
          <Textarea
            aria-label={t("Connector JSON")}
            value={configText}
          ...
          {result && (
            <div className="space-y-2 text-sm">
              {result.imported.length > 0 && (
                <div>
                  <span className="font-semibold">{t("Imported:")}</span>{" "}
