...
const CreateMCPServer: React.FC<CreateMCPServerProps> = ({
  ...
  const {
    ...
    onTokenReceived: (token, registeredClient) => {
      ...
      toast.success(t("OAuth authorization successful! Please click 'Create MCP Server' to save the configuration."));
    },
    ...
    try {
      if (accessToken != null) {
        ...
        if (isAdmin) {
          ...
          toast.success(t("MCP Server submitted for admin review"), {
      ...
      toast.fromError(isAdmin ? t("Error creating MCP Server: {reason}", { reason }) : t("Error submitting MCP Server: {reason}", { reason }));
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <DialogContent className="top-8 max-h-[calc(100dvh-4rem)] translate-y-0 overflow-y-auto sm:max-w-[1000px]">
        <DialogHeader>
          <div className="flex items-center gap-3 border-b border-border pb-4">
            ...
            <img src={mcpLogoImg} alt={t("MCP Logo")} className="size-5 object-contain" />
                ...
                <div className="grid grid-cols-1 gap-6">
                  ...
                  <MountedFormField
                    label={
                      <span className="text-sm font-medium text-foreground flex items-center">
                        {t("Alias")}
                        <SimpleTooltip content={t("A short, unique identifier for this server. Defaults to the server name if not provided. Cannot contain spaces or hyphens; use underscores instead.")}>
                          <Info className="ml-2 size-4 text-info hover:text-info/80 cursor-help" />
                        </SimpleTooltip>
                      </span>
                    }
                  ...
                  <MountedFormField
                    label={<span className="text-sm font-medium text-foreground">{t("Description")}</span>}
                    name="description"
                  >
                    {(control) => (
                      <Input
                  ...
                  <MountedFormField
                    label={<span className="text-sm font-medium text-foreground">{t("GitHub / Source URL")}</span>}
                    name="source_url"
                  >
                    {(control) => (
                      <Input
                  ...
                  <MountedFormField
                    label={<span className="text-sm font-medium text-foreground">{t("Transport Type")}</span>}
                    name="transport"
                    required
                    rules={{ validate: { required: requiredRule(t("Please select a transport type")) } }}
                  >
                    {(control) => (
                      <Select
                  ...
                  {(transportType === "http" || transportType === "sse") && (
                    <MountedFormField
                      label={<span className="text-sm font-medium text-foreground">{t("MCP Server URL")}</span>}
                      name="url"
                      required
                      rules={{
                        validate: {
                  ...
                  {transportType !== "stdio" && transportType !== "" && (
                    <Collapsible defaultOpen className="mb-4">
                      <CollapsibleTrigger className="group flex w-full items-center justify-between gap-4 py-2 text-left">
                        <span className="text-sm font-semibold text-foreground">{t("Authentication settings")}</span>
                      ...
                      <CollapsibleContent keepMounted className="space-y-6 pt-2">
                        <MountedFormField
                          ...
                          name="auth_type"
                          required
                          rules={{ validate: { required: requiredRule(t("Please select an auth type")) } }}
                        >
                          {(control) => (
                        ...
                        {shouldShowAuthValueField && (
                          <MountedFormField
                            ...
                            rules={{
                              validate: {
                                notWhitespace: notOnlyWhitespace(t("Authentication value cannot be empty whitespace")),
                              },
                            }}
                          >
                            {(control) => (
