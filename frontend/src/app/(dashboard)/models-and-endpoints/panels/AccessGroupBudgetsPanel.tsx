...
function EmptyState() {
  return (
    <div className="flex flex-col items-center gap-1 py-6">
      ...
      <div className="text-sm font-medium text-foreground">{t("No model access groups yet")}</div>
      <div className="text-sm text-muted-foreground">
        {t("Put a deployment in an access group from its model settings, then give the group a shared budget here.")}
      </div>
    </div>
  );
}
...
export default function AccessGroupBudgetsPanel() {
  ...
  const handleSubmit = (params: SetModelAccessGroupBudgetParams) => {
    ...
    setBudget.mutate(
      ...
      {
        onSuccess: () => {
          toast.success(t("Budget saved for \"{accessGroup}\"", { accessGroup }));
          setEditing(null);
        },
      },
    );
  ...
  const handleConfirmClear = () => {
    ...
    clearBudget.mutate(accessGroup, {
      onSuccess: () => {
        toast.success(t("Budget cleared for \"{accessGroup}\"", { accessGroup }));
        setClearing(null);
      },
    });
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      <p className="text-sm text-muted-foreground">
        {t("A model access group can carry one budget that every key granted the group by name draws from together. Keys that reach the group's models through a wildcard or all-proxy-models are not charged against it.")}
      </p>
      ...
      <DataTable
        ...
        isLoading={isLoading}
        loadingMessage={t("Loading model access groups…")}
      ...
      <DeleteResourceModal
        ...
        message={t("Are you sure you want to clear this access group's budget? The recorded shared spend is cleared with it, and the group's models stay available.")}
        resourceInformationTitle="Access Group"
        resourceInformation={[
          { label: t("Access Group"), value: clearing?.access_group ?? null, code: true },
          { label: t("Max Budget"), value: clearing?.budget?.max_budget?.toString() ?? null },
        ]}
