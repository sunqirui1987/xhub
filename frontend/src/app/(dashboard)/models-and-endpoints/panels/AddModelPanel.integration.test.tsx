...
describe("AddModelPanel submit payload contract", () => {
  ...
  it("merges typed XHub Params into litellm_params", async () => {
    const { user, openAdvanced, fillRequired, submit } = await setup();
    await fillRequired();
    await openAdvanced();
    await user.type(screen.getByLabelText("XHub Params"), '{{"rpm": 7}');
    await submit();
  ...
  it("restores the typed value when the section is expanded again", async () => {
    ...
    await closeAdvanced();
    await openAdvanced();
    expect(screen.getByLabelText("XHub Params")).toHaveValue('{"rpm": 7}');
...
describe("AddModelPanel validation gates", () => {
  ...
  it("blocks the submit when XHub Params is not valid JSON", async () => {
    mockPtuEnabled.mockReturnValue(false);
    const { user, openAdvanced, fillRequired, submitExpectingRejection } = await setup();
    await fillRequired();
    await openAdvanced();
...
describe("AddModelPanel behaviours the removed Advanced Settings form instance never drove", () => {
  ...
  it("leaves XHub Params untouched when pass through routes is switched on", async () => {
    const { user, openAdvanced, fillRequired, submit } = await setup();
    await fillRequired();
    await openAdvanced();
    await user.click(screen.getByRole("switch", { name: "Use in pass through routes" }));
    expect(screen.getByLabelText("XHub Params")).toHaveValue("");
