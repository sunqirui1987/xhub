...
export function AutoRoutersPanel({
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      toast.success(t("Deleted auto router: {value0}", { value0: (deletingRouter.name) }));
      setDeletingRouter(null);
      await invalidateAutoRouters();
    } catch (error) {
      toast.fromError(t("Failed to delete auto router: {error}", { error }));
    } finally {
      setIsDeleting(false);
    }
  };
    ...
    <div className="w-full space-y-4">
      ...
      {deletingRouter && (
        <DeleteResourceModal
          ...
          message={t("Are you sure you want to delete \"{value0}\"? Any client still calling this model name will start failing.", { value0: (deletingRouter.name) })}
