...
const budgetSchema = z
  ...
  .refine(hasAnyBudgetValue, {
    message: t("Set at least one of max budget, soft budget or reset window"),
    path: ["max_budget"],
  });
  ...
  return (
    <Dialog open={accessGroup !== null} onOpenChange={(open) => !open && onCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[560px]">
        <DialogHeader>
          <DialogTitle>
            {t("{value0} budget for \"{value1}\"", { value0: (budget ? t("Edit") : "Set"), value1: (accessGroup?.access_group) })}</DialogTitle>
        </DialogHeader>
        <p className="text-sm text-muted-foreground">
          {t("Every key granted this access group by name draws from this one budget. A key that reaches the group's models through a wildcard or")} <code>all-proxy-models</code> {t("is not charged against it.")}
        </p>
          ...
          <TooltipProvider>
            <FieldGroup className="mt-4">
              <FormField
                ...
                label={labelWithHint(
                  t("Max Budget (USD)"),
                  t("Total the whole group may spend. Once its shared spend reaches this, every key that draws from the group is refused"),
                )}
              >
              ...
              <FormField
                ...
                label={labelWithHint(
                  t("Soft Budget (USD)"),
                  t("Fires an alert when the group's spend reaches this. Requests keep succeeding"),
                )}
              >
                {({ ref, value, ...field }) => <NumericalInput {...field} value={value ?? ""} step={0.01} />}
              </FormField>
              ...
              <FormField
                ...
                label={labelWithHint(
                  t("Reset Budget"),
                  t("How often the group's spend resets. Leave empty for a budget that never resets"),
                )}
              >
                {({ id, value, onChange }) => (
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              {t("A field left blank keeps whatever the budget already has. Use Clear budget to remove the budget itself.")}
            </p>
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <Button type="submit" disabled={isSaving}>
                {isSaving ? t("Saving...") : t("Save Budget")}
              </Button>
            </div>
          </TooltipProvider>
        </form>
