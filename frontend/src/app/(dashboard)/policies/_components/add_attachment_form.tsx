...
const attachmentShape = {
  policy_names: z.array(z.string()).min(1, t("Please select at least one policy")),
  ...
  const handleSubmit = async (values: AttachmentFormValues) => {
    try {
      ...
      if (successCount > 0 && failed.length === 0) {
        toast.success(
          successCount === 1 ? t("Attachment created successfully") : t("{successCount} attachments created successfully", { successCount }),
        );
      } else if (successCount > 0 && failed.length > 0) {
        toast.fromError(t("{successCount} attachments created, {value1} failed", { successCount, value1: (failed.length) }));
      } else {
        throw new Error(failed[0]?.reason instanceof Error ? failed[0].reason.message : t("Failed to create attachments"));
      }
      ...
      toast.fromError(t("Failed to create attachment: ") + (error instanceof Error ? error.message : String(error)));
    } finally {
  ...
  return (
    <Dialog open={visible} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[600px]">
        <DialogHeader>
          <DialogTitle>{t("Create Policy Attachment")}</DialogTitle>
        </DialogHeader>
        <TooltipProvider>
          <form onSubmit={(event) => event.preventDefault()} noValidate>
            <FieldGroup>
              <FormField control={form.control} name="policy_names" label={t("Policies")}>
                {({
                  id,
                  value,
                  onChange,
                  ...
                  <TokenSelect
                    ...
                    emptyText={t("No matching policies")}
              ...
              <div>
                <FieldTitle className="mb-2">{t("Scope Type")}</FieldTitle>
                <RadioGroup value={scopeType} onValueChange={(value: unknown) => setScopeType(value as ScopeType)}>
                  <FieldLabel className="font-normal">
                    <RadioGroupItem value="specific" />
                    {t("Specific (teams, keys, models, or tags)")}
                  </FieldLabel>
                  <FieldLabel className="font-normal">
                    <RadioGroupItem value="global" />
                    {t("Global (applies to all requests)")}
                  </FieldLabel>
                </RadioGroup>
              </div>
              ...
              {scopeType === "specific" && (
                <>
                  <FormField
                    ...
                    label={labelWithHint(
                      t("Teams"),
                      t("Select team aliases or enter custom patterns. Supports wildcards (e.g., healthcare-*)"),
                    )}
                  >
                    {({
                      id,
                  ...
                  <FormField
                    ...
                    label={labelWithHint(
                      t("Keys"),
                      t("Select key aliases or enter custom patterns. Supports wildcards (e.g., dev-*)"),
                    )}
                  >
                    {({
                      id,
                  ...
                  <FormField
                    ...
                    label={labelWithHint(
                      t("Models"),
                      t("Model names this attachment applies to. Supports wildcards (e.g., gpt-4*). Leave empty to apply to all models."),
                    )}
                  >
                    {({
                      id,
                      ...
                      <TokenSelect
                        ...
                        onBlur={onBlur}
                        placeholder={
                          isLoadingModels ? t("Loading models...") : t("Select or enter model names (e.g., gpt-4, bedrock/*)")
                        }
                  ...
                  <FormField
                    ...
                    label={labelWithHint(
                      t("Tags"),
                      t("Match against tags set in key or team metadata. Use exact values (e.g., healthcare) or wildcard patterns (e.g., health-*) where * matches any suffix."),
                    )}
                    description={
                      <span className="text-xs">
                        {t("Matches tags from key/team")} <code>metadata.tags</code> {t("or tags passed dynamically in the request body. Use")} <code>*</code> {t("as a suffix wildcard (e.g.,")} <code>prod-*</code> {t("matches")}{" "}
