    ...
    <div>
      <div className="bg-card border border-border rounded-lg p-6 mb-6">
        ...
        <form onSubmit={(event) => event.preventDefault()} noValidate>
          <FieldGroup className="grid grid-cols-2 gap-4">
            <FormField control={form.control} name="team_alias" label={t("Team Alias")}>
              {({ id, value, onChange }) => (
                <ContextCombobox
                  id={id}
                  value={value}
            ...
            <FormField control={form.control} name="model" label={t("Model")}>
              {({ id, value, onChange }) => (
                <ContextCombobox
                  id={id}
                  value={value}
      ...
      {!hasSearched && (
        <div className="bg-card border border-border rounded-lg p-8 text-center">
          ...
          <p className="text-sm font-medium text-foreground mb-1">{t("No simulation run yet")}</p>
        ...
        <div className="bg-card border border-border rounded-lg p-6">
          {result.matched_policies.length === 0 ? (
            <div className="py-6 text-center">
              ...
              <p className="text-sm text-muted-foreground">{t("No policies matched this context")}</p>
            </div>
          ) : (
            <>
              <div className="mb-4">
      ...
      {hasSearched && !result && !isLoading && (
        <Alert variant="error">
          ...
          <AlertDescription>{t("Failed to resolve policies. Check the proxy logs.")}</AlertDescription>
        </Alert>
      )}
    </div>
  );
