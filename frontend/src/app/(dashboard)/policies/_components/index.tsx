...
const DismissibleAlert: React.FC<DismissibleAlertProps> = ({ title, icon, children }) => {
  ...
  return (
    <Alert className="mb-6">
      ...
      <AlertAction>
        <Button variant="ghost" size="icon-sm" onClick={() => setIsDismissed(true)} aria-label={t("Dismiss {title}", { title })}>
          <X />
        </Button>
      </AlertAction>
    </Alert>
...
const PoliciesPanel: React.FC<PoliciesPanelProps> = ({ accessToken, userRole }) => {
  ...
  const fetchPolicies = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t("Failed to fetch policies"));
    } finally {
      setIsLoading(false);
    }
  }, [accessToken]);
  ...
  const fetchAttachments = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t("Failed to fetch attachments"));
    } finally {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(t("Policy \"{value0}\" deleted successfully", { value0: (policyToDelete.policy_name) }));
      await fetchPolicies();
    } catch (error) {
  ...
  const proceedWithTemplate = async (template: any) => {
    ...
    try {
      ...
      console.error("Error fetching guardrails:", error);
      toast.error(t("Failed to load guardrails. Please try again."));
    }
  };
  ...
  const handleParameterConfirm = async (
    ...
    try {
      ...
      toast.error(t("Failed to configure template. Please try again."));
      setIsEnrichingTemplate(false);
    }
  };
  ...
  const handleGuardrailSelectionConfirm = async (selectedGuardrailDefinitions: any[]) => {
    ...
    try {
      ...
      if (createdGuardrails.length > 0) {
        toast.success(
          t("Created {value0} guardrail{value1}! Complete the policy form to save.", { value0: (createdGuardrails.length), value1: (createdGuardrails.length > 1 ? "s" : "") }),
        );
      } else {
        toast.success(t("Template ready! Complete the policy form to save."));
      }
      ...
      toast.error(t("Failed to create guardrails. Please try again."));
    }
  };
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="policies" keepMounted>
          ...
          <DeleteResourceModal
            ...
            message={t("Are you sure you want to delete policy: {value0}? This action cannot be undone.", { value0: (policyToDelete?.policy_name) })}
        ...
        <TabsContent value="attachments" keepMounted>
          <DismissibleAlert title={t("About Policy Attachments")} icon={<Info />}>
            <p className="mb-3">
              {t("Policy attachments control where your policies apply. Policies don't do anything until you attach them to specific teams, keys, models, tags, or globally.")}
            </p>
            ...
            <ul className="mb-3 ml-2 list-inside list-disc space-y-1">
              ...
              <li>
                <strong>{t("Tags")}</strong> {t("- Matches tags from key/team")} <code>metadata.tags</code> {t("or tags passed dynamically in the request body (")}<code>metadata.tags</code>{t("). Use this to enforce policies across groups, e.g. \"all keys tagged")} <code>healthcare</code> {t("get HIPAA guardrails.\" Supports wildcards (")}
                <code>prod-*</code>).
              </li>
            </ul>
