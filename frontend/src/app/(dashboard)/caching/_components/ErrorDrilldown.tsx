  ...
  return (
    <div className="min-w-40 rounded-lg border border-border/50 bg-background px-2.5 py-1.5 text-xs shadow-xl">
      <p className="mb-1.5 font-medium text-foreground">
        {t("Error code {value0}: {value1} failed", { value0: (String(label)), value1: (datum[FAILED_REQUESTS_SERIES].toLocaleString()) })}</p>
...
export const ErrorDrilldownCard = ({ callType, buckets, valueFormatter, onClose }: ErrorDrilldownCardProps) => (
  <Card className="mt-4">
    <CardHeader className="flex flex-row items-center justify-between">
      <CardTitle className="text-base font-semibold">{t("Failed requests by error code: {callType}", { callType })}</CardTitle>
      <Button variant="outline" size="icon-sm" onClick={onClose} aria-label={t("Close error breakdown")}>
        <X />
      </Button>
    </CardHeader>
    <CardContent>
      <p className="text-sm text-muted-foreground">{t("Hover a bar to see the error classes behind that code.")}</p>
      <BarChart
