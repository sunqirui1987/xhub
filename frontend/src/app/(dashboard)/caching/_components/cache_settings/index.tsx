...
const CacheSettings: React.FC<CacheSettingsProps> = ({ accessToken }) => {
  ...
  const loadCacheSettings = useCallback(async () => {
    ...
    try {
      ...
      toast.fromError(t("Failed to load cache settings"));
    }
  }, [accessToken, form]);
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      if (result.status === "success") {
        toast.success(t("Cache connection test successful!"));
      } else {
        toast.fromError(t("Connection test failed: {value0}", { value0: (result.message || result.error) }));
      }
    } catch (error) {
      console.error("Test connection error:", error);
      toast.fromError(t("Connection test failed: {value0}", { value0: (error instanceof Error ? error.message : t("Unknown error")) }));
    } finally {
      setIsTesting(false);
    }
  };
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      toast.success(t("Cache settings updated successfully"));
      await loadCacheSettings();
    } catch (error) {
      console.error("Failed to save cache settings:", error);
      toast.fromError(t("Failed to update cache settings"));
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <FormProvider {...form}>
        <form onSubmit={(event) => event.preventDefault()} className="space-y-6">
          <div className="max-w-3xl">
            <h3 className="text-sm font-medium text-foreground">{t("Cache Settings")}</h3>
            <p className="text-xs text-muted-foreground mt-1">{t("Configure Redis cache for LiteLLM")}</p>
          </div>
          ...
          <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen} className="mt-4">
            <CollapsibleTrigger className="group flex w-full items-center justify-between py-2 text-left">
              <span className="text-sm font-medium text-foreground">{t("Advanced Settings")}</span>
