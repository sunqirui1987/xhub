...
const CoordinationRedisSettings: React.FC = () => {
  ...
  useEffect(() => {
    if (isError) {
      toast.fromError(t("Failed to load coordination Redis settings"));
    }
  }, [isError]);
  ...
  const handleTestConnection = async () => {
    ...
    try {
      ...
      if (result.status === "healthy") {
        toast.success(t("Coordination Redis connection test successful!"));
      } else {
        toast.fromError(t("Connection test failed: {value0}", { value0: (result.error ?? t("Unknown error")) }));
      }
    } catch (error) {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      toast.success(t("Coordination Redis settings saved. Restart the proxy to apply them."));
    } catch {
      toast.fromError(t("Failed to update coordination Redis settings"));
    }
  };
        ...
        <form onSubmit={(event) => event.preventDefault()} className="space-y-6">
          <div className="max-w-3xl space-y-2">
            <div className="flex items-center gap-3">
              <h3 className="text-sm font-medium text-foreground">{t("Coordination Redis")}</h3>
              {!isLoading && (
            ...
            <p className="text-xs text-warning">{t("Saved changes take effect on proxy restart.")}</p>
          </div>
