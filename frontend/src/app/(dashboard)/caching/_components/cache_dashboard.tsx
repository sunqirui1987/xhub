...
const CacheDashboard: React.FC<CachePageProps> = ({ accessToken, token, userRole, userID, premiumUser }) => {
  ...
  const runCachingHealthCheck = async () => {
    try {
      toast.info(t("Running cache health check..."));
  ...
  const statCards = [
    { label: t("pages.caching.hitRatio"), value: `${hasRequests ? totals.cache_hit_ratio.toFixed(2) : "0"}%` },
    { label: t("pages.caching.hits"), value: valueFormatterNumbers(totals?.cache_hits ?? 0) },
    { label: t("pages.caching.cachedTokens"), value: valueFormatterNumbers(totals?.cached_completion_tokens ?? 0) },
  ];
        ...
        <div className="flex items-center space-x-2">
          {lastRefreshed && (
            <p className="text-sm text-muted-foreground">
              {t("pages.caching.lastRefreshed")} {lastRefreshed}
            </p>
          )}
          <Button variant="outline" size="icon-sm" onClick={handleRefreshClick} aria-label={t("pages.caching.refresh")}>
            <RefreshCw />
          </Button>
        </div>
      </div>
        ...
        <Card>
          <CardContent>
            ...
            <Card className="mt-4">
              ...
              <CardContent>
                <p className="text-sm text-muted-foreground">
                  {t("Click a red failed-requests segment to see which error codes caused those failures.")}
                </p>
