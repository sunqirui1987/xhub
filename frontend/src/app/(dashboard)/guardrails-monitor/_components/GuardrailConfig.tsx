  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card border border-border rounded-lg p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-sm font-medium text-foreground">{t("Version:")}</span>
            <Select
          ...
          <div className="flex items-center gap-2">
            ...
            <Button>
              <Save />
              {t("Save as v")}{parseInt(version.replace("v", ""), 10) + 1}
            </Button>
          </div>
        </div>
      ...
      <div className="bg-card border border-border rounded-lg p-6">
        <h3 className="text-base font-semibold text-foreground mb-1">{t("Parameters")}</h3>
        <p className="text-xs text-muted-foreground mb-5">{t("Configure {guardrailName} behavior", { guardrailName })}</p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">{t("Action on Failure")}</label>
            <Select
              items={ACTION_ITEMS}
              value={action}
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">{t("Provider")}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-foreground mb-1.5">{t("Guardrail Type")}</label>
          ...
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-foreground mb-1.5">{t("Categories (comma-separated)")}</label>
          ...
          <div className="md:col-span-2 flex items-center gap-3">
            ...
            <Label htmlFor={enabledToggleId} className="font-normal text-foreground">
              {t("Guardrail enabled in production")}
            </Label>
          </div>
        </div>
      </div>
      ...
      <div className="bg-card border border-border rounded-lg p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Switch aria-label={t("Custom Code Override")} checked={useCustomCode} onCheckedChange={setUseCustomCode} />
        </div>
    ...
    return {"score": 0.9, "passed": True, "reason": "No violations"}`}
      ...
      <div className="bg-card border border-border rounded-lg p-6">
        <h3 className="text-base font-semibold text-foreground mb-1">{t("Test Configuration")}</h3>
        ...
        <div className="flex items-center gap-3">
          ...
          {rerunStatus === "success" && (
            <span className="text-sm text-success flex items-center gap-2">
              <CircleCheck className="size-4" /> {t("7/10 would now pass with new config")}
            </span>
          )}

          {rerunStatus === "error" && <span className="text-sm text-destructive">{t("Error running tests")}</span>}
        </div>
      </div>
    </div>
  );
