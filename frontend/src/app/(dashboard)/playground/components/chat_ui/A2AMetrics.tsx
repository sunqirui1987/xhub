            ...
            <TooltipTrigger
              render={
                <Button
                  ...
                  aria-label={t("Copy task ID {taskId}", { taskId })}
                />
              }
            >
              <FileText className="size-3" />
              {t("Task:")} {truncateId(taskId)}
              <Copy className="size-3 text-muted-foreground" />
            </TooltipTrigger>
            <TooltipContent>{t("Click to copy: {taskId}", { taskId })}</TooltipContent>
          </Tooltip>
        )}
            ...
            <TooltipTrigger
              render={
                <Button
                  ...
                  aria-label={t("Copy session ID {contextId}", { contextId })}
                />
              }
            >
              <Link className="size-3" />
              {t("Session:")} {truncateId(contextId)}
              <Copy className="size-3 text-muted-foreground" />
            </TooltipTrigger>
            <TooltipContent>{t("Click to copy: {contextId}", { contextId })}</TooltipContent>
          </Tooltip>
        )}
      ...
      <Collapsible open={showDetails} onOpenChange={setShowDetails}>
        <CollapsibleContent>
          <div className="mt-2 ml-4 p-3 bg-muted rounded-md text-muted-foreground border border-border">
            ...
            {status?.message && (
              <div className="mb-2">
                <span className="font-medium text-foreground">{t("Status Message:")}</span>
            ...
            {taskId && (
              <div className="mb-1.5 flex items-center">
                <span className="font-medium text-foreground w-24">{t("Task ID:")}</span>
            ...
            {contextId && (
              <div className="mb-1.5 flex items-center">
                <span className="font-medium text-foreground w-24">{t("Session ID:")}</span>
                <code className="ml-2 px-2 py-1 bg-card border border-border rounded-sm text-xs font-mono">
                  {contextId}
                </code>
                <Button
            ...
            {metadata && Object.keys(metadata).length > 0 && (
              <div className="mt-3">
                <span className="font-medium text-foreground">{t("Custom Metadata:")}</span>
