...
const ChatUI: React.FC<ChatUIProps> = ({
  ...
  const handleSendMessage = async () => {
    ...
    if (endpointType === EndpointType.MCP) {
      ...
      if (!rawSelected) {
        toast.fromError(t("Please select an MCP server to test"));
        return;
      }
      if (!selectedMCPDirectTool) {
        toast.fromError(t("Please select an MCP tool to call"));
        return;
      }
      ...
      if (!mcpTool) {
        toast.fromError(t("Please wait for tool schema to load"));
        return;
      }
      try {
        mcpToolArguments = (await mcpToolArgsFormRef.current?.getSubmitValues()) ?? {};
      } catch (err) {
        toast.fromError(err instanceof Error ? err.message : t("Please fill in all required parameters"));
        return;
      }
    }
    ...
    if (modelRequiredEndpoints.includes(endpointType as EndpointType) && !selectedModel) {
      toast.fromError(t("Please select a model before sending a request"));
      return;
    }
                      ...
                      {uploadedImages.length === 0 ? (
                        ...
                        <div className="flex flex-wrap gap-2">
                          ...
                          <label className="flex h-32 w-32 cursor-pointer flex-col items-center justify-center rounded-md border-2 border-dashed border-border hover:border-ring">
                            ...
                            <p className="mt-1 text-xs text-muted-foreground">{t("Add more")}</p>
                            <input
                              type="file"
                              accept={IMAGE_EDIT_ACCEPT}
                              multiple
                        ...
                        <div className="flex items-center gap-2">
                          {isLoading ? (
                            <>
                              ...
                              <span className="text-sm font-medium text-info">{t("Running Python code...")}</span>
                            </>
                          ) : (
                            <>
                              <Code2 className="size-4 text-info" aria-hidden="true" />
                              <span className="text-sm font-medium text-info">{t("Code Interpreter Active")}</span>
                            </>
                          )}
                        </div>
                        <button
                          ...
                          <CodeInterpreterToggle
                            ...
                            onToggle={() => {
                              ...
                              if (!codeInterpreter.enabled) {
                                toast.success(t("Code Interpreter enabled!"));
                              }
                            }}
                          />
                        )}
