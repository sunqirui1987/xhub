...
const REMOVAL_COPY = {
  discount: { title: t("Remove Provider Discount"), noun: "discount" },
  margin: { title: t("Remove Provider Margin"), noun: "margin" },
} as const;
    ...
    <div className="w-full p-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-6">
        <div>
          <div className="flex items-center gap-2">
            <p className="text-xl font-medium text-foreground">{t("Cost Tracking Settings")}</p>
      ...
      <div className="bg-card rounded-lg shadow-sm w-full max-w-full space-y-4">
        ...
        {isProxyAdmin && (
          <Collapsible className="rounded-lg border">
            <SectionHeader
              title={t("pages.costTracking.providerDiscounts")}
              description={t("pages.costTracking.providerDiscountsBody")}
            />
              ...
              <Tabs defaultValue="discounts">
                <TabsList variant="line" className="mx-6 mt-4 h-auto justify-start rounded-none border-b p-0">
                  <TabsTrigger value="discounts" className="flex-none rounded-none px-4 py-2">
                    {t("pages.costTracking.discounts")}
                  </TabsTrigger>
                  <TabsTrigger value="test-it" className="flex-none rounded-none px-4 py-2">
                    {t("pages.costTracking.testIt")}
                  </TabsTrigger>
                </TabsList>
                <TabsContent value="discounts" keepMounted>
                  <div className="p-6">
                    <div className="flex justify-end mb-4">
                      <Button onClick={() => setIsModalVisible(true)}>{t("pages.costTracking.addDiscount")}</Button>
                    </div>
                    {isFetching ? (
                      <div className="py-12 text-center">
                        <p className="text-muted-foreground">{t("pages.costTracking.loadingConfig")}</p>
                      </div>
                      ...
                      <div className="py-16 px-6 text-center">
                        ...
                        <p className="text-foreground font-medium mb-2">{t("pages.costTracking.noDiscounts")}</p>
                        <p className="text-muted-foreground text-sm">
                          {t("pages.costTracking.noDiscountsHint")}
                        </p>
                      </div>
                    )}
                  </div>
        ...
        {isProxyAdmin && (
          <Collapsible className="rounded-lg border">
            <SectionHeader
              title={t("pages.costTracking.feeMargin")}
              description={t("pages.costTracking.feeMarginBody")}
            />
            <CollapsibleContent className="px-0">
              <div className="p-6">
                <div className="flex justify-end mb-4">
                  <Button onClick={() => setIsMarginModalVisible(true)}>{t("pages.costTracking.addMargin")}</Button>
                </div>
                {isFetching ? (
                  <div className="py-12 text-center">
                    <p className="text-muted-foreground">{t("pages.costTracking.loadingConfig")}</p>
                  ...
                  <div className="py-16 px-6 text-center">
                    ...
                    <p className="text-foreground font-medium mb-2">{t("pages.costTracking.noMargins")}</p>
                    <p className="text-muted-foreground text-sm">{t("pages.costTracking.noMarginsHint")}</p>
                  </div>
                )}
              </div>
            </CollapsibleContent>
        ...
        {isProxyAdmin && (
          <Collapsible className="rounded-lg border">
            <SectionHeader
              title={t("pages.costTracking.blockUnpriced")}
              description={t("pages.costTracking.blockUnpricedBody")}
            />
            <CollapsibleContent className="px-0">
              <div className="p-6">
                <div className="flex items-center justify-between">
                  <div className="pr-6">
                    <p className="text-foreground font-medium">{t("pages.costTracking.blockUnpricedToggle")}</p>
                    <p className="text-sm text-muted-foreground mt-1">{t("pages.costTracking.blockUnpricedHelp")}</p>
                  </div>
                  <Switch
        ...
        <Collapsible defaultOpen={true} className="rounded-lg border">
          <SectionHeader
            title={t("pages.costTracking.calculator")}
            description={t("pages.costTracking.calculatorBody")}
          />
          <CollapsibleContent className="px-0">
            <div className="p-6">
        ...
        <AlertDialog open onOpenChange={(open) => !open && !isRemoving && setPendingRemoval(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {t("Are you sure you want to remove the {value0} for {value1}?", { value0: (REMOVAL_COPY[pendingRemoval.kind].noun), value1: (pendingRemoval.displayName) })}</AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        <DialogContent className="top-8 max-h-[calc(100dvh-4rem)] translate-y-0 overflow-y-auto sm:max-w-[1000px]">
          <DialogHeader>
            <div className="flex items-center space-x-3 pb-4 border-b border-border">
              <DialogTitle className="text-xl font-semibold text-foreground">{t("pages.costTracking.addDiscountTitle")}</DialogTitle>
            </div>
          </DialogHeader>
          <div className="mt-6">
      ...
      <Dialog open={isMarginModalVisible} onOpenChange={(open) => !open && handleMarginModalCancel()}>
        <DialogContent className="top-8 max-h-[calc(100dvh-4rem)] translate-y-0 overflow-y-auto sm:max-w-[1000px]">
          <DialogHeader>
            <div className="flex items-center space-x-3 pb-4 border-b border-border">
              <DialogTitle className="text-xl font-semibold text-foreground">{t("pages.costTracking.addMarginTitle")}</DialogTitle>
            </div>
          </DialogHeader>
          <div className="mt-6">
            <p className="text-sm text-muted-foreground mb-6">
