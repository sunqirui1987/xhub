  ...
  return (
    <div className="space-y-3 bg-muted p-4 rounded-lg">
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("Input Cost")}</p>
          <p className="text-sm break-words">{formatCost(result.input_cost_per_request)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("Output Cost")}</p>
          <p className="text-sm break-words">{formatCost(result.output_cost_per_request)}</p>
        </div>
        <div className="min-w-0">
          <p className="text-xs text-muted-foreground block">{t("Margin Fee")}</p>
      ...
      {periodCost !== null && (
        <div className="grid grid-cols-4 gap-4 pt-2 border-t border-border">
          ...
          <div className="min-w-0">
            <p className="text-xs text-muted-foreground block">{t("{periodLabel} Margin Fee", { periodLabel })}</p>
      ...
      {(result.input_cost_per_token || result.output_cost_per_token) && (
        <div className="text-xs text-muted-foreground pt-2 border-t border-border">
          ...
          {result.input_cost_per_token && (
            <span>{t("Input ${value0}/1M", { value0: (formatNumberWithCommas(result.input_cost_per_token * 1_000_000, 2)) })}</span>
          )}
          {result.input_cost_per_token && result.output_cost_per_token && " | "}
          {result.output_cost_per_token && (
            <span>{t("Output ${value0}/1M", { value0: (formatNumberWithCommas(result.output_cost_per_token * 1_000_000, 2)) })}</span>
          )}
        </div>
      )}
    </div>
  ...
  if (!hasAnyResult && !isAnyLoading && !hasAnyError) {
    return (
      <div className="py-6 text-center border border-dashed border-border rounded-lg bg-muted">
        <p className="text-muted-foreground">{t("Select models above to see cost estimates")}</p>
      </div>
    );
  }
  ...
  if (!hasAnyResult && isAnyLoading && !hasAnyError) {
    return (
      <div className="py-6 text-center">
        ...
        <p className="text-muted-foreground block mt-2">{t("Calculating costs...")}</p>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between">
          <p className="text-base font-semibold text-foreground">{t("Cost Estimates")}</p>
    ...
    <div className="space-y-4">
      ...
      <Card size="sm" className="px-4 bg-linear-to-r from-slate-50 to-blue-50 dark:from-slate-900 dark:to-blue-950">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2">
          <div className="min-w-0">
            <span className="text-xs text-muted-foreground">{t("Total Per Request")}</span>
            <div className="text-lg font-mono text-info break-words">
              {formatCost(multiResult.totals.cost_per_request)}
            </div>
          </div>
          <div className="min-w-0">
            <span className="text-xs text-muted-foreground">{t("Total {periodLabel}", { periodLabel })}</span>
            <div
        ...
        {hasMargin && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 mt-3 pt-3 border-t border-border">
            <div className="min-w-0">
              <div className="text-xs text-muted-foreground">{t("Margin Fee/Request")}</div>
              <div className="text-sm font-mono text-warning break-words">
                {formatCost(multiResult.totals.margin_per_request)}
              </div>
            </div>
            <div className="min-w-0">
              <div className="text-xs text-muted-foreground">{t("{periodLabel} Margin Fee", { periodLabel })}</div>
              <div className="text-sm font-mono text-warning break-words">
                    ...
                    <TableCell className="text-right">
                      {!record.error && (
                        <Button
                          ...
                          aria-label={t("{value0} cost breakdown for {value1}", { value0: (isExpanded ? t("Hide") : t("Show")), value1: (record.model) })}
