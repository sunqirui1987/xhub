...
export function useMarginConfig({ accessToken }: UseMarginConfigProps): UseMarginConfigReturn {
  ...
  const fetchMarginConfig = useCallback(async () => {
    try {
      ...
      toast.fromError(t("Failed to fetch margin configuration"));
    }
  }, [accessToken]);
  ...
  const saveMarginConfig = useCallback(
    async (config: MarginConfig) => {
      try {
        ...
        if (response.ok) {
          toast.success(t("Margin configuration updated successfully"));
        ...
        toast.fromError(t("Failed to update margin configuration"));
      }
    },
  ...
  const handleAddMargin = useCallback(
    async (params: AddMarginParams): Promise<boolean> => {
      ...
      if (!selectedProvider) {
        toast.fromError(t("Please select a provider"));
        return false;
      }
      ...
      if (marginConfig[providerValue]) {
        ...
        toast.fromError(t("Margin for {displayName} already exists. Edit it in the table above.", { displayName }));
        return false;
      }
      ...
      if (marginType === "percentage") {
        ...
        if (isNaN(percentValue) || percentValue < 0 || percentValue > 1000) {
          toast.fromError(t("Percentage must be between 0% and 1000%"));
          return false;
        }
        ...
        if (isNaN(fixedValue) || fixedValue < 0) {
          toast.fromError(t("Fixed amount must be non-negative"));
          return false;
        }
