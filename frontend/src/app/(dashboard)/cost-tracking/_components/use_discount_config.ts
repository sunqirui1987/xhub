...
export function useDiscountConfig({ accessToken }: UseDiscountConfigProps): UseDiscountConfigReturn {
  ...
  const fetchDiscountConfig = useCallback(async () => {
    try {
      ...
      toast.fromError(t("Failed to fetch discount configuration"));
    }
  }, [accessToken]);
  ...
  const saveDiscountConfig = useCallback(
    async (config: DiscountConfig) => {
      try {
        ...
        if (response.ok) {
          toast.success(t("Discount configuration updated successfully"));
        ...
        toast.fromError(t("Failed to update discount configuration"));
      }
    },
  ...
  const handleAddProvider = useCallback(
    async (selectedProvider: string | undefined, newDiscount: string): Promise<boolean> => {
      if (!selectedProvider || !newDiscount) {
        toast.fromError(t("Please select a provider and enter discount percentage"));
        return false;
      }

      const percentageValue = parseFloat(newDiscount);
      if (isNaN(percentageValue) || percentageValue < 0 || percentageValue > 100) {
        toast.fromError(t("Discount must be between 0% and 100%"));
        return false;
      }
      ...
      if (!providerValue) {
        toast.fromError(t("Invalid provider selected"));
        return false;
      }

      if (discountConfig[providerValue]) {
        toast.fromError(
          t("Discount for {value0} already exists. Edit it in the table above.", { value0: (Providers[selectedProvider as keyof typeof Providers]) }),
        );
        return false;
      }
