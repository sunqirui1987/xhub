      ...
      <div className="space-y-6">
        <FieldGroup>
          <Field>
            <FieldLabel htmlFor={PROVIDER_FIELD_ID}>
              {labelWithHint(t("pages.costTracking.provider"), t("Select the LLM provider you want to configure a discount for"))}
            </FieldLabel>
            <Combobox
              items={options}
            ...
            >
              <ComboboxInput id={PROVIDER_FIELD_ID} placeholder={t("pages.costTracking.selectProvider")} className="w-full">
                {selectedOption && (
              ...
              <ComboboxContent>
                <ComboboxEmpty>{t("pages.costTracking.noProviders")}</ComboboxEmpty>
                <ComboboxList>
          ...
          <Field>
            <FieldLabel htmlFor={DISCOUNT_FIELD_ID}>
              {labelWithHint(t("Discount Percentage"), t("Enter a percentage value (e.g., 5 for 5% discount)"))}
            </FieldLabel>
        ...
        <div className="flex items-center justify-end space-x-3 pt-6 border-t border-border">
          <Button type="submit" onClick={onAddProvider} disabled={!selectedProvider || !newDiscount}>
            {t("pages.costTracking.addDiscountTitle")}
          </Button>
        </div>
      </div>
    </TooltipProvider>
