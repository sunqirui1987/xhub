...
const createTagShape = {
  tag_name: z.string().min(1, t("Please input a tag name")),
    ...
    <Dialog open={visible} onOpenChange={(open) => !open && handleCancel()}>
      <DialogContent className="max-h-[calc(100dvh-2rem)] overflow-y-auto sm:max-w-[800px]">
        ...
        <form onSubmit={form.handleSubmit(handleFinish)} noValidate>
          <TooltipProvider>
            <FieldGroup>
              <FormField control={form.control} name="tag_name" label={t("Tag Name")}>
              ...
              <FormField
                ...
                label={labelWithHint(
                  t("Allowed Models"),
                  t("Select which models are allowed to process requests from this tag"),
                )}
              >
                {({ value, onChange }) => (
                  <MultiSelect
              ...
              <CollapsibleContent className="px-4 pb-4">
                <FieldGroup className="mt-4">
                  <FormField
                    ...
                    label={labelWithHint(
                      t("Max Budget (USD)"),
                      t("Maximum amount in USD this tag can spend. When reached, requests with this tag will be blocked"),
                    )}
                  >
                  ...
                  <FormField
                    ...
                    label={labelWithHint(
                      t("Reset Budget"),
                      t("How often the budget should reset. For example, setting 'daily' will reset the budget every 24 hours"),
                    )}
                  >
                    {({ id, value, onChange }) => (
