...
const tagEditShape = {
  name: z.string().min(1, t("Please input a tag name")),
...
const TagEditForm: React.FC<TagEditFormProps> = ({ tag, seedBudgetFields, userModels, onCancel, onSave }) => {
  ...
  return (
    <form onSubmit={form.handleSubmit(submitVisibleValues)} noValidate>
      <FieldGroup>
        <FormField control={form.control} name="name" label={t("Tag Name")}>
        ...
        <FormField
          ...
          description={t("Select which models are allowed to process this type of data")}
        >
          {({ value, onChange }) => (
            <MultiSelect options={modelOptions} value={value} onValueChange={onChange} placeholder={t("Select Models")} />
          )}
        </FormField>
      </FieldGroup>
        ...
        <CollapsibleContent className="px-4 pb-4">
          <FieldGroup className="mt-4">
            <FormField
              ...
              description={t("Maximum amount in USD this tag can spend")}
            >
            ...
            <FormField
              ...
              description={t("How often the budget should reset")}
            >
              {({ id, value, onChange }) => (
...
const TagInfoView: React.FC<TagInfoViewProps> = ({ tagId, onClose, accessToken, is_admin, editTag }) => {
  ...
  const fetchTagDetails = async () => {
    ...
    try {
      ...
      toast.fromError(t("Error fetching tag details: ") + error);
    }
  };
  ...
  const handleSave = async (values: TagEditFormValues) => {
    ...
    try {
      ...
      toast.fromError(t("Error updating tag: ") + error);
    }
  };
    ...
    <div className="p-4">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{tagDetails.description || t("No description")}</p>
        </div>
      ...
      {isEditing ? (
        ...
        <div className="space-y-6">
          <Card>
            <CardContent>
              <CardTitle>{t("Tag Details")}</CardTitle>
              <div className="space-y-4 mt-4">
                ...
                <div>
                  <p className="font-medium">{t("Allowed Models")}</p>
                  <div className="flex flex-wrap gap-2 mt-2">
                    {!tagDetails.models || tagDetails.models.length === 0 ? (
                      <Badge variant="secondary">{t("All Models")}</Badge>
                    ) : (
                ...
                <div className="space-y-4 mt-4">
                  ...
                  {tagDetails.litellm_budget_table.tpm_limit !== undefined &&
                    tagDetails.litellm_budget_table.tpm_limit !== null && (
                      <div>
                        <p className="font-medium">{t("TPM Limit")}</p>
                  ...
                  {tagDetails.litellm_budget_table.rpm_limit !== undefined &&
                    tagDetails.litellm_budget_table.rpm_limit !== null && (
                      <div>
                        <p className="font-medium">{t("RPM Limit")}</p>
