...
const TagManagement: React.FC<TagProps> = ({ accessToken, userID, userRole }) => {
  ...
  const handleCreate = async (formValues: any) => {
    ...
    try {
      ...
      toast.fromError(t("Error creating tag: ") + error);
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      toast.fromError(t("Error deleting tag: ") + error);
    } finally {
      setIsDeleting(false);
    ...
    if (userID && userRole && accessToken) {
      const fetchModels = async () => {
        try {
          ...
          toast.fromError(t("Error fetching models: ") + error);
        }
      };
      fetchModels();
    }
      ...
      {selectedTagId ? (
        ...
        <div className="flex h-full w-full flex-col p-8 pt-10">
          <div className="mt-2 mb-4 flex w-full items-center justify-between">
            <h1>{t("pages.tagManagement.title")}</h1>
            <div className="flex items-center space-x-2">
              {lastRefreshed && <p className="text-sm">{t("Last Refreshed: {lastRefreshed}", { lastRefreshed })}</p>}
