...
export const ptuCountRules: ValidatorRule[] = [
  {
    validator: (_, value) =>
      isPositiveWholePtuCount(value)
        ? Promise.resolve()
        : Promise.reject(new Error(t("PTU Count must be a whole number between 1 and {value0}", { value0: (MAX_PTU_COUNT.toLocaleString()) }))),
  },
];
...
export const ptuRateRules: ValidatorRule[] = [
  {
    validator: (_, value) =>
      isNonNegativePtuRate(value)
        ? Promise.resolve()
        : Promise.reject(
            new Error(t("Cost per PTU / Hour must be between 0 and {value0}", { value0: (MAX_COST_PER_PTU_PER_HOUR.toLocaleString()) })),
          ),
  },
];
  ...
  ({ getFieldValue }: FormInstance): ValidatorRule => ({
    validator: (_, value) =>
      isFilledPtuValue(value) === isFilledPtuValue(getFieldValue(siblingField))
        ? Promise.resolve()
        : Promise.reject(new Error(t("PTU Count and Cost per PTU / Hour must be set together"))),
  });
  ...
  ({ getFieldValue, isFieldTouched }: FormInstance): ValidatorRule => ({
    validator: (_, value) => {
      ...
      return echoed || !isFilledPtuValue(getFieldValue(countField)) || !isFilledPtuValue(value) || Number(value) === 0
        ? Promise.resolve()
        : Promise.reject(new Error(t("A PTU deployment bills by reserved capacity, so this cost must be 0 or blank")));
    },
  });
  ...
  ({ getFieldValue }: FormInstance): ValidatorRule => ({
    validator: (_, value) =>
      isFilledPtuValue(value) || !isFilledPtuValue(getFieldValue(countField))
        ? Promise.resolve()
        : Promise.reject(new Error(t("PTU Effective From is required when PTU Count is set"))),
  });
  ...
  ({ getFieldValue }: FormInstance): ValidatorRule => ({
    validator: (_, value) => {
      ...
      return ptuWindowIsOrdered(start, end)
        ? Promise.resolve()
        : Promise.reject(new Error(t("PTU Effective To must be after PTU Effective From")));
    },
  });
