...
func (s *Server) serveDataPlaneCatalog(w http.ResponseWriter, r *http.Request) {
	...
	if op != "" && (r.Method == http.MethodPost || r.Method == http.MethodPut) {
		...
		default:
			...
			if !s.enforceIdentityLimits(w, path, p, str(body["model"]), estimateTokens(body)) {
				return
			}
			s.writeInferenceNative(w, op, body)
			return
		}
	}
	s.resourceCRUD(w, r, path, raw)
}

func (s *Server) responsesAPI(w http.ResponseWriter, r *http.Request) {
...
func modelFromPath(path string) string {
	parts := split(strings.TrimSuffix(path, "/"))
	for i, p := range parts {
...
func (s *Server) writeInferenceNative(w http.ResponseWriter, op string, body map[string]any) {
	id := httpx.CallID()
...
func (s *Server) resourceCRUD(w http.ResponseWriter, r *http.Request, path string, raw []byte) {
...
func (s *Server) serveMixedCatalog(w http.ResponseWriter, r *http.Request) {
	if s.requireMixed(w, r) == nil {
		return
	}
	s.writeCatalogPersist(w, r, readMap(r))
}

func (s *Server) serveMgmtCatalog(w http.ResponseWriter, r *http.Request) {
...
func (s *Server) writeCatalogPersist(w http.ResponseWriter, r *http.Request, body map[string]any) {
	path := r.URL.Path
	...
	case "delete":
		...
		n := 0
		for _, x := range ids {
			if s.Store.DeleteKV(kind, x) == nil {
				n++
			}
		}
...
func freezeFamily(kind string, obj map[string]any) {
	...
	case "config":
		setDefault(obj, "status", "ok")
		setDefault(obj, "version", Version)
