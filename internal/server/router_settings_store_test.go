...
func TestRedisRouterAndSpendVisibleToSecondProcess(t *testing.T) {
	...
	mk := func() *Server {
		...
		if asInt(s.mergedRouterSettings()["allowed_fails"]) != 3 {
			t.Fatalf("shipped allowed_fails default %v", s.mergedRouterSettings()["allowed_fails"])
		}
	...
	if !b.routerState().Cooldown[failedID] {
		t.Fatal("second process did not see cooldown from the request path")
	}
	picked := router.Pick(b.Cfg.ModelList, "gpt-4o-mini", "latency-based-routing", b.routerState())
	if picked == nil || router.DeploymentID(*picked) == failedID {
		t.Fatalf("cooled deployment still picked %#v", picked)
	}
	if b.routerState().Usage[okID] != 10 {
		t.Fatalf("second process usage %v", b.routerState().Usage)
	}
	if _, ok := b.routerState().Latency[okID]; !ok {
