...
func TestFlushAckFailureRetriesWithoutDoubleSpend(t *testing.T) {
	...
	orig := spendAck
	calls := 0
	spendAck = func(c *live.Client, deltas map[string]float64, n int, head string) error {
		calls++
		if calls == 1 {
...
func openSpendRedis(t *testing.T) (s *Server, h http.Handler, master, plain, teamID, userID, orgID string) {
	t.Helper()
	redisURL := testRedisURL()
	dbURL := testDatabaseURL(t)
	up := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
