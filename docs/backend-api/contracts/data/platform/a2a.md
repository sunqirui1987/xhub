...
|---|---|---|---|
| `POST` | `/a2a/{agent_id}` | `/a2a/{agent_id}` | `proxy/agent_endpoints/a2a_endpoints.py:618` |
| `GET` | `/a2a/{agent_id}/.well-known/agent-card.json` | `/a2a/{agent_id}/.well-known/agent-card.json` | `proxy/agent_endpoints/a2a_endpoints.py:548` |
| `GET` | `/a2a/{agent_id}/.well-known/agent.json` | `/a2a/{agent_id}/.well-known/agent.json` | `proxy/agent_endpoints/a2a_endpoints.py:553` |
| `POST` | `/a2a/{agent_id}/message/send` | `/a2a/{agent_id}/message/send` | `proxy/agent_endpoints/a2a_endpoints.py:623` |
| `POST` | `/v1/a2a/discover` | `/v1/a2a/discover` | `proxy/a2a/endpoints.py:66` |
| `POST` | `/v1/a2a/{agent_id}/message/send` | `/v1/a2a/{agent_id}/message/send` | `proxy/agent_endpoints/a2a_endpoints.py:628` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`message/send: message`、`metadata`。
...
冻结响应字段：`task id`、`status`、`artifacts`。
