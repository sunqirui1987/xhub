...
|---|---|---|---|
| `GET` | `/anthropic/{endpoint}` | `/anthropic/{endpoint}` | `catalog.families:0` |
| `POST` | `/anthropic/{endpoint}` | `/anthropic/{endpoint}` | `catalog.families:0` |
| `GET` | `/azure/{endpoint}` | `/azure/{endpoint}` | `catalog.families:0` |
| `POST` | `/azure/{endpoint}` | `/azure/{endpoint}` | `catalog.families:0` |
| `GET` | `/bedrock/{endpoint}` | `/bedrock/{endpoint}` | `catalog.families:0` |
| `POST` | `/bedrock/{endpoint}` | `/bedrock/{endpoint}` | `catalog.families:0` |
| `POST` | `/comprehendmedical` | `/comprehendmedical` | `proxy/pass_through_endpoints/llm_passthrough_endpoints.py:1287` |
| `POST` | `/comprehendmedical/{operation}` | `/comprehendmedical/{operation}` | `proxy/pass_through_endpoints/llm_passthrough_endpoints.py:1210` |
| `GET` | `/gemini/{endpoint}` | `/gemini/{endpoint}` | `catalog.families:0` |
| `POST` | `/gemini/{endpoint}` | `/gemini/{endpoint}` | `catalog.families:0` |
| `WEBSOCKET` | `/openai/{endpoint:path}` | `/openai/{endpoint}` | `proxy/pass_through_endpoints/llm_passthrough_endpoints.py:2479` |
| `WEBSOCKET` | `/openai_passthrough/{endpoint:path}` | `/openai_passthrough/{endpoint}` | `proxy/pass_through_endpoints/llm_passthrough_endpoints.py:2478` |
| `GET` | `/vertex_ai/{endpoint}` | `/vertex_ai/{endpoint}` | `catalog.families:0` |
| `POST` | `/vertex_ai/{endpoint}` | `/vertex_ai/{endpoint}` | `catalog.families:0` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`上游原生 body；认证用虚拟 Key，上游密钥由部署提供`。
