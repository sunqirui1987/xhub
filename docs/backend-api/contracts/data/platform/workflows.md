...
|---|---|---|---|
| `GET` | `/v1/workflows/runs` | `/v1/workflows/runs` | `proxy/management_endpoints/workflow_management_endpoints.py:229` |
| `POST` | `/v1/workflows/runs` | `/v1/workflows/runs` | `proxy/management_endpoints/workflow_management_endpoints.py:194` |
| `GET` | `/v1/workflows/runs/{run_id}` | `/v1/workflows/runs/{run_id}` | `proxy/management_endpoints/workflow_management_endpoints.py:274` |
| `PATCH` | `/v1/workflows/runs/{run_id}` | `/v1/workflows/runs/{run_id}` | `proxy/management_endpoints/workflow_management_endpoints.py:308` |
| `GET` | `/v1/workflows/runs/{run_id}/events` | `/v1/workflows/runs/{run_id}/events` | `proxy/management_endpoints/workflow_management_endpoints.py:419` |
| `POST` | `/v1/workflows/runs/{run_id}/events` | `/v1/workflows/runs/{run_id}/events` | `proxy/management_endpoints/workflow_management_endpoints.py:353` |
| `GET` | `/v1/workflows/runs/{run_id}/messages` | `/v1/workflows/runs/{run_id}/messages` | `proxy/management_endpoints/workflow_management_endpoints.py:503` |
| `POST` | `/v1/workflows/runs/{run_id}/messages` | `/v1/workflows/runs/{run_id}/messages` | `proxy/management_endpoints/workflow_management_endpoints.py:449` |

## 请求

数据面虚拟 Key，管理面 session/master key。OAuth 回调走浏览器 session。见 [鉴权决策](../../../architecture/auth-decision.md)。

冻结字段（不得改名）：`messages content; patch: status`。
...
冻结响应字段：`run_id`、`status`、`events[]`、`messages[]`。
...
Console: /workflows。动作对照 [前端绑定](../../../../frontend/binding.md) 与 [页面规格](../../../../frontend/pages/README.md)。无控制台入口的路径仍须对 SDK/curl 可用。
