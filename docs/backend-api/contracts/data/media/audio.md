...
|---|---|---|---|
| `POST` | `/audio/speech` | `/audio/speech` | `proxy/proxy_server.py:11552` |
| `POST` | `/audio/transcriptions` | `/audio/transcriptions` | `proxy/proxy_server.py:11680` |
| `POST` | `/v1/audio/speech` | `/v1/audio/speech` | `proxy/proxy_server.py:11547` |
| `POST` | `/v1/audio/transcriptions` | `/v1/audio/transcriptions` | `proxy/proxy_server.py:11675` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`speech: model`、`input`、`voice`、`response_format`、`speed; transcriptions: file`、`model`、`language`、`prompt`、`response_format`、`temperature`。
...
冻结响应字段：`speech: audio bytes; transcriptions: text`、`language`、`duration`、`segments`。
