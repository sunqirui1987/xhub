...
|---|---|---|---|
| `POST` | `/images/edits` | `/images/edits` | `proxy/image_endpoints/endpoints.py:228` |
| `POST` | `/images/generations` | `/images/generations` | `proxy/image_endpoints/endpoints.py:65` |
| `POST` | `/openai/deployments/{model:path}/images/edits` | `/openai/deployments/{model}/images/edits` | `proxy/image_endpoints/endpoints.py:233` |
| `POST` | `/openai/deployments/{model:path}/images/generations` | `/openai/deployments/{model}/images/generations` | `proxy/image_endpoints/endpoints.py:71` |
| `POST` | `/v1/images/edits` | `/v1/images/edits` | `proxy/image_endpoints/endpoints.py:223` |
| `POST` | `/v1/images/generations` | `/v1/images/generations` | `proxy/image_endpoints/endpoints.py:59` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`model`、`prompt`、`n`、`size`、`quality`、`response_format`、`user; edits also image (multipart)`。
