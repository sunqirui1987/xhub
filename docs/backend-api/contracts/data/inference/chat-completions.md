...
本族是网关下列 HTTP 路径的完整分母。字段名与默认值以冻结契约及 OpenAI Chat Completions + 网关扩展字段 为准，网关不得改名。
...
|---|---|---|---|
| `POST` | `/chat/completions` | `/chat/completions` | `proxy/proxy_server.py:10958` |
| `POST` | `/cursor/chat/completions` | `/cursor/chat/completions` | `proxy/response_api_endpoints/endpoints.py:469` |
| `GET` | `/cursor/v1/models` | `/cursor/v1/models` | `proxy/response_api_endpoints/endpoints.py:447` |
| `POST` | `/engines/{model:path}/chat/completions` | `/engines/{model}/chat/completions` | `proxy/proxy_server.py:10963` |
| `POST` | `/openai/deployments/{model:path}/chat/completions` | `/openai/deployments/{model}/chat/completions` | `proxy/proxy_server.py:10968` |
| `POST` | `/queue/chat/completions` | `/queue/chat/completions` | `proxy/proxy_server.py:15697` |
| `POST` | `/v1/chat/completions` | `/v1/chat/completions` | `proxy/proxy_server.py:10953` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`model`、`messages`、`temperature`、`max_tokens`、`max_completion_tokens`、`n`、`stream`、`stream_options`、`tools`、`tool_choice`、`parallel_tool_calls`、`response_format`、`user`、`seed`、`stop`、`presence_penalty`、`frequency_penalty`、`logit_bias`、`logprobs`、`top_logprobs`、`reasoning_effort`、`guardrails`、`caching`、`fallbacks`、`metadata`。
...
冻结响应字段：`id`、`object`、`created`、`model`、`choices`、`usage`、`system_fingerprint`。
