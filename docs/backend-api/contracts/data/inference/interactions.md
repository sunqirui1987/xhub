...
|---|---|---|---|
| `POST` | `/interactions` | `/interactions` | `proxy/google_endpoints/endpoints.py:226` |
| `DELETE` | `/interactions/{interaction_id}` | `/interactions/{interaction_id}` | `proxy/google_endpoints/endpoints.py:380` |
| `GET` | `/interactions/{interaction_id}` | `/interactions/{interaction_id}` | `proxy/google_endpoints/endpoints.py:312` |
| `POST` | `/interactions/{interaction_id}/cancel` | `/interactions/{interaction_id}/cancel` | `proxy/google_endpoints/endpoints.py:448` |
| `POST` | `/v1beta/interactions` | `/v1beta/interactions` | `proxy/google_endpoints/endpoints.py:220` |
| `DELETE` | `/v1beta/interactions/{interaction_id}` | `/v1beta/interactions/{interaction_id}` | `proxy/google_endpoints/endpoints.py:374` |
| `GET` | `/v1beta/interactions/{interaction_id}` | `/v1beta/interactions/{interaction_id}` | `proxy/google_endpoints/endpoints.py:306` |
| `POST` | `/v1beta/interactions/{interaction_id}/cancel` | `/v1beta/interactions/{interaction_id}/cancel` | `proxy/google_endpoints/endpoints.py:442` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`model`、`input`、`store`。
