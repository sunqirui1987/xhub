...
|---|---|---|---|
| `POST` | `/embeddings` | `/embeddings` | `proxy/proxy_server.py:11301` |
| `POST` | `/engines/{model:path}/embeddings` | `/engines/{model}/embeddings` | `proxy/proxy_server.py:11307` |
| `POST` | `/openai/deployments/{model:path}/embeddings` | `/openai/deployments/{model}/embeddings` | `proxy/proxy_server.py:11313` |
| `POST` | `/v1/embeddings` | `/v1/embeddings` | `proxy/proxy_server.py:11295` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`model`、`input`、`encoding_format`、`dimensions`、`user`。
...
冻结响应字段：`object`、`data`、`model`、`usage`。
