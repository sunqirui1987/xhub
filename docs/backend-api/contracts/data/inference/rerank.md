...
|---|---|---|---|
| `POST` | `/rerank` | `/rerank` | `proxy/rerank_endpoints/endpoints.py:35` |
| `POST` | `/v1/rerank` | `/v1/rerank` | `proxy/rerank_endpoints/endpoints.py:29` |
| `POST` | `/v2/rerank` | `/v2/rerank` | `proxy/rerank_endpoints/endpoints.py:23` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`model`、`query`、`documents`、`top_n`、`return_documents`。
