...
|---|---|---|---|
| `POST` | `/v1/messages` | `/v1/messages` | `proxy/anthropic_endpoints/endpoints.py:93` |
| `POST` | `/v1/messages/count_tokens` | `/v1/messages/count_tokens` | `proxy/anthropic_endpoints/endpoints.py:260` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`model`、`messages`、`max_tokens`、`system`、`tools`、`tool_choice`、`thinking`、`temperature`、`stream`、`metadata`。
...
冻结响应字段：`id`、`type`、`role`、`content`、`model`、`stop_reason`、`usage`。
