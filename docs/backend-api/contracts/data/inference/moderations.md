...
|---|---|---|---|
| `POST` | `/moderations` | `/moderations` | `proxy/proxy_server.py:11424` |
| `POST` | `/v1/moderations` | `/v1/moderations` | `proxy/proxy_server.py:11418` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`model`、`input`。
