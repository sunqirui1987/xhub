# Vector Stores (data plane)

- Family: `data.vector_stores`
- Status: `specified`
- 参考: 下表定位列
...
|---|---|---|---|
| `GET` | `/v1/vector_stores` | `/v1/vector_stores` | `proxy/vector_store_endpoints/endpoints.py:349` |
| `POST` | `/v1/vector_stores` | `/v1/vector_stores` | `proxy/vector_store_endpoints/endpoints.py:179` |
| `POST` | `/v1/vector_stores/{vector_store_id:path}/search` | `/v1/vector_stores/{vector_store_id}/search` | `proxy/vector_store_endpoints/endpoints.py:100` |
| `DELETE` | `/v1/vector_stores/{vector_store_id}` | `/v1/vector_stores/{vector_store_id}` | `proxy/vector_store_endpoints/endpoints.py:485` |
| `GET` | `/v1/vector_stores/{vector_store_id}` | `/v1/vector_stores/{vector_store_id}` | `proxy/vector_store_endpoints/endpoints.py:286` |
| `POST` | `/v1/vector_stores/{vector_store_id}` | `/v1/vector_stores/{vector_store_id}` | `proxy/vector_store_endpoints/endpoints.py:419` |
| `GET` | `/vector_stores` | `/vector_stores` | `proxy/vector_store_endpoints/endpoints.py:350` |
| `POST` | `/vector_stores` | `/vector_stores` | `proxy/vector_store_endpoints/endpoints.py:180` |
| `POST` | `/vector_stores/{vector_store_id:path}/search` | `/vector_stores/{vector_store_id}/search` | `proxy/vector_store_endpoints/endpoints.py:104` |
| `DELETE` | `/vector_stores/{vector_store_id}` | `/vector_stores/{vector_store_id}` | `proxy/vector_store_endpoints/endpoints.py:486` |
| `GET` | `/vector_stores/{vector_store_id}` | `/vector_stores/{vector_store_id}` | `proxy/vector_store_endpoints/endpoints.py:287` |
| `POST` | `/vector_stores/{vector_store_id}` | `/vector_stores/{vector_store_id}` | `proxy/vector_store_endpoints/endpoints.py:420` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`name`、`file_ids`、`expires_after`、`metadata; search: query`、`max_num_results`。
...
冻结响应字段：`id`、`object`、`name`、`status`、`file_counts`、`created_at`。
