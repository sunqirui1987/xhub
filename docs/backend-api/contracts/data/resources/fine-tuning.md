...
|---|---|---|---|
| `GET` | `/fine_tuning/jobs` | `/fine_tuning/jobs` | `proxy/fine_tuning_endpoints/endpoints.py:374` |
| `POST` | `/fine_tuning/jobs` | `/fine_tuning/jobs` | `proxy/fine_tuning_endpoints/endpoints.py:68` |
| `GET` | `/fine_tuning/jobs/{fine_tuning_job_id:path}` | `/fine_tuning/jobs/{fine_tuning_job_id}` | `proxy/fine_tuning_endpoints/endpoints.py:228` |
| `POST` | `/fine_tuning/jobs/{fine_tuning_job_id:path}/cancel` | `/fine_tuning/jobs/{fine_tuning_job_id}/cancel` | `proxy/fine_tuning_endpoints/endpoints.py:501` |
| `GET` | `/v1/fine_tuning/jobs` | `/v1/fine_tuning/jobs` | `proxy/fine_tuning_endpoints/endpoints.py:368` |
| `POST` | `/v1/fine_tuning/jobs` | `/v1/fine_tuning/jobs` | `proxy/fine_tuning_endpoints/endpoints.py:62` |
| `GET` | `/v1/fine_tuning/jobs/{fine_tuning_job_id:path}` | `/v1/fine_tuning/jobs/{fine_tuning_job_id}` | `proxy/fine_tuning_endpoints/endpoints.py:222` |
| `POST` | `/v1/fine_tuning/jobs/{fine_tuning_job_id:path}/cancel` | `/v1/fine_tuning/jobs/{fine_tuning_job_id}/cancel` | `proxy/fine_tuning_endpoints/endpoints.py:495` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`model`、`training_file`、`hyperparameters`、`suffix`、`validation_file`。
...
冻结响应字段：`id`、`object`、`model`、`status`、`created_at`、`fine_tuned_model`。
