...
|---|---|---|---|
| `GET` | `/batches` | `/batches` | `proxy/batches_endpoints/endpoints.py:691` |
| `POST` | `/batches` | `/batches` | `proxy/batches_endpoints/endpoints.py:107` |
| `GET` | `/batches/{batch_id:path}` | `/batches/{batch_id}` | `proxy/batches_endpoints/endpoints.py:400` |
| `POST` | `/batches/{batch_id:path}/cancel` | `/batches/{batch_id}/cancel` | `proxy/batches_endpoints/endpoints.py:873` |
| `GET` | `/v1/batches` | `/v1/batches` | `proxy/batches_endpoints/endpoints.py:686` |
| `POST` | `/v1/batches` | `/v1/batches` | `proxy/batches_endpoints/endpoints.py:102` |
| `GET` | `/v1/batches/{batch_id:path}` | `/v1/batches/{batch_id}` | `proxy/batches_endpoints/endpoints.py:395` |
| `POST` | `/v1/batches/{batch_id:path}/cancel` | `/v1/batches/{batch_id}/cancel` | `proxy/batches_endpoints/endpoints.py:868` |
| `GET` | `/{provider}/v1/batches` | `/{provider}/v1/batches` | `proxy/batches_endpoints/endpoints.py:681` |
| `POST` | `/{provider}/v1/batches` | `/{provider}/v1/batches` | `proxy/batches_endpoints/endpoints.py:97` |
| `GET` | `/{provider}/v1/batches/{batch_id:path}` | `/{provider}/v1/batches/{batch_id}` | `proxy/batches_endpoints/endpoints.py:390` |
| `POST` | `/{provider}/v1/batches/{batch_id:path}/cancel` | `/{provider}/v1/batches/{batch_id}/cancel` | `proxy/batches_endpoints/endpoints.py:863` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`input_file_id`、`endpoint`、`completion_window`、`metadata`。
...
冻结响应字段：`id`、`object`、`endpoint`、`status`、`input_file_id`、`output_file_id`、`request_counts`、`created_at`。
