...
|---|---|---|---|
| `GET` | `/containers` | `/containers` | `proxy/container_endpoints/endpoints.py:167` |
| `POST` | `/containers` | `/containers` | `proxy/container_endpoints/endpoints.py:35` |
| `DELETE` | `/containers/{container_id}` | `/containers/{container_id}` | `proxy/container_endpoints/endpoints.py:377` |
| `GET` | `/containers/{container_id}` | `/containers/{container_id}` | `proxy/container_endpoints/endpoints.py:272` |
| `GET` | `/v1/containers` | `/v1/containers` | `proxy/container_endpoints/endpoints.py:161` |
| `POST` | `/v1/containers` | `/v1/containers` | `proxy/container_endpoints/endpoints.py:29` |
| `DELETE` | `/v1/containers/{container_id}` | `/v1/containers/{container_id}` | `proxy/container_endpoints/endpoints.py:371` |
| `GET` | `/v1/containers/{container_id}` | `/v1/containers/{container_id}` | `proxy/container_endpoints/endpoints.py:266` |

## 请求

`Authorization: Bearer sk-...` 或 `x-litellm-api-key`。`key_type` 为 `llm_api` 或 `default`。master key 默认不能调本族，除非 `general_settings` 显式允许。

冻结字段（不得改名）：`name`、`expires_after`、`file_ids`。
