...
- Family: `mgmt.callbacks`
- Status: `specified`
- 参考: 下表定位列
- Console: /logging-and-alerts /chat/integrations
- Auth: management
...
|---|---|---|---|
| `GET` | `/active/callbacks` | `/active/callbacks` | `proxy/health_endpoints/_health_endpoints.py:1556` |
| `GET` | `/callbacks/configs` | `/callbacks/configs` | `proxy/management_endpoints/callback_management_endpoints.py:35` |
| `GET` | `/callbacks/list` | `/callbacks/list` | `proxy/management_endpoints/callback_management_endpoints.py:17` |
| `GET` | `/get/config/callbacks` | `/get/config/callbacks` | `proxy/proxy_server.py:17960` |
| `GET` | `/logs` | `/logs` | `catalog.families:0` |
| `POST` | `/logs` | `/logs` | `catalog.families:0` |

## 请求

master key、SSO session，或 `key_type=management` / 具备对应角色的虚拟 Key。按 `user_api_key_auth`。

冻结字段（不得改名）：`callback name`、`callback_vars`。
...
冻结响应字段：`callbacks[]`、`success_callback[]`、`failure_callback[]`。
...
Console: /logging-and-alerts /chat/integrations。动作对照 [前端绑定](../../../../frontend/binding.md) 与 [页面规格](../../../../frontend/pages/README.md)。无控制台入口的路径仍须对 SDK/curl 可用。
