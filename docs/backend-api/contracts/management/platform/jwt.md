...
|---|---|---|---|
| `POST` | `/jwt/key/mapping/delete` | `/jwt/key/mapping/delete` | `proxy/management_endpoints/jwt_key_mapping_endpoints.py:206` |
| `GET` | `/jwt/key/mapping/info` | `/jwt/key/mapping/info` | `proxy/management_endpoints/jwt_key_mapping_endpoints.py:277` |
| `GET` | `/jwt/key/mapping/list` | `/jwt/key/mapping/list` | `proxy/management_endpoints/jwt_key_mapping_endpoints.py:239` |
| `POST` | `/jwt/key/mapping/new` | `/jwt/key/mapping/new` | `proxy/management_endpoints/jwt_key_mapping_endpoints.py:92` |
| `POST` | `/jwt/key/mapping/update` | `/jwt/key/mapping/update` | `proxy/management_endpoints/jwt_key_mapping_endpoints.py:144` |

## 请求

master key、SSO session，或 `key_type=management` / 具备对应角色的虚拟 Key。按 `user_api_key_auth`。

冻结字段（不得改名）：`jwt_claim_name`、`jwt_claim_value`、`token`、`description`。
...
冻结响应字段：`id`、`jwt_claim_name`、`jwt_claim_value`。
...
Console: /admin-panel。动作对照 [前端绑定](../../../../frontend/binding.md) 与 [页面规格](../../../../frontend/pages/README.md)。无控制台入口的路径仍须对 SDK/curl 可用。
