...
|---|---|---|---|
| `GET` | `/log-event` | `/log-event` | `catalog.families:0` |
| `POST` | `/log-event` | `/log-event` | `catalog.families:0` |
| `GET` | `/network/client-ip` | `/network/client-ip` | `proxy/management_endpoints/mcp_management_endpoints.py:975` |
| `GET` | `/provider/budgets` | `/provider/budgets` | `proxy/spend_tracking/spend_management_endpoints.py:4156` |
| `GET` | `/robots.txt` | `/robots.txt` | `catalog.families:0` |
| `POST` | `/robots.txt` | `/robots.txt` | `catalog.families:0` |
| `GET` | `/user/available_users` | `/user/available_users` | `catalog.families:0` |
| `POST` | `/user/available_users` | `/user/available_users` | `catalog.families:0` |
| `GET` | `/v1/vector_store/list` | `/v1/vector_store/list` | `proxy/vector_store_endpoints/management_endpoints.py:320` |

## 请求

master key、SSO session，或 `key_type=management` / 具备对应角色的虚拟 Key。按 `user_api_key_auth`。

冻结字段（不得改名）：`log-event payload; available_users query`。
