...
本族是网关下列 HTTP 路径的完整分母。字段名与默认值以冻结契约及 SCIM 2.0 User/Group 为准，网关不得改名。
...
|---|---|---|---|
| `GET` | `/Groups` | `/Groups` | `catalog.families:0` |
| `POST` | `/Groups` | `/Groups` | `catalog.families:0` |
| `GET` | `/ResourceTypes` | `/ResourceTypes` | `catalog.families:0` |
| `POST` | `/ResourceTypes` | `/ResourceTypes` | `catalog.families:0` |
| `GET` | `/Schemas` | `/Schemas` | `catalog.families:0` |
| `POST` | `/Schemas` | `/Schemas` | `catalog.families:0` |
| `GET` | `/ServiceProviderConfig` | `/ServiceProviderConfig` | `catalog.families:0` |
| `POST` | `/ServiceProviderConfig` | `/ServiceProviderConfig` | `catalog.families:0` |
| `GET` | `/Users` | `/Users` | `catalog.families:0` |
| `POST` | `/Users` | `/Users` | `catalog.families:0` |

## 请求

master key、SSO session，或 `key_type=management` / 具备对应角色的虚拟 Key。按 `user_api_key_auth`。

冻结字段（不得改名）：`SCIM User: userName`、`emails`、`active`、`name; Group: displayName`、`members`。
...
冻结响应字段：`id`、`userName`、`meta`、`schemas`。
