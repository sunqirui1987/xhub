...
|---|---|---|---|
| `GET` | `/audit` | `/audit` | `catalog.families:0` |
| `POST` | `/audit` | `/audit` | `catalog.families:0` |
| `GET` | `/audit/{id}` | `/audit/{id}` | `catalog.families:0` |
| `POST` | `/audit/{id}` | `/audit/{id}` | `catalog.families:0` |

## 请求

master key、SSO session，或 `key_type=management` / 具备对应角色的虚拟 Key。按 `user_api_key_auth`。

冻结字段（不得改名）：`query: start_date`、`end_date`、`changed_by`、`table_name`。
...
冻结响应字段：`id`、`updated_at`、`changed_by`、`changed_by_api_key`、`table_name`、`object_id`、`action`、`before_value`、`updated_values`。
