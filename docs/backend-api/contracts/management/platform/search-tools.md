...
|---|---|---|---|
| `POST` | `/search_tools` | `/search_tools` | `proxy/search_endpoints/search_tool_management.py:267` |
| `GET` | `/search_tools/list` | `/search_tools/list` | `proxy/search_endpoints/search_tool_management.py:135` |
| `POST` | `/search_tools/test_connection` | `/search_tools/test_connection` | `proxy/search_endpoints/search_tool_management.py:555` |
| `GET` | `/search_tools/ui/available_providers` | `/search_tools/ui/available_providers` | `proxy/search_endpoints/search_tool_management.py:646` |
| `DELETE` | `/search_tools/{search_tool_id}` | `/search_tools/{search_tool_id}` | `proxy/search_endpoints/search_tool_management.py:422` |
| `GET` | `/search_tools/{search_tool_id}` | `/search_tools/{search_tool_id}` | `proxy/search_endpoints/search_tool_management.py:478` |
| `PUT` | `/search_tools/{search_tool_id}` | `/search_tools/{search_tool_id}` | `proxy/search_endpoints/search_tool_management.py:339` |

## 请求

master key、SSO session，或 `key_type=management` / 具备对应角色的虚拟 Key。按 `user_api_key_auth`。

冻结字段（不得改名）：`search_tool_name`、`litellm_params (search_provider`、`api_key)`。
...
冻结响应字段：`search_tool_id`、`search_tool_name`。
