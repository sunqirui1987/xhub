...
|---|---|---|---|
| `POST` | `/compliance/eu-ai-act` | `/compliance/eu-ai-act` | `proxy/management_endpoints/compliance_endpoints.py:26` |
| `POST` | `/compliance/gdpr` | `/compliance/gdpr` | `proxy/management_endpoints/compliance_endpoints.py:55` |

## 请求

master key、SSO session，或 `key_type=management` / 具备对应角色的虚拟 Key。按 `user_api_key_auth`。

冻结字段（不得改名）：`query: regime`、`start_date`、`end_date`。
