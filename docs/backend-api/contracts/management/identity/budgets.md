...
|---|---|---|---|
| `POST` | `/budget/delete` | `/budget/delete` | `proxy/management_endpoints/budget_management_endpoints.py:328` |
| `POST` | `/budget/info` | `/budget/info` | `proxy/management_endpoints/budget_management_endpoints.py:203` |
| `GET` | `/budget/list` | `/budget/list` | `proxy/management_endpoints/budget_management_endpoints.py:300` |
| `POST` | `/budget/new` | `/budget/new` | `proxy/management_endpoints/budget_management_endpoints.py:35` |
| `GET` | `/budget/settings` | `/budget/settings` | `proxy/management_endpoints/budget_management_endpoints.py:232` |
| `POST` | `/budget/update` | `/budget/update` | `proxy/management_endpoints/budget_management_endpoints.py:118` |
| `GET` | `/budgets` | `/budgets` | `proxy/management_endpoints/management_v1/budgets.py:138` |

## 请求

master key、SSO session，或 `key_type=management` / 具备对应角色的虚拟 Key。按 `user_api_key_auth`。

冻结字段（不得改名）：`max_budget`、`soft_budget`、`tpm_limit`、`rpm_limit`、`model_max_budget`、`budget_duration`、`max_parallel_requests`。
...
冻结响应字段：`budget_id`、`max_budget`、`tpm_limit`、`rpm_limit`、`budget_duration`、`budget_reset_at`。
...
Console: /budgets。动作对照 [前端绑定](../../../../frontend/binding.md) 与 [页面规格](../../../../frontend/pages/README.md)。无控制台入口的路径仍须对 SDK/curl 可用。
