...
- Family: `mgmt.invitations`
- Status: `specified`
- 参考: 下表定位列
- Console: /onboarding /users
- Auth: management
...
|---|---|---|---|
| `POST` | `/invitation/delete` | `/invitation/delete` | `proxy/proxy_server.py:16778` |
| `GET` | `/invitation/info` | `/invitation/info` | `proxy/proxy_server.py:16676` |
| `POST` | `/invitation/new` | `/invitation/new` | `proxy/proxy_server.py:16605` |
| `POST` | `/invitation/update` | `/invitation/update` | `proxy/proxy_server.py:16721` |

## 请求

master key、SSO session，或 `key_type=management` / 具备对应角色的虚拟 Key。按 `user_api_key_auth`。

冻结字段（不得改名）：`user_id`、`user_email`。
...
冻结响应字段：`id`、`user_id`、`is_accepted`、`expires`。
...
Console: /onboarding /users。动作对照 [前端绑定](../../../../frontend/binding.md) 与 [页面规格](../../../../frontend/pages/README.md)。无控制台入口的路径仍须对 SDK/curl 可用。
