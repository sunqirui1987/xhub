...
|---|---|---|---|
| `POST` | `/onboarding/claim_token` | `/onboarding/claim_token` | `proxy/proxy_server.py:16332` |
| `GET` | `/onboarding/get_token` | `/onboarding/get_token` | `proxy/proxy_server.py:16133` |

## 请求

数据面虚拟 Key，管理面 session/master key。OAuth 回调走浏览器 session。见 [鉴权决策](../../../architecture/auth-decision.md)。

冻结字段（不得改名）：`claim: invitation_id`、`password`、`user_email`。
...
Console: /onboarding。动作对照 [前端绑定](../../../../frontend/binding.md) 与 [页面规格](../../../../frontend/pages/README.md)。无控制台入口的路径仍须对 SDK/curl 可用。
