...
- Family: `mgmt.projects`
- Status: `independent-impl`
- 参考: 下表定位列
- Console: /projects
- Auth: management
...
|---|---|---|---|
| `GET` | `/project/delete` | `/project/delete` | `catalog.families:0` |
| `POST` | `/project/delete` | `/project/delete` | `catalog.families:0` |
| `GET` | `/project/info` | `/project/info` | `catalog.families:0` |
| `POST` | `/project/info` | `/project/info` | `catalog.families:0` |
| `GET` | `/project/list` | `/project/list` | `catalog.families:0` |
| `POST` | `/project/list` | `/project/list` | `catalog.families:0` |
| `GET` | `/project/new` | `/project/new` | `catalog.families:0` |
| `POST` | `/project/new` | `/project/new` | `catalog.families:0` |
| `GET` | `/project/update` | `/project/update` | `catalog.families:0` |
| `POST` | `/project/update` | `/project/update` | `catalog.families:0` |

## 请求

master key、SSO session，或 `key_type=management` / 具备对应角色的虚拟 Key。按 `user_api_key_auth`。

冻结字段（不得改名）：`project_alias`、`team_id`、`organization_id`、`max_budget`、`models`、`metadata`。
...
冻结响应字段：`project_id`、`project_alias`、`team_id`、`spend`、`max_budget`、`blocked`、`created_at`。
...
Console: /projects。动作对照 [前端绑定](../../../../frontend/binding.md) 与 [页面规格](../../../../frontend/pages/README.md)。无控制台入口的路径仍须对 SDK/curl 可用。
